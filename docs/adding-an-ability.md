...
If the ability introduces a new stat or config flag the engine needs to read, follow the four-step bridge sequence. Compare runs the Rust engine only, so a TS-only change is a silent no-op in Compare - skip any of these four steps and the field is dropped at the bridge. This is the most common source of "I added the field but Compare ignores it" bugs.
...
1. **Reference entry** in `src/pages/referenceContent.ts`:
   ...
   ```
2. **Wire the field**: not needed here - the aura keys off the caster's passive-ability list rather than a new stat, so the bridge sequence is skipped.
...
Lay out the options and your recommendation so the maintainer can decide which way to go.
