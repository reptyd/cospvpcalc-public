 ...
 * Unreachable engine files are dropped from the bundle by Vite's
 * tree-shaker, but they still weigh on developer attention, so this
 * surfaces them for manual cleanup.
 *
...
// Matches static `from "x"`, side-effect `import "x"`, and dynamic `import("x")`
// (the `\(?` — several live modules are only reached through a lazy `import()`,
// so missing them would falsely flag a reachable file as an orphan).
const importPattern = /(?:\bfrom|\bimport)\s*\(?\s*['\"]([^'\"]+)['\"]/g;
