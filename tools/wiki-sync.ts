 ...
 *   npx tsx tools/wiki-sync.ts --no-icons      # skip the backfill; every run without it fetches
 *                                             # an icon for each creature that has none locally
...
import { buildSubspeciesSet, type SubspeciesOverrides } from "../src/shared/subspecies";
...
const MANUAL_OVERRIDES_FILE = path.join(DATA, "manual_overrides.json");
const SUBSPECIES_FILE = path.join(DATA, "subspecies.json");
...
// Subspecies (e.g. "Icebreaker Meorlark") are ordinary synced creatures now
// that the wiki lists them as such - they are auto-detected from the roster
// rather than hand-listed. The only special-case: an early-authored subspecies
// not yet on the wiki must not be flagged for removal. These manual overrides
// patch the rare heuristic miss/false positive; see src/shared/subspecies.ts.
const SUBSPECIES_OVERRIDES = JSON.parse(
  fs.readFileSync(SUBSPECIES_FILE, "utf8"),
) as SubspeciesOverrides;
...
interface UserSpecificAbilityChange {
  source: string;
  note: string;
  // null when the override removes the ability (empty replacement) rather than
  // replacing it with one or more abilities.
  local: AbilityRef | null;
  wiki: AbilityRef | null;
  reason: "wiki-missing" | "wiki-different" | "user-removed";
}
...
interface UserSpecificAbilityOverride {
  ...
  note: string;
  /**
   * Anchor: the value the wiki's matched ability is expected to currently hold
   * (the wrong value this override corrects). The override applies ONLY while
   * the live wiki still shows this value. The moment the wiki changes that
   * ability's value to anything else - it fixed it, rebalanced it, or broke it
   * differently - the override EXPIRES: it is not applied, the live wiki value
   * flows through unchanged, and a loud warning asks for it to be removed or
   * updated. This stops a stale override from silently forcing an old value
   * after the wiki moves on. Omit for legacy always-apply behavior.
   */
  expectedWikiValue?: AbilityRef["value"];
}

interface UserSpecificStatOverride {
  creature: string;
  field: keyof CreatureStats;
  value: CreatureStats[keyof CreatureStats];
  source: string;
  note: string;
  /** Anchor: the wiki stat value this override corrects. Same auto-expiry rule
   *  as the ability override's `expectedWikiValue` - the override applies only
   *  while the live wiki still shows this value, and expires (loud warning, not
   *  applied) the moment the wiki changes it. Omit for legacy always-apply. */
  expectedWikiValue?: CreatureStats[keyof CreatureStats];
}

/**
 * A manual override entry as persisted in data/manual_overrides.json. Both
 * wiki-sync and the early-changes tool read this file, so the overlay layer
 * lives in one place instead of a hardcoded array. `kind` discriminates an
 * ability override (replace/remove one of the wiki's abilities) from a stat
 * override (force a stat value). Lifecycle:
 *   - `expectedWikiValue` (anchor): applies only while the live wiki still
 *     shows this value; expires the moment the wiki moves on.
 *   - `expiresAt` (ISO date): a hard cutoff; the override stops applying once
 *     that date is reached.
 *   - `retiredAt` (ISO date): set when it is manually retired; never applies.
 * `channel` records provenance ("early" pre-wiki data vs a "user-specific"
 * report) and drives which tool surfaces it.
 */
type ManualOverrideEntry = {
  id: string;
  channel: "early" | "user-specific";
  creature: string;
  source: string;
  note: string;
  expiresAt?: string;
...
/**
 * Load the manual-override layer, dropping entries past their lifecycle:
 * `retiredAt` set (manually retired) or `expiresAt` reached. Anchor
 * (`expectedWikiValue`) expiry is evaluated later against the live wiki, per
 * creature. Date-expired entries print a loud one-liner so they get cleaned up.
 */
function loadManualOverrides(): {
  ability: UserSpecificAbilityOverride[];
  stat: UserSpecificStatOverride[];
} {
  const entries = JSON.parse(
    fs.readFileSync(MANUAL_OVERRIDES_FILE, "utf8"),
  ) as ManualOverrideEntry[];
  const now = Date.now();
  const ability: UserSpecificAbilityOverride[] = [];
  const stat: UserSpecificStatOverride[] = [];
  for (const entry of entries) {
    if (entry.retiredAt) continue;
    if (entry.expiresAt && new Date(entry.expiresAt).getTime() <= now) {
      console.warn(
        `  ⚠ manual override date-expired: ${entry.id} (${entry.creature}) ` +
          `expiresAt ${entry.expiresAt}. Not applied; live value kept. Retire or extend it.`,
      );
      continue;
    }
    if (entry.kind === "ability") {
      ability.push({
        ...
        matchAbilityName: entry.matchAbilityName,
        ability: entry.ability,
        abilities: entry.abilities,
        source: entry.source,
        note: entry.note,
        expectedWikiValue: entry.expectedWikiValue,
      });
    } else {
      stat.push({
        creature: entry.creature,
...
const { ability: USER_SPECIFIC_ABILITY_OVERRIDES, stat: USER_SPECIFIC_STAT_OVERRIDES } =
  loadManualOverrides();
...
/** Loud one-line notice that an override's anchor no longer matches the wiki, so
 *  it was skipped and the live wiki value was kept. Printed during the sync. */
function warnOverrideExpired(label: string, wikiNow: unknown, expected: unknown, note: string): void {
  console.warn(
    `  ⚠ user-specific override EXPIRED: ${label} — wiki now ${JSON.stringify(wikiNow)}, ` +
      `override expected ${JSON.stringify(expected)}. Not applied; live wiki value kept. ` +
      `Remove or update this override. note: ${note}`,
  );
}

/** A stat override is active while its anchor matches the live wiki value (or it
 *  has no anchor - legacy always-apply). Pure: callers decide whether to warn. */
function statOverrideActive(
  override: { field: keyof CreatureStats; expectedWikiValue?: CreatureStats[keyof CreatureStats] },
  wikiCreature: CreatureRuntime,
): boolean {
  if (override.expectedWikiValue === undefined) return true;
  return JSON.stringify(wikiCreature.stats[override.field]) === JSON.stringify(override.expectedWikiValue);
}

/** An ability override is active while its anchor (`expectedWikiValue`) matches
 *  the value of the wiki's same-named ability (or it has no anchor). If the wiki
 *  no longer carries that ability, or carries it with a different value, the
 *  anchor is broken and the override has expired. Pure: callers decide whether
 *  to warn. */
function userSpecificOverrideActive(
  override: UserSpecificAbilityOverride,
  wikiAbs: AbilityRef[],
): boolean {
  if (override.expectedWikiValue === undefined) return true;
  const matchName = canonicalAbilityName(userSpecificOverrideMatchName(override));
  const wikiMatch = wikiAbs.find((wa) => canonicalAbilityName(wa.name) === matchName);
  if (!wikiMatch) return false;
  return JSON.stringify(wikiMatch.value) === JSON.stringify(override.expectedWikiValue);
}
...
function applyUserSpecificStatOverrides(creature: CreatureRuntime): CreatureRuntime {
  ...
  const active = overrides.filter((override) => {
    if (statOverrideActive(override, creature)) return true;
    warnOverrideExpired(
      `${creature.name}.${String(override.field)}`,
      creature.stats[override.field],
      override.expectedWikiValue,
      override.note,
    );
    return false;
  });
  if (active.length === 0) return creature;
...
function mergeCreature(
  ...
  for (const override of userSpecificStatOverridesFor(local.name)) {
    if (!statOverrideActive(override, wiki)) continue; // expired - warned in applyUserSpecificStatOverrides
  ...
  function mergeAbilities(
    ...
    // Drop expired overrides (anchor no longer matches the wiki) before applying
    // any - the wiki value then flows through untouched. Warn once per expired.
    const userOverrides = userSpecificOverridesFor(local.name, category).filter((override) => {
      if (userSpecificOverrideActive(override, wikiAbs)) return true;
      const matchName = canonicalAbilityName(userSpecificOverrideMatchName(override));
      const wikiMatch = wikiAbs.find((wa) => canonicalAbilityName(wa.name) === matchName);
      warnOverrideExpired(
        `${local.name} / ${userSpecificOverrideMatchName(override)} (${category})`,
        wikiMatch ? wikiMatch.value : "missing",
        override.expectedWikiValue,
        override.note,
      );
      return false;
    ...
    for (const wa of wikiAbs) {
      ...
      if (localMatch) {
        ...
        if (userOverride) {
          ...
          if (replacementAbilities.length === 0) {
            // Removal override: drop the ability and record it as a visible
            // change so it both shows in the diff and is classified as a write.
          ...
          } else if (!abilityListEquivalent(replacementAbilities, [wa])) {
...
// The wiki stores image links either as a bare file path or as a full
// "/revision/latest?cb=...&format=..." URL, sometimes with a trailing slash
// and always percent-encoded. A naive `url.split("/").pop()` grabs the query
// string or an empty segment instead of the actual file name, which silently
// breaks the creature-token match below. Strip the revision/query suffix and
// decode before taking the last path segment.
function extractImageBasename(url: string): string {
  const withoutQuery = url.split("?")[0];
  const withoutRevision = withoutQuery.replace(/\/revision\/.*$/i, "");
  const raw = withoutRevision.replace(/\/+$/, "").split("/").pop() ?? "";
  try {
...
// True only if the image's file name is actually for this creature (its
// canonical token appears in the file's canonical token). The wiki's own
// per-creature `imageLink` field is occasionally wrong - a copy/paste
// mistake that still points at a valid, downloadable file for a *different*
// creature - so this check must gate any URL taken on trust, not just score it.
function iconUrlMatchesCreature(name: string, url: string): boolean {
  const basenameToken = canonicalCreatureToken(extractImageBasename(url));
  return basenameToken.includes(canonicalCreatureToken(name));
}
...
function extractCreaturePageImageCandidates(name: string, html: string): string[] {
  ...
  for (const rawMatch of matches) {
    ...
    if (!canonicalCreatureToken(extractImageBasename(candidate)).includes(token)) continue;
...
async function fetchCreaturePageApiImageCandidates(name: string): Promise<string[]> {
  ...
  try {
    ...
    const nameToken = canonicalCreatureToken(name);
    const likelyFiles = images.filter(
      (fileName) =>
        /\.(png|jpg|jpeg|webp|gif)$/i.test(fileName) &&
        canonicalCreatureToken(fileName).includes(nameToken),
    );
    ...
    // MediaWiki's `titles` param caps at 50 values for anonymous callers
    // (`toomanyvalues`, returned as a 200 with a JSON `error` body, not an
    // HTTP failure - easy to swallow silently). Creature pages with a big
    // trial/recolor gallery routinely exceed that, so batch the query.
    const candidates: string[] = [];
    for (let i = 0; i < likelyFiles.length; i += 50) {
      const batch = likelyFiles.slice(i, i + 50);
      ...
      if (imageInfoJson.error) {
        console.warn(`  imageinfo query failed for ${name}: ${imageInfoJson.error.info ?? imageInfoJson.error.code}`);
        continue;
      }
...
async function resolveIconCandidates(request: IconSyncRequest): Promise<string[]> {
  ...
  // preferredUrl/wiki.imageLink come straight from the wiki's per-creature
  // data row, which is occasionally wrong (a still-downloadable file for a
  // different creature). Unlike the page-scraped candidates below, nothing
  // else validates these, so require the token match before trusting them.
  const pushIfMatching = (candidate: string | null | undefined) => {
    if (!candidate || candidate === "N/A") return;
    if (!iconUrlMatchesCreature(request.name, candidate)) return;
    push(candidate);
  };

  pushIfMatching(request.preferredUrl);
  pushIfMatching(request.wiki?.imageLink);
...
async function downloadIcon(
  ...
  try {
    ...
    for (const imageUrl of imageUrls) {
      ...
      if (!force && fs.existsSync(outPath)) return `/icons/creatures/${fileName}`;
    ...
    lines.push(
      `  ${label} user-specific: ${diff.userSpecific
        .map((entry) => {
          ...
          if (entry.local === null) {
            return `${wikiText} removed; source ${entry.source}; ${entry.note}`;
          }
...
async function main() {
  ...
  // A creature added by a sync arrives without an icon, and the backfill that
  // fetches one used to wait for a flag nobody passed, so the icons were
  // collected by hand afterwards. Every run now backfills; --no-icons opts out.
  const downloadAllMissingIcons = !args.includes("--no-icons");
  ...
  const webhookUrl = readWebhook("COS_WIKI_SYNC_DISCORD_WEBHOOK", WEBHOOK_FILE);
  const pathsToPush = prodPushPaths(downloadIcons || downloadAllMissingIcons);

  // Icon-only refresh: re-download every local creature's icon from the wiki,
  // overwriting whatever is on disk (the normal download path skips existing
  // files). This is a pure icon pass - it does not diff/apply creature stats,
  // touch the effects catalog, or push. Commit the icon changes separately.
  if (args.includes("--icons-refresh")) {
    console.log("\n=== CoS Wiki Sync: refresh ALL creature icons ===\n");
    const luaSource = await fetchWikiModule();
    const wikiCreatures = parseLuaCreatures(luaSource);
    const localCreatures = readLocalCreatures();
    const icons = readIcons();
    console.log(`  Refreshing icons for ${localCreatures.length} local creatures...\n`);
    let downloaded = 0;
    const unresolved: string[] = [];
    for (const creature of localCreatures) {
      const wiki = wikiCreatures.get(creature.name);
      // Try the wiki module's direct image link first (one fetch, no extra
      // API calls); fall back to the full candidate resolver when it is
      // missing, unusable, or - the wiki's per-creature data row is
      // occasionally wrong - doesn't actually name this creature.
      const directUrl =
        wiki?.imageLink &&
        wiki.imageLink !== "N/A" &&
        iconUrlMatchesCreature(creature.name, wiki.imageLink)
          ? [normalizeWikiImageUrl(wiki.imageLink)]
          : [];
      let localPath = directUrl.length
        ? await downloadIcon(creature.name, directUrl, true)
        : null;
      if (!localPath) {
        const candidates = await resolveIconCandidates({ name: creature.name, wiki });
        localPath = await downloadIcon(creature.name, candidates, true);
      }
      if (localPath) {
        icons[creature.name] = localPath;
        downloaded++;
      } else {
        unresolved.push(creature.name);
      }
    }
    writeIcons(icons);
    console.log(
      `\n  Refreshed ${downloaded} icons, ${Object.keys(icons).length} in registry.`,
    );
    if (unresolved.length > 0) {
      console.log(`  Unresolved (${unresolved.length}): ${unresolved.join(", ")}`);
    }
    console.log("\n  Done (icons only - commit separately).\n");
    return;
  }
  ...
  // Subspecies derived from the combined roster (local + wiki). Used only to
  // protect an early-authored subspecies that is not on the wiki yet from being
  // flagged as removed - subspecies otherwise sync like any other creature.
  const tierByName = new Map<string, number | null>();
  for (const creature of localCreatures) tierByName.set(creature.name.toLowerCase(), creature.stats.tier);
  for (const [name, wikiData] of wikiCreatures) {
    if (!tierByName.has(name.toLowerCase())) {
      const tier = Number.parseFloat(wikiData.tier);
      tierByName.set(name.toLowerCase(), Number.isNaN(tier) ? null : tier);
    }
  }
  const subspeciesSet = buildSubspeciesSet(
    [...localCreatures.map((c) => c.name), ...wikiCreatures.keys()],
    SUBSPECIES_OVERRIDES,
    (name) => tierByName.get(name.toLowerCase()),
  );
  ...
  const removedCreatureNames = localCreatures
    ...
    .filter((name) => !wikiCreatures.has(name) && !subspeciesSet.has(name));
  ...
  if (shouldSendDiscord && webhookUrl) {
    ...
    await sendDiscordMessages(webhookUrl, messages, USER_AGENT);
