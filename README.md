...
A pre-built WASM bundle ships in `src/rust-pkg/`, so the frontend runs without a Rust toolchain. If you change Rust code, rebuild the bundle via `npm run rust:build` (`wasm-pack`) and commit the regenerated `src/rust-pkg/`. CI checks that the committed bundle still matches a fresh rebuild, but it never commits one, so an un-rebuilt change is silently absent from the running engine. Full procedure in [CONTRIBUTING.md](CONTRIBUTING.md).
...
After changing Rust, rebuild the WASM bundle with `npm run rust:build` and commit the regenerated `src/rust-pkg/`. CI never commits a rebuild of its own - the `wasm-fresh` job rebuilds only to check the committed bundle still behaves the same, so an un-rebuilt change ships inert.
