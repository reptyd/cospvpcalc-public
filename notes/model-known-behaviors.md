...
- Each item is confirmed current behavior, not a guess - linked to a test or code location where possible.
- These are deliberate or known quirks. The behavior an item describes should change only as a design decision, not as a silent cleanup; check the linked source first.
...
Source: [approximationNotes.ts](../src/engine/approximationNotes.ts)
...
- Explicit ability scope decisions live in [ability-model-scope.md](../notes/ability-model-scope.md).

### Confirmed current quirks

#### Spirit Glare auto-fire gating
- Confirmed by test: [engine.breath.test.ts](../src/engine/engine.breath.test.ts)
- Relevant code: [breathResourceRuntime.ts](../src/engine/breathResourceRuntime.ts), [breathSpecialRuntime.ts](../src/engine/breathSpecialRuntime.ts)
- Current behavior:
...
- Status runtime: [engine.statuses.test.ts](../src/engine/engine.statuses.test.ts)
- Active ability regressions: [engine.specials.test.ts](../src/engine/engine.specials.test.ts)
- Breath regressions: [engine.breath.test.ts](../src/engine/engine.breath.test.ts)
- Golden scenarios: [engine.golden.test.ts](../src/engine/engine.golden.test.ts)
- Optimizer flow regressions: [optimizerPageFlow.test.ts](../src/optimizer/optimizerPageFlow.test.ts), [optimizer.test.ts](../src/optimizer/optimizer.test.ts)
