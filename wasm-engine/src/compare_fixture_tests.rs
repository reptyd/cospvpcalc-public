...
///
/// Fixtures in `wasm-engine/fixtures/compare_matchup_contract.json` are
/// committed golden snapshots: each entry stores the Rust arguments produced by
/// the Compare mapping (`toRustComposableArgsFromCompare`) alongside the
/// expected `BestBuildsMatchupSummary`. This test loads the fixture, runs
/// `simulate_composable_matchup` on the mapped arguments, and asserts the output
/// still matches the stored summary. (The fixtures were originally generated
/// from the now-retired TS Compare oracle; they are kept as frozen golden data.)
///
