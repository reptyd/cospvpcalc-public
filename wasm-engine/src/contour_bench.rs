// Composable-engine policy bench. Measures what differs in runtime cost
// between the `reallyFast`, `fast`, and `ideal` timing policies on a single
// Kendyll matchup.
//
...
mod bench {
    ...
    fn perf_guard_reference_matchup_loop_budget() {
        ...
        // 117 -> 123: the Boreal Warden matchup is frost-themed, so raising
        // Frostbite's bite-cooldown from 2.0% to 2.5% per stack (#7) slows the
        // afflicted side's bites and lengthens the fight, adding a few events.
        // 123 -> 127: Warden's Rage precision harvest now fires under continuous
        // fire (release at the regen tick + precision OFF override), so the
        // carrier toggles Rage off/on at each harvested tick - a few more events.
        const LOOP_ITERATIONS_BUDGET: u64 = 127;
