...
pub struct BestBuildsMatchupSummary {
    ...
    pub ehp_b: f64,
    /// Damage-reduction amplification (Hunker/Guilt at full strength). The
    /// Compare view multiplies the windowed (max HP + healing-to-cutoff) pool by
    /// this to get the mode-aware Effective HP.
    #[serde(default = "default_damage_taken_multiplier", rename = "ehpMitigationMultA")]
    pub ehp_mitigation_mult_a: f64,
    #[serde(default = "default_damage_taken_multiplier", rename = "ehpMitigationMultB")]
    pub ehp_mitigation_mult_b: f64,
    ...
    /// Event-loop iterations this fight consumed, inner replays included - the
    /// engine's own deterministic cost unit (`composable::work_meter`). Best
    /// Builds sizes its search against it; nothing in the engine reads it back.
    #[serde(default, rename = "workUnits")]
    pub work_units: u64,
...
pub struct SimulationDebug {
    ...
    #[serde(default)]
    pub bite_count: u32,
    /// Clock time of every breath tick the side fired. Populated only on
    /// traced runs; each entry covers one `BREATH_TICK_SEC` of firing.
    #[serde(default)]
    pub breath_fire_times: Vec<f64>,
    #[serde(default)]
...
/// decision DSL. These never affect combat math directly -
...
pub struct CreatureIdentity {
    ...
    #[serde(default)]
    pub tier: f64,
    /// True when the creature can take to the air - a flier or a glider (it has
    /// a fly-speed or take-off stat). Drives the Aerial Dodge rule: a grounded
    /// creature never dodges. Default false => non-aerial.
    #[serde(default, rename = "isAerial")]
    pub is_aerial: bool,
}
...
pub struct SimpleCombatantStats {
    ...
    /// Breath-channel analog of `damage_taken_multiplier_on_being_bitten`.
    /// Guilt is an all-damage reducer in-game (`Guilt.OnDamage -> x0.5`), so it
    /// halves incoming breath as well as bites; bite-only reducers do not feed
    /// this field. 1.0 = no reduction.
    #[serde(default = "default_damage_taken_multiplier")]
    #[serde(rename = "damageTakenMultiplierOnBreath")]
    pub damage_taken_multiplier_on_breath: f64,
    #[serde(default)]
    ...
    /// Umbrella / all-ailment block (game `BlockAilment`, i.e. the elder
    /// `ailmentBlockPct`). Applies to every incoming ailment and, unlike the
    /// specific native/plushie blocks above, is immune to Radiation's block
    /// reduction (Radiation only weakens the per-ailment stat blocks).
    #[serde(default)]
    #[serde(rename = "elderBlockFraction")]
    pub elder_block_fraction: f64,
    ...
    /// Oceanwing plushie: percent of Muddy's own regen bonus the equipped
    /// copies add, summed over them. The game's `Muddy.OnGet` walks the
    /// wearer's plushies and lifts the 1.25 multiplier by 0.125 for each
    /// Oceanwing it finds, so one copy takes Muddy's +25% to +37.5% (50 here)
    /// and two take it to +50% (100). 0 leaves Muddy at its registry value.
    #[serde(default)]
    #[serde(rename = "muddyStrengthBoostPct")]
    pub muddy_strength_boost_pct: f64,
    ...
    /// TS bridge from the per-creature attachment list. Each id is
    ...
    /// Seconds-to-depletion of this creature's oxygen pool, sourced from
    /// `FinalStats.oxygenTime`. Consumed only by the Compare-only
    /// Oxygen/Moisture drain mode (`oxygen_moisture_mode = "underwater"`):
    /// the pool starts here and drains 1/sec, then drowning damage begins.
    /// 0 (the default) = immune underwater. Inert in every other mode, so
    /// off-path payloads round-trip byte-identical.
    #[serde(default)]
    #[serde(rename = "oxygenTime")]
    pub oxygen_time: f64,
    /// Seconds-to-depletion of this creature's moisture pool, sourced from
    /// `FinalStats.moistureTime`. Consumed only by the Compare-only
    /// Oxygen/Moisture drain mode (`oxygen_moisture_mode = "ground"`). 0
    /// (the default) = immune on ground. Inert in every other mode.
    #[serde(default)]
    #[serde(rename = "moistureTime")]
    pub moisture_time: f64,
    /// Compare-only Aerial Dodge rule: the chance (0..100) that an incoming
    /// bite or breath tick LANDS on this creature. Consulted only when the
    /// side's `aerial_dodge_active` runtime flag is seeded on
    /// (`ComposableAbilityConfig::aerial_dodge_active`). The TS bridge sets it
    /// to the per-creature configured hit chance for a flier and to 100 for a
    /// non-flier (always hit); Shredded Wings overrides it to 100 in-engine.
    /// 0 (the default) is never read while the rule is off, so off-path
    /// payloads round-trip byte-identical.
    #[serde(default)]
    #[serde(rename = "aerialDodgeHitChancePct")]
    pub aerial_dodge_hit_chance_pct: f64,
}
...
impl Default for SimpleCombatantStats {
    /// A neutral combatant with every field at its serde default: the four
    /// multiplier/threshold fields at 1.0 (sharing the `default_*` fns the
    /// `#[serde(default = "...")]` attributes use, so the two never drift),
    /// everything else zero/false/empty/`None`. Builders set only the fields
    /// that differ from this baseline via `..Default::default()`.
    fn default() -> Self {
        Self {
            health: 0.0,
            weight: 0.0,
            ...
            damage2: 0.0,
            health_regen: 0.0,
            active_cooldown_multiplier: default_active_cooldown_multiplier(),
            quick_recovery_hp_ratio_threshold: 0.0,
            unbreakable_damage_cap_pct: 0.0,
            damage_taken_multiplier_on_being_bitten: default_damage_taken_multiplier(),
            damage_taken_multiplier_on_breath: default_damage_taken_multiplier(),
            breath_resistance: 0.0,
            berserk_bite_cooldown_multiplier: default_berserk_bite_cooldown_multiplier(),
            berserk_hp_ratio_threshold: 0.0,
            first_strike_pct: 0.0,
            first_strike_hp_ratio_threshold: default_first_strike_hp_ratio_threshold(),
            ...
            elder_block_fraction: 0.0,
            plushie_reflect_avg_pct: 0.0,
            muddy_strength_boost_pct: 0.0,
            ...
            oxygen_time: 0.0,
            moisture_time: 0.0,
            aerial_dodge_hit_chance_pct: 0.0,
        }
    }
}
...
pub struct SimpleAppliedStatus {
    ...
    /// Explicit lifetime for the resulting instance, in seconds. None leaves it
    /// on the registry's per-stack decay pacing (`status_decay_sec`), which is
    /// what an ordinary on-hit application wants; a caller that knows the
    /// duration up front - a Compare buff seeded at t=0, an ability with a
    /// fixed window - pins it here instead of hand-building the instance.
    #[serde(default)]
    #[serde(rename = "remainingSec")]
    pub remaining_sec: Option<f64>,
    /// Permanent instance: the stack never decays. Same idea as
    /// `seed_permanent_status`, expressed on the application payload.
    #[serde(default)]
    #[serde(rename = "noDecay")]
    pub no_decay: bool,
    /// `"durationOnly"` when the stack count is a lifetime rather than a
    /// magnitude, so the effect reads one stack however many were applied.
    /// None leaves the stacks meaning what they say.
    #[serde(default)]
...
pub struct AbilityPolicyOverrides {
    ...
    #[serde(default, rename = "Guardians Passage")]
    pub guardians_passage: Option<SimpleAbilityTimingMode>,
    ...
    /// Per-fight active-level override for each
