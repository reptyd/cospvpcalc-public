...
// Not test-gated: `schema_hash` is exported through the WASM bridge as the
// contract version. Only the file-writing half of the module is test-only.
pub mod contract_manifest;
#[cfg(test)]
mod contract_readers;
mod actives;
mod combat;
mod compare_hunger;
mod composable;
