...
/// independent of `composable::` internals (pillar 4).
#[derive(Debug, Clone)]
pub struct PolicySide {
    ...
    /// Sprint 5.x: surfaced on the policy snapshot so user
    ...
    /// Per-side extras for user/constructor fields (pillar 3a).
    pub extras: BTreeMap<String, PolicyValue>,
    /// Round 46 / B2: sliding-window damage logs (cloned from
    ...
    /// var paths in `lookup_var`. Bite-only this round; extends to
    /// breath/DOT/trap sources alongside the A13 pre-damage hook.
    pub recent_damage_taken: Vec<(f64, f64)>,
    pub recent_damage_dealt: Vec<(f64, f64)>,
    /// Phase 5 / G9: the side's committed posture as a stable label
    ...
    /// (pillar 4). Defaults to `"Standing"` in synthetic test states.
    pub posture: String,
}
