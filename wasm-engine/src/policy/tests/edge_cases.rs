//! Pillar 5.1 - edge-case sanity per ability x per timing mode.
//!
...
//! or `ToggleDecision::is_eligible` / `really_fast_default`) - no full
...
#[test]
fn reflect_fires_whenever_available() {
    // Delete-to-gate: Reflect recasts as soon as it is available, independent of
    // opponent DPS. Utility is a flat positive when the gate fires.
    let mut state = fresh_state();
...
/// Below 50 % max HP the game's CanUse gate blocks Hunters Curse, so the
/// decision is unavailable and scores 0 - not a survival-check skip.
#[test]
fn hunters_curse_below_fifty_percent_hp_is_unavailable() {
    let mut state = fresh_state();
    state.self_side.hp = 4_000.0; // 40 % of the 10_000 max
    let d = HuntersCurseDecision::new();
    assert!(!d.is_available(&state), "HC blocked below 50 % HP");
...
#[test]
fn unbridled_rage_fires_whenever_available() {
    // Delete-to-gate: UR fires as soon as it is available (ASAP dominates), so
    // utility is a flat positive when the gate fires.
    let mut state = fresh_state();
...
fn rewind_cooldown_pending_skips() {
    use crate::policy::decisions::rewind::RESTORED_HP_DELTA_KEY;
