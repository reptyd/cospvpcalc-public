...
/// P4 regression: Ideal must NOT fire Fortify when only 1 trivial
...
/// P4 monotonicity guard: the new utility shape must preserve
/// `damage(Ideal) >= damage(ReallyFast)` at the engine-policy level.
/// The original bug the user reported was Ideal firing too early →
