//! Pillar 9 — `decide_timed` cost budget.
//!
//! The pillars doc mandates `decide_timed` per ability under Ideal
//! mode complete in ≤1 ms on the reference matchup. This test uses
//! `std::time::Instant` (no Criterion — keeps the bench in-tree and
//! sandbox-friendly per the pillars).
//!
//! Notes:
//!
