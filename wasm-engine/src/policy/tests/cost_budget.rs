...
/// Hunker's per-CALL cost is now just the pi-zero gate
/// (`AlwaysOnIfEligibleTogglePolicy`) - that is what `toggle_state_now`
/// runs every tick between epochs and for the fast modes, so it must
/// stay under the per-call budget. The precision on/off VALUE moved to
/// the engine-replay `toggle_replay_bridge`, which is re-homed to the
/// posture per-epoch tier (order-tens replays per fight, not per call,
/// so it is not under the 1 ms/call budget - see the toggle-replay cost
/// probe in `phase_tests.rs`).
#[test]
fn hunker_toggle_gate_under_one_ms() {
    let mean = measure_per_call_ms(|| {
    ...
    assert!(
        mean < BUDGET_MS_PER_CALL,
        "Hunker π₀ gate under Ideal must be <{BUDGET_MS_PER_CALL} ms/call: got {mean:.4} ms"
    );
}
