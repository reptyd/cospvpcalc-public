...
//!   non-`None` elder. `self.tier` — numeric rarity tier. Phase 5 / G8.
...
pub enum Expr {
    ...
    /// Round 34 / A1: deterministic-pseudo-random roll in `[0, 1)`.
...
impl Expr {
    ...
    pub fn eval(&self, state: &PolicyState) -> f64 {
        match self {
            ...
            Expr::Rand => {
                // Round 34 / A1: deterministic-pseudo-random roll in
...
/// Round 34 / A1: deterministic-pseudo-random roll in `[0, 1)`. Same
...
fn lookup_var(path: &str, state: &PolicyState) -> f64 {
    ...
    // Round 45 / B4: combat-meta counters seeded by the engine on
    // every bite event:
    ...
    // Round 32 / A5: compare-page environment flags. Five 0/1 numeric
    ...
    if path.starts_with("scaling.") {
        // Round 40 / A11: per-ability scaling table. The dispatcher
    ...
    let side = match side_kw {
        ...
        // Round 45 / B5: both `opponent` and `opp` resolve to the
        // other side. `opp` is the conventional spelling in the DSL
        // (`opp.hp_ratio < 0.3`); pre-B5 the resolver only matched
    ...
    match rest {
        ...
        // Phase 5 / G8: numeric creature tier (ordinal — `opp.tier >= 4`
        ...
        other => {
            ...
            // Phase 5 / G9: posture read. `<side>.is_posture.<P>` returns
            ...
            // Phase 5 / G8: creature-identity reads as boolean builtins.
            ...
            // Round 46 / B2: sliding-window damage helpers. `<N>` is the
            ...
            // Bite damage only this round; breath/DOT/trap sources extend
            // alongside the A13 pre-damage hook.
            ...
            if let Some(key) = other.strip_prefix("extra.") {
                // Round 46 / B3: `.sum` / `.last` derived reads on
...
/// Round 46 / B3: read the `.length` slot of a numbered-key extras
...
/// Round 46 / B3: sum every numeric entry of a numbered-key extras
...
/// Round 46 / B3: read the most recently-pushed entry of a
...
pub struct UserAbilitySpec {
    ...
    /// Sprint 4 built the UserTimingSpec spec layer; this completes
    ...
    /// Round 40 / A11: total number of levels this ability has.
    /// `1` (default) ⇒ no levels — ability behaves identically to a
    /// pre-A11 spec. `> 1` opens up the `scaling` table.
    #[serde(default = "default_levels")]
    pub levels: u32,
    /// Round 40 / A11: default level the spec ships with (1-indexed,
    /// clamped to `1..=levels` at registration). Compare UI will
    /// override this per matchup in Round 42; until then this is the
    /// effective level at dispatch time.
    #[serde(default = "default_levels")]
    pub default_level: u32,
    /// Round 40 / A11: named numeric scaling tables. Each entry is a
...
impl Default for UserAbilitySpec {
    /// Round 40 / A11: convenience default — empty utility / is_available,
...
impl UserAbilitySpec {
    /// Round 40 / A11: clamp `default_level` into `1..=levels` and
...
pub struct TriggerHooks {
    ...
    /// Round 37 / A7 + v2 Phase 4 (G4): fired when the actor received
    ...
    /// Round 37 / A7: fired when one or more user-defined
    ...
    /// v2 Phase 4 (G5): built-in active windows now fire too, under stable
    ...
    /// Round 43 / A13: fires on the VICTIM immediately before incoming
    ...
    /// for this round and will route through the hook in a follow-up.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub on_before_take_damage: Option<EffectBatch>,
    /// Round 43 / A13: symmetric to `on_before_take_damage` but fires
...
pub enum TriggerHook {
    ...
    OnKill,
    OnFirstStrike,
    /// Round 37 / A7: actor received healing this iteration. Event
    ...
    /// this trigger in this round. Documented as A7b.
    OnHeal,
    /// Round 37 / A7: one or more user-defined `active_until` windows
    ...
    /// Curse, etc.) ending do NOT fire this trigger in this round —
    /// only user-scoped `user_active_until` keys are tracked. Future
    /// rounds can extend to built-ins per request.
    OnActiveEnd,
    /// Round 43 / A13: pre-damage hook on the VICTIM, fires before
    /// HP changes apply. Handler can write `event.damage_override` to
    /// replace the final amount.
    OnBeforeTakeDamage,
    /// Round 43 / A13: pre-damage hook on the DEALER, fires before
    /// HP changes apply. Handler can write `event.damage_override` to
...
pub enum SpecError {
    ...
    NoEffects,
    /// Round 40 / A11: `default_level` was out of range. Always
    ...
    /// Round 40 / A11: a `scaling` entry's array length didn't match
...
impl UserAbilitySpec {
    ...
    pub fn validate(&self) -> Result<(), SpecError> {
        ...
        // Round 40 / A11: validate the level fields. `parse_user_ability_spec`
...
pub fn parse_user_ability_spec(json: &str) -> Result<UserAbilitySpec, ParseError> {
    ...
    // Round 40 / A11: normalize first (pad/truncate scaling arrays, clamp
...
mod tests {
    ...
    fn is_posture_reads_side_posture_label() {
        // Phase 5 / G9: `<side>.is_posture.<P>` resolves the committed
    ...
    fn identity_reads_type_diet_elder_tier() {
        // Phase 5 / G8: is_type / is_diet / is_elder boolean builtins +
    ...
    #[test]
    fn pre_a11_json_loads_with_default_levels_round40_a11() {
        // JSON without levels/default_level/scaling fields — pre-A11
        // saved spec. Should load as levels=1, default_level=1, empty
        // scaling.
        let json = r#"{
            "version": 1,
            "id": "user.legacy",
    ...
    fn event_heal_amount_resolves_from_extras() {
        // Round 37 / A7: on_heal trigger surfaces heal_amount.
        let mut state = default_state();
        state
            .extras
    ...
    fn event_ended_per_ability_resolves_from_extras() {
        // Round 37 / A7: on_active_end surfaces ended.<id> per ended
        // user.* active window. Mirror of the A9 applied/expired pattern.
        let mut state = default_state();
        state
            .extras
    ...
    fn event_damage_kind_flags_resolve_from_extras() {
        // Round 36 / A10: `event.is_bite` / `event.is_breath` /
    ...
    fn rand_returns_value_in_unit_interval_and_is_deterministic() {
        // Round 34 / A1: rand() returns a deterministic-pseudo-random
    ...
    fn event_applied_per_status_id_resolves_via_state_extras() {
        // Round 33 / A9: `event.applied.<status_id>` and
    ...
    fn env_var_path_resolves_via_self_side_extras() {
        // Round 32 / A5: env.* paths read from self_side.extras after
    ...
    /// built-ins do. This is the proof of pillar 3 — the engine
