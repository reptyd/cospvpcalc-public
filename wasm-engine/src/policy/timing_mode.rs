...
//! | Extreme | dense grid 0-12 s @ 0.1, 12.5-30 @ 0.5, 32-120 @ 2 | 120 s | ~200 candidates. |
//!
...
impl TimingMode {
    ...
    ///
    /// Every mode resolves toggles through "always on if eligible"
    /// (pi-zero). ReallyFast / Fast keep this cheap gate; the precision
    /// modes' toggle value is decided by the engine-replay
    /// `toggle_replay_bridge` behind [`TOGGLE_ROLLOUT`] (flag off =>
    /// they too fall back to pi-zero, byte-identically).
    pub fn default_toggle_policy(self) -> Box<dyn TogglePolicy> {
        Box::new(AlwaysOnIfEligibleTogglePolicy::new(self.toggle_policy_id()))
    }
...
/// timed/toggle decisions follow - see
/// `docs/policy_engine_pillars.md` for the design rationale). The policy itself is dumb on
...
mod tests {
    ...
    fn extreme_candidates_have_dense_low_band_and_sparse_long_tail() {
        // [REF:policy_semi_ideal_ideal_and_extreme] "Extreme checks 0 to 12
        // seconds every 0.1 seconds, 12.5 to 30 every half second, and 32 to
        // 120 every 2 seconds."
    ...
    #[test]
    fn semi_ideal_and_ideal_propose_the_ladders_the_reference_lists() {
        // [REF:policy_semi_ideal_ideal_and_extreme] "Semi-ideal checks six
        // delays - 0, 0.5, 1, 2, 4, and 8 seconds." / "Ideal checks eleven
        // delays - 0, 0.25, 0.5, 1, 1.5, 2, 3, 4, 6, 8, and 12 seconds."
        assert_eq!(semi_ideal_candidates(), vec![0.0, 0.5, 1.0, 2.0, 4.0, 8.0]);
        assert_eq!(
            ideal_candidates(),
            vec![0.0, 0.25, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0, 12.0]
        );
    ...
    #[test]
    fn fast_proposes_three_delays_and_none_of_them_is_deep() {
        // [REF:policy_fast] "over three candidate delays: now, one second, and
        // three seconds" and "because the candidates stop at three seconds,
        // fast cannot choose a delay any deeper mode would reach for."
        let fast = fast_candidates();
        assert_eq!(fast, vec![0.0, 1.0, 3.0]);

        let deepest = |v: Vec<f64>| v.into_iter().fold(f64::NEG_INFINITY, f64::max);
        let fast_max = deepest(fast);
        for deeper in [semi_ideal_candidates(), ideal_candidates(), extreme_candidates()] {
            assert!(
                deepest(deeper) > fast_max,
                "every deeper mode must reach past fast's {fast_max}"
            );
        }
    }

    #[test]
    fn fast_runs_the_search_and_really_fast_does_not() {
        // [REF:policy_fast] "It runs the same search the deeper modes run" /
        ...
        for mode in [
            ...
            let id = mode.default_policy().id().to_string();
            assert!(
                id.starts_with("builtin."),
                "{mode:?} must resolve to a built-in policy, got {id}"
            );
        }
        assert_ne!(
            TimingMode::Fast.default_policy().id(),
            TimingMode::ReallyFast.default_policy().id(),
            "fast must not resolve to the really fast gate"
        );
    }
}
