...
//
// Custom Abilities (2026-05-09) added the `register_user_*_js`
...
use crate::composable::{
    capture_defensive_pin_schedule, simulate_composable_matchup_pinned,
    simulate_composable_matchup_with_posture_script, simulate_composable_matchup_with_trace,
    ComposableAbilityConfig, DefensivePinControl, DefensivePinControlDto,
};
use crate::contracts::{
...
/// Deduplicated inputs for [`simulate_composable_matchup_batch_js`]. Every
/// distinct attacker / defender / breath profile / ability config appears once;
/// `fights` picks them per fight by index.
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct ComposableMatchupBatch {
    attackers: Vec<SimpleCombatantStats>,
    defenders: Vec<SimpleCombatantStats>,
    #[serde(default)]
    attacker_breaths: Vec<SimpleBreathProfile>,
    #[serde(default)]
    defender_breaths: Vec<SimpleBreathProfile>,
    configs: Vec<ComposableAbilityConfig>,
    ability_policy: SimpleAbilityTimingMode,
    max_time_sec: f64,
    fights: Vec<i32>,
}

/// Index entries per fight in `ComposableMatchupBatch::fights`:
/// attacker, defender, attacker breath, defender breath, config.
const BATCH_FIGHT_STRIDE: usize = 5;

fn batch_pick<T>(pool: &[T], index: i32, what: &str) -> Result<usize, JsValue> {
    usize::try_from(index)
        .ok()
        .filter(|resolved| *resolved < pool.len())
        .ok_or_else(|| JsValue::from_str(&format!("batch fight references missing {what} #{index}")))
}

fn batch_pick_breath<'a>(
    pool: &'a [SimpleBreathProfile],
    index: i32,
    what: &str,
) -> Result<Option<&'a SimpleBreathProfile>, JsValue> {
    if index < 0 {
        return Ok(None);
    }
    Ok(Some(&pool[batch_pick(pool, index, what)?]))
}

/// Run many matchups in one JS<->WASM crossing.
///
/// The Best Builds funnel screens a rectangle of builds against a rectangle of
/// opponents, so the same attacker stats, defender stats and ability config
/// recur across hundreds of fights while only the pairing changes. Marshalling
/// them per fight costs a sizeable fraction of a cheap fight, so this export
/// takes deduplicated pools plus a flat index stream and each distinct input
/// deserialises exactly once.
///
/// `fights` holds five indices per fight - attacker, defender, attacker breath,
/// defender breath, config - with a negative breath index meaning "no breath
/// profile on that side". Summaries come back in fight order.
#[wasm_bindgen]
pub fn simulate_composable_matchup_batch_js(batch: JsValue) -> Result<JsValue, JsValue> {
    let batch: ComposableMatchupBatch =
        serde_wasm_bindgen::from_value(batch).map_err(|err| JsValue::from_str(&err.to_string()))?;
    if !batch.fights.len().is_multiple_of(BATCH_FIGHT_STRIDE) {
        return Err(JsValue::from_str(
            "batch fight stream length must be a multiple of 5",
        ));
    }

    let mut summaries = Vec::with_capacity(batch.fights.len() / BATCH_FIGHT_STRIDE);
    for fight in batch.fights.chunks_exact(BATCH_FIGHT_STRIDE) {
        let attacker = &batch.attackers[batch_pick(&batch.attackers, fight[0], "attacker")?];
        let defender = &batch.defenders[batch_pick(&batch.defenders, fight[1], "defender")?];
        let attacker_breath = batch_pick_breath(&batch.attacker_breaths, fight[2], "attacker breath")?;
        let defender_breath = batch_pick_breath(&batch.defender_breaths, fight[3], "defender breath")?;
        let config = &batch.configs[batch_pick(&batch.configs, fight[4], "config")?];
        summaries.push(simulate_composable_matchup_with_trace(
            attacker,
            defender,
            attacker_breath,
            defender_breath,
            batch.ability_policy,
            config,
            batch.max_time_sec,
            false,
        ));
...
/// Capture both sides' committed defensive schedule (Fortify fire timelines +
/// each toggle's held-state timeline) from a full `Ideal` run, serialized for
/// the JS funnel. Feed the returned value back into
/// [`simulate_composable_matchup_pinned_js`] to reproduce the fight under
/// `GateOnly` without the per-decision rollout search.
#[wasm_bindgen]
pub fn capture_defensive_pin_schedule_js(
    attacker: JsValue,
    defender: JsValue,
    attacker_breath: JsValue,
    defender_breath: JsValue,
    ...
    let ability_config: ComposableAbilityConfig = serde_wasm_bindgen::from_value(ability_config)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    let control = capture_defensive_pin_schedule(
        &attacker,
        &defender,
        attacker_breath.as_ref(),
        defender_breath.as_ref(),
    ...
    serde_wasm_bindgen::to_value(&control.to_dto())
        .map_err(|err| JsValue::from_str(&err.to_string()))
}

/// Replay a matchup under `GateOnly` with a captured schedule (from
/// [`capture_defensive_pin_schedule_js`]) pinned on both sides. Returns the same
/// summary shape as [`simulate_composable_matchup_js`].
#[wasm_bindgen]
#[allow(clippy::too_many_arguments)]
pub fn simulate_composable_matchup_pinned_js(
    attacker: JsValue,
    defender: JsValue,
    attacker_breath: JsValue,
    defender_breath: JsValue,
    ability_config: JsValue,
    pinned_schedule: JsValue,
    max_time_sec: f64,
    record_trace: Option<bool>,
) -> Result<JsValue, JsValue> {
    let attacker: SimpleCombatantStats = serde_wasm_bindgen::from_value(attacker)
    ...
    let ability_config: ComposableAbilityConfig = serde_wasm_bindgen::from_value(ability_config)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    let pins: DefensivePinControlDto = serde_wasm_bindgen::from_value(pinned_schedule)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    let summary = simulate_composable_matchup_pinned(
        &attacker,
        &defender,
        attacker_breath.as_ref(),
        defender_breath.as_ref(),
        ...
        max_time_sec,
        record_trace.unwrap_or(false),
        DefensivePinControl::from_dto(pins),
    );
    serde_wasm_bindgen::to_value(&summary).map_err(|err| JsValue::from_str(&err.to_string()))
}
...
pub fn rust_matchup_contract_version() -> String {
    crate::contract_manifest::schema_hash()
}
...
// The engine wires `composable::simulate_composable_matchup` to consult these
...
/// Read-only snapshot of a single registered user-status spec.
/// Returns `None` for unknown ids - the status-runtime seams in
...
//   `sandbox_force_bite_js(id, side, variant)`, `sandbox_force_breath_js(id, side)`,
//   `sandbox_force_ability_js(id, side, name)`, `sandbox_force_posture_js(id, side, target)`,
...
#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct SandboxReconfigurePayload {
    attacker: SimpleCombatantStats,
    defender: SimpleCombatantStats,
    #[serde(default)]
    attacker_breath: Option<SimpleBreathProfile>,
...
#[wasm_bindgen]
pub fn sandbox_reconfigure_js(id: u64, payload: JsValue) -> Result<JsValue, JsValue> {
    let req: SandboxReconfigurePayload = serde_wasm_bindgen::from_value(payload)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    with_sandbox(id, |rt| {
        rt.reconfigure(
            req.attacker,
...
#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct SandboxApplyMeterPayload {
    side: SandboxSide,
    meter: crate::composable::sandbox::SandboxMeter,
    value: f64,
}

#[wasm_bindgen]
pub fn sandbox_apply_meter_js(id: u64, payload: JsValue) -> Result<JsValue, JsValue> {
    let req: SandboxApplyMeterPayload = serde_wasm_bindgen::from_value(payload)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    with_sandbox(id, |rt| rt.apply_meter(req.side, req.meter, req.value))?;
    sandbox_view_result(id)
}
...
#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct SandboxForceBitePayload {
    side: SandboxSide,
    /// Omitted by older callers -> defaults to `Primary`.
    #[serde(default)]
    variant: SandboxBiteVariant,
}

#[wasm_bindgen]
pub fn sandbox_force_bite_js(id: u64, payload: JsValue) -> Result<JsValue, JsValue> {
    let req: SandboxForceBitePayload = serde_wasm_bindgen::from_value(payload)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    with_sandbox(id, |rt| rt.force_bite(req.side, req.variant))?;
    sandbox_view_result(id)
}
...
#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct SandboxAutomationModePayload {
    mode: SandboxAutomationMode,
}

#[wasm_bindgen]
pub fn sandbox_set_automation_mode_js(id: u64, payload: JsValue) -> Result<JsValue, JsValue> {
    let req: SandboxAutomationModePayload = serde_wasm_bindgen::from_value(payload)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    with_sandbox(id, |rt| rt.set_automation_mode(req.mode))?;
    sandbox_view_result(id)
}

#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct SandboxForcePosturePayload {
    side: SandboxSide,
    target: Posture,
}

#[wasm_bindgen]
pub fn sandbox_force_posture_js(id: u64, payload: JsValue) -> Result<JsValue, JsValue> {
    let req: SandboxForcePosturePayload = serde_wasm_bindgen::from_value(payload)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    with_sandbox(id, |rt| rt.force_posture(req.side, req.target))?;
    sandbox_view_result(id)
}
...
#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct SandboxSetBadOmenOutcomesPayload {
    outcomes: Vec<crate::contracts::SimpleBadOmenOutcome>,
}

#[wasm_bindgen]
pub fn sandbox_set_bad_omen_outcomes_js(id: u64, payload: JsValue) -> Result<JsValue, JsValue> {
    let req: SandboxSetBadOmenOutcomesPayload = serde_wasm_bindgen::from_value(payload)
        .map_err(|err| JsValue::from_str(&err.to_string()))?;
    with_sandbox(id, |rt| rt.set_bad_omen_outcomes(req.outcomes.clone()))?;
    sandbox_view_result(id)
}
...
mod tests {
    ...
    //! mirrors the wasm_bindgen entry. The browser-driven
    //! Playwright flow exercises the surface end-to-end.
    use super::*;
    ...
    fn user_status_drives_runtime_seams_and_leaves_builtins_intact() {
        ...
        assert!(crate::statuses::is_fortify_removable_status(id));
        assert!((crate::statuses::compute_simple_dot_damage(10_000.0, id, 5.0) - 45.0).abs() < 1e-9);
        ...
        let poison = crate::statuses::compute_simple_dot_damage(10_000.0, "Poison_Status", 5.0);
        ...
        assert_eq!(crate::statuses::compute_simple_dot_damage(10_000.0, id, 5.0), 0.0);
    }

    #[test]
    fn user_status_modifier_seams_compose_in_combat() {
        // The spec's stat-modifier knobs reach the
        ...
        assert!((crate::combat::outgoing_damage_multiplier_from_statuses(&map) - 0.5).abs() < 1e-9);
