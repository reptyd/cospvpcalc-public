...
//!   and registers in `DecisionRegistry`.
...
//!             status: SimpleAppliedStatus { status_id: "Burn_Status".into(), stacks: 2.0, ..Default::default() },
//!         },
//!     ],
//!     ..Default::default()
//! };
...
/// Hard cap on numbered-key extras arrays. Pushes past
...
/// One queued scheduled-effect entry. Used to live as a
...
/// deserializes identically to the earlier tuple shape).
///
...
pub enum EffectKind {
    ...
    /// Apply N statuses in one effect. Same canonical
    ...
    ///
    /// Engine read-path (`composable/loop_iter.rs`):
    ...
    SwapHpRatio,
    /// FormSwap: glass-cannon <-> tank form swap. Writes each `stat_changes`
    ...
    /// Remove N named statuses in one effect. Each id
    ...
    /// Append a value to a numbered-key extras array.
    ...
    /// Clear a numbered-key extras array - removes
    ...
    /// Set the absolute timestamp at which the named
    ...
    /// Find the FIRST queued entry on the caster's
    ...
    /// Weighted one-of-N exclusive branch picker.
...
/// engine adapter increments `EffectContext::chain_depth`
...
/// even though the read-path implementation lives on the engine read-path -
...
/// Whether `extras` carries any `modifier.*` key at all. Ordered map, so the
/// answer is one range probe rather than a scan; every effective-stat read
/// takes it first, because with no modifier present the read is an identity
/// and the general path would allocate a format key per field.
pub fn has_modifier_keys(
    extras: &BTreeMap<String, crate::policy::state::PolicyValue>,
) -> bool {
    extras
        .range::<str, _>((
            std::ops::Bound::Included(MODIFIER_KEY_PREFIX),
            std::ops::Bound::Unbounded,
        ))
        .next()
        .is_some_and(|(key, _)| key.starts_with(MODIFIER_KEY_PREFIX))
}
...
/// are skipped (expired).
///
/// Sourced modifiers. Two abilities both writing
...
pub fn effective_stat_value(
    ...
    for (key, value) in extras.iter() {
        ...
        // `rest` is `<mode>.<source>` (current format) or just `<mode>`
        // (legacy format - defensive: shouldn't appear at runtime but kept
        // robust against the unlikely case of legacy keys lingering
...
pub struct EffectBatch {
    ...
    pub effects: Vec<EffectKind>,
    /// Optional gate expression. When present and the
...
pub struct EffectContext<'a> {
    ...
    /// Per-side modifier scratch space. `ModifyStat`
    ...
    /// Optional accumulator for `iter_healing_taken` on
    ...
    /// adapter bumps this on each nested expansion and
    ...
    pub chain_depth: u32,
    /// Id of the ability currently firing. `ModifyStat`
    ...
    /// to the literal `"default"` for backward compat with legacy keys.
...
impl EffectContext<'_> {
    ...
    /// Best-effort write to the target side's healing
...
pub fn apply_effect_batch(batch: &EffectBatch, ctx: &mut EffectContext<'_>) -> u32 {
    // Batch-level gate. When `when` is set and
...
fn apply_effect_count(effect: &EffectKind, ctx: &mut EffectContext<'_>) -> u32 {
    match effect {
        ...
        EffectKind::CancelSchedule { name } => {
            // Remove every queued entry on the
        ...
        EffectKind::Reschedule { name, delay_sec } => {
            // Find the FIRST entry on the caster's
        ...
        EffectKind::Choose { branches } => {
            // Pick exactly one branch by weighted random.
        ...
        EffectKind::TriggerAbility { ability_id } => {
            // Look up the target ability and inline its
    ...
    match effect {
        ...
        EffectKind::HealHp { target, amount } => {
            ...
            *hp = (*hp + amount).min(max_hp);
            ...
            // Feed OnHeal trigger accumulator.
        ...
        EffectKind::ApplyStatusesToTarget { target, statuses: applied } => {
            // Array variant. Same resist/plushie path
        ...
        EffectKind::ModifyStat {
            ...
            // Write modifier to target's per-side
            ...
            // Stamp source into the key so multiple
        ...
        EffectKind::TransferHp { from, to, amount } => {
            ...
            *ctx.target_hp(to) = dest_hp + receivable;
            // Recipient side gained HP -> fire OnHeal
        ...
        EffectKind::ClearStatuses { target, status_ids } => {
            // Array variant. Each id processed
        ...
        EffectKind::ModifyStatusStacks {
            ...
            let new_stacks = match mode {
                ...
                // Mul on stacks is a footgun (0 x anything = 0
        ...
        EffectKind::SetStatusNextTick {
            ...
            // Twin of SetStatusNextDecay but writes to
        ...
        EffectKind::ModifyStatExpr {
            ...
            // Same source-stamp scheme as ModifyStat.
            let source = ctx.firing_ability_id.unwrap_or("default");
            let value_key = format!("{MODIFIER_KEY_PREFIX}{field}.{mode_str}.{source}.value");
            let until_key = format!("{MODIFIER_KEY_PREFIX}{field}.{mode_str}.{source}.until");
            let extras = ctx.target_extras(target);
...
mod tests {
    ...
    fn schedule_effect_named_then_cancel_clears_entry() {
        // Schedule with a name, then cancel by name,
    ...
    fn reschedule_named_entry_moves_due_at() {
        // Schedule + reschedule moves the due time.
        let stats = fresh_stats(10_000.0);
        let mut caster_hp = 10_000.0;
        let mut opponent_hp = 10_000.0;
        let mut cs = BTreeMap::new();
    ...
    fn apply_statuses_to_target_lands_every_entry() {
        // Each entry in the array goes through the
    ...
    fn choose_picks_the_only_positive_weight_branch() {
        // When only one branch has positive weight, the
    ...
    fn set_status_next_tick_writes_into_status_instance() {
        // Writes `next_tick_at` on the named status,
    ...
    fn modify_stat_apply_writes_modifier_keys_into_extras() {
        // Apply path writes to target's user_extras
        ...
        // Source segment is "default" when
    ...
    fn modifier_set_takes_precedence_over_mul_add_round39_a8() {
        ...
        // Even though mul says x10, set wins -> 42.
