...
use super::setup::{derive_loop_flags, populate_combat_sides_and_flags, ComposableLoopFlags};
...
pub struct SandboxView {
    pub time: f64,
    pub halted: bool,
    /// The mode the run is in right now. The UI control follows this rather
    /// than the other way round: seeking backwards past a mode switch puts
    /// the run back in the mode it was in at that time.
    pub automation_mode: SandboxAutomationMode,
...
enum SandboxAction {
    ...
    SetAutomationMode { time: f64, mode: SandboxAutomationMode },
...
impl SandboxAction {
    /// Whether the action edits the setup (and so has to be re-applied when
    /// the setup underneath it is replaced) rather than acting in the fight.
    fn is_override(&self) -> bool {
        matches!(
            self,
            SandboxAction::OverrideStat { .. }
                | SandboxAction::OverrideAbility { .. }
                | SandboxAction::OverrideAbilityNumber { .. }
                | SandboxAction::OverrideAbilityString { .. }
                | SandboxAction::OverridePassiveBool { .. }
                | SandboxAction::OverridePassiveNumber { .. }
                | SandboxAction::OverrideBreath { .. }
                | SandboxAction::OverrideResist { .. }
                | SandboxAction::OverrideOffensiveStatus { .. }
                | SandboxAction::OverrideDefensiveStatus { .. }
        )
    }
    ...
    fn time(&self) -> f64 {
        match self {
            ...
            | SandboxAction::SetAutomationMode { time, .. }
...
pub struct SandboxRuntime {
    ...
    /// The mode the run started in. A rewind restores it before replaying,
    /// so the mode switches in the action log land at their own times
    /// instead of the whole replay running under the latest mode.
    initial_automation_mode: SandboxAutomationMode,
...
impl SandboxRuntime {
    ...
    pub fn new(
        ...
        let mut loop_state = super::loop_iter::LoopState {
            ...
            bite_count_a: 0,
            bite_count_b: 0,
            breath_scheduler_ticks_a: 0,
            breath_scheduler_ticks_b: 0,
            regen_ticks_a: 0,
            regen_ticks_b: 0,
            regen_healed_a: 0.0,
            regen_healed_b: 0.0,
        ...
        Box::new(Self {
            ...
            ability_policy,
            automation_mode,
            initial_automation_mode: automation_mode,
    ...
    fn rewind_to(&mut self, target_time: f64) {
        ...
        self.automation_mode = self.initial_automation_mode;
        ...
        self.loop_state.breath_scheduler_ticks_a = 0;
        self.loop_state.breath_scheduler_ticks_b = 0;
    ...
    fn apply_action(&mut self, action: &SandboxAction) {
        match action {
            ...
            SandboxAction::SetAutomationMode { mode, .. } => self.set_automation_mode(*mode),
    ...
    /// Replace the setup the fight is running on - stats, breath profiles,
    /// ability policy and config - while the fight itself carries on. Every
    /// setting outside the two creatures is a description of the fight rather
    /// than an event in it, so the new setup becomes the baseline a Reset and
    /// a rewind both start from.
    ///
    /// What survives untouched: the clock, both sides' HP, statuses, meters,
    /// cooldowns and timers, and the whole log. HP is only clamped, never
    /// restored, when a smaller maximum arrives. Overrides the user applied by
    /// hand are re-applied on top of the new baseline, because they live in
    /// the stats being replaced.
    ///
    /// Starting statuses are the one thing that does not land: they belong to
    /// the opening of a fight, and this one has already started. Seeking back
    /// to zero picks them up.
    pub fn reconfigure(
        &mut self,
        attacker: SimpleCombatantStats,
        defender: SimpleCombatantStats,
        ...
        self.attacker_snapshot = attacker.clone();
        self.defender_snapshot = defender.clone();
        self.config_snapshot = config.clone();
        self.attacker_stats = attacker;
        self.defender_stats = defender;
        self.config = config;
        self.ability_policy = ability_policy;
        self.event_phase_order = normalize_ordered_event_phases(&self.config.combat_event_order);

        for (slot, profile) in [
            (SandboxSide::A, attacker_breath),
            (SandboxSide::B, defender_breath),
        ] {
            let current = match slot {
                SandboxSide::A => &self.attacker_breath,
                SandboxSide::B => &self.defender_breath,
            };
            if current.as_ref() == profile.as_ref() {
                continue;
            }
            // A breath the fight did not have until now, or one that just
            // changed shape, starts from a full bar and is ready at once -
            // the same re-seed the Breath override does.
            let (stored, side_state) = match slot {
                SandboxSide::A => (&mut self.attacker_breath, &mut self.loop_state.a),
                SandboxSide::B => (&mut self.defender_breath, &mut self.loop_state.b),
            };
            side_state.breath_capacity = profile.as_ref().map(|p| p.capacity).unwrap_or(0.0);
            side_state.next_breath = if profile.is_some() {
                self.loop_state.time.max(0.0)
            } else {
                f64::INFINITY
            };
            *stored = profile;
        ...
        for side in [SandboxSide::A, SandboxSide::B] {
        ...
        // The overrides were written into the stats that were just replaced.
        // Re-applying them from the log is what the rewind does, and it keeps
        // a hand-set value from disappearing because an unrelated toggle moved.
        let log = std::mem::take(&mut self.action_log);
        self.replaying = true;
        for action in log.iter() {
    ...
    /// Switch between Manual and SemiAuto without disturbing the fight. The
    /// clock, both sides and the log are left exactly as they are, so a run
    /// can be set up by hand and then handed to the engine.
    ///
    /// Manual holds bites and breath out of the scheduler, so their timers
    /// sit in the past by the time the user switches away. Pulling them up
    /// to the current time makes the first automatic bite land here rather
    /// than at a timestamp the fight has already passed.
    pub fn set_automation_mode(&mut self, mode: SandboxAutomationMode) {
        if self.automation_mode == mode {
            return;
        }
        self.automation_mode = mode;
        if matches!(mode, SandboxAutomationMode::SemiAuto) {
            let now = self.loop_state.time.max(0.0);
            for side in [&mut self.loop_state.a, &mut self.loop_state.b] {
                if side.next_hit < now {
                    side.next_hit = now;
                }
                if side.next_breath < now {
                    side.next_breath = now;
                }
            }
        }
        let time = self.loop_state.time;
        self.record_action(SandboxAction::SetAutomationMode { time, mode });
    }
    ...
    pub fn force_breath(&mut self, side: SandboxSide) {
        ...
        let pre_breath_count = match side {
            SandboxSide::A => self.loop_state.breath_scheduler_ticks_a,
            SandboxSide::B => self.loop_state.breath_scheduler_ticks_b,
        };
        for _ in 0..50 {
...
mod tests {
    ...
    fn mode_switch_sandbox() -> Box<SandboxRuntime> {
        SandboxRuntime::new(
            basic_stats(500.0, 50.0),
            basic_stats(500.0, 50.0),
            None,
            None,
            ComposableAbilityConfig::default(),
    ...
    #[test]
    fn switching_to_semi_auto_leaves_the_clock_and_both_sides_untouched() {
        let mut rt = mode_switch_sandbox();
        rt.step_to_time(10.0);
        rt.force_bite(SandboxSide::A, SandboxBiteVariant::Primary);
        let before = rt.snapshot_view();

        rt.set_automation_mode(SandboxAutomationMode::SemiAuto);

        let after = rt.snapshot_view();
        assert_eq!(after.automation_mode, SandboxAutomationMode::SemiAuto);
        assert!((after.time - before.time).abs() < 1e-9, "the clock must not move");
        assert!((after.side_a.hp - before.side_a.hp).abs() < 1e-9);
        assert!((after.side_b.hp - before.side_b.hp).abs() < 1e-9);
        assert_eq!(after.log.len(), before.log.len(), "no event may be added or lost");
    }

    #[test]
    fn a_run_set_up_by_hand_carries_on_under_the_engine() {
        let mut rt = mode_switch_sandbox();
        rt.step_to_time(10.0);
        assert_eq!(
            bite_times(&rt, "A"),
            Vec::<f64>::new(),
            "Manual holds bites out of the scheduler"
        );

        rt.set_automation_mode(SandboxAutomationMode::SemiAuto);
        rt.step_to_time(14.0);

        let bites = bite_times(&rt, "A");
        assert!(!bites.is_empty(), "the engine bites once it is in charge");
        assert!(
            bites.iter().all(|t| *t >= 10.0 - 1e-9),
            "the held bite fires at the switch, not at a time already passed: {bites:?}"
        );
    }

    #[test]
    fn seeking_back_past_a_mode_switch_returns_the_run_to_the_earlier_mode() {
        let mut rt = mode_switch_sandbox();
        rt.step_to_time(10.0);
        rt.set_automation_mode(SandboxAutomationMode::SemiAuto);
        rt.step_to_time(20.0);
        assert!(!bite_times(&rt, "A").is_empty());

        rt.step_to_time(5.0);

        let view = rt.snapshot_view();
        assert_eq!(view.automation_mode, SandboxAutomationMode::Manual);
        assert_eq!(
            bite_times(&rt, "A"),
            Vec::<f64>::new(),
            "the replayed stretch was Manual, so nothing bit in it"
        );
    }

    fn damage_breath(dps_pct: f64) -> SimpleBreathProfile {
        SimpleBreathProfile {
            dps_pct,
            capacity: 10.0,
            regen_rate: 1.0,
    ...
    fn reconfigure_sandbox() -> Box<SandboxRuntime> {
        SandboxRuntime::new(
            basic_stats(500.0, 50.0),
            basic_stats(500.0, 50.0),
            Some(damage_breath(1.0)),
            None,
            ComposableAbilityConfig::default(),
            SimpleAbilityTimingMode::Ideal,
            SandboxAutomationMode::SemiAuto,
    ...
    /// Reconfigure with the same setup the sandbox already runs on, save for
    /// the one field each test is about.
    fn same_setup(
        rt: &mut SandboxRuntime,
        edit: impl FnOnce(&mut ComposableAbilityConfig, &mut Option<SimpleBreathProfile>),
    ) {
        let mut config = ComposableAbilityConfig::default();
        let mut attacker_breath = Some(damage_breath(1.0));
        edit(&mut config, &mut attacker_breath);
        rt.reconfigure(
            basic_stats(500.0, 50.0),
            basic_stats(500.0, 50.0),
            attacker_breath,
    ...
    #[test]
    fn changing_a_setting_leaves_the_fight_where_it_is() {
        let mut rt = reconfigure_sandbox();
        rt.step_to_time(10.0);
        rt.apply_status(SandboxSide::B, "Poison_Status", 4.0);
        let before = rt.snapshot_view();
        assert!(before.time > 0.0 && before.side_b.hp < 500.0, "the fight has to be under way");

        same_setup(&mut rt, |config, _| config.attacker_thorn_trap = true);
        ...
        assert!((after.side_b.hp - before.side_b.hp).abs() < 1e-9);
        assert_eq!(
            after.side_b.statuses.iter().map(|s| (s.id.clone(), s.stacks)).collect::<Vec<_>>(),
            before.side_b.statuses.iter().map(|s| (s.id.clone(), s.stacks)).collect::<Vec<_>>(),
        );
        assert_eq!(after.log.len(), before.log.len(), "no event may be added or lost");
        assert!(rt.flags.has_any_thorn_trap, "the new setting is the one in force");
    }

    #[test]
    fn a_hand_set_override_survives_a_setting_change() {
        let mut rt = reconfigure_sandbox();
        rt.step_to_time(4.0);
        rt.override_stat(SandboxSide::A, "damage", 999.0);

        same_setup(&mut rt, |config, _| config.defender_hunters_curse = true);

        assert!(
            (rt.attacker_stats.damage - 999.0).abs() < 1e-9,
            "the override was written into the stats the reconfigure replaced"
        );
    }

    #[test]
    fn turning_breath_off_mid_fight_stops_it_and_keeps_the_clock() {
        let mut rt = reconfigure_sandbox();
        rt.step_to_time(6.0);
        let before = rt.snapshot_view();
        assert!(!rt.loop_state.a.breath_fire_times.is_empty(), "A was breathing");

        same_setup(&mut rt, |_, breath| *breath = None);

        let fired_by_now = rt.loop_state.a.breath_fire_times.len();
        rt.step_to_time(12.0);
        let after = rt.snapshot_view();
        assert!(after.time > before.time);
        assert_eq!(
            rt.loop_state.a.breath_fire_times.len(),
            fired_by_now,
            "no tick may fire after the breath is taken away"
        );
        assert!(
            !rt.loop_state.a.next_breath.is_finite(),
            "the breath must leave the scheduler, not sit due forever"
        );

        // And back on: a full bar, ready at the current time rather than at
        // one the fight has already passed.
        let resumed_at = rt.snapshot_view().time;
        same_setup(&mut rt, |_, _| {});
        assert!((rt.loop_state.a.breath_capacity - 10.0).abs() < 1e-9);
        assert!((rt.loop_state.a.next_breath - resumed_at).abs() < 1e-9);
        rt.step_to_time(14.0);
        assert!(rt.loop_state.a.breath_fire_times.len() > fired_by_now);
    }

    fn bite_times(rt: &SandboxRuntime, label: &str) -> Vec<f64> {
        rt.loop_state
            .combat_log
            .iter()
            .filter(|e| e.entry_type == "bite" && e.attacker == label)
            .map(|e| e.time)
            .collect()
    }
