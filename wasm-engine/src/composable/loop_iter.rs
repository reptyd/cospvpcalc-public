...
use super::phases::{
    ...
    process_phase_4_misc_and_cocoon_cluster, process_phase_4_oxygen_moisture_tick,
    process_phase_4_status_applies_cluster,
    ...
    process_phase_9_lance_aura, process_phase_status_decay_gate, process_regen_pending_release,
    SchedulerPassiveFlags, SchedulerStep,
};
...
use super::{
    ...
    user_dispatch, user_status_dispatch, DamageCounters, DefensivePinControl,
...
/// Whether ability-activation policy runs its full decision or collapses to
/// the ReallyFast gate. The live driver uses `Normal`. Every inner engine
/// replay (posture / bite-variant / Fortify rollout) uses `GateOnly` when the
/// `FORTIFY_ROLLOUT` flag is on, so a rollout-driven ability (Fortify) resolves
/// on its cheap ReallyFast gate inside the projection instead of re-entering
/// its own rollout. Flag off => `Normal` everywhere => byte-identical.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(super) enum AbilityPolicyMode {
    Normal,
    GateOnly,
}
...
/// Per-side posture override consulted inside the iter. Returns `Some(action)`
/// to drive that side's posture directly (scripted / benchmark / engine-replay
/// opponent), or `None` to defer to the live policy (`decide_stance_now`) for
/// that side - so one closure can script one side while the other runs live.
pub(super) type DecideOverrideFn<'a> = dyn Fn(&CombatSide, &CombatSide, f64, bool) -> Option<super::posture_policy::PostureAction> + 'a;
...
/// Pluggable toggle resolver installed by a toggle engine-replay
/// (`toggle_replay_bridge`). When `LoopParams::decide_toggle_override`
/// is `Some(fn)`, the pre-resolve in this iter consults it for each
/// toggle instead of running a (nested) toggle replay.
///
/// Signature: `fn(toggle_id, actor_side, time, is_attacker) ->
/// Option<bool>`. `Some(v)` forces the evaluated toggle held-state `v`;
/// `None` defers that toggle to its pi-zero gate. Inside a toggle inner
/// replay exactly one `(toggle_id, side)` answers `Some`; every other
/// runs pi-zero.
pub(super) type DecideToggleOverrideFn<'a> =
    dyn Fn(&'static str, &CombatSide, f64, bool) -> Option<bool> + 'a;
...
pub(super) struct LoopState {
    ...
    /// How many times the breath phase came due for each side, counted
    /// before the posture / necropoison / capacity gates decide whether the
    /// tick fires. Sandbox's `force_breath` steps the loop until this moves.
    /// Firing ticks are `CombatSide::breath_fire_times`, which is a different
    /// and usually smaller number.
    pub breath_scheduler_ticks_a: u32,
    pub breath_scheduler_ticks_b: u32,
    ...
    pub fortify_control: DefensivePinControl,
}
...
pub(super) struct LoopParams<'a> {
    ...
    /// Whether ability activation runs its full policy (`Normal`) or the
    /// ReallyFast gate only (`GateOnly`, inside inner replays). See
    /// [`AbilityPolicyMode`]. Default `Normal` in the live driver.
    pub ability_policy_mode: AbilityPolicyMode,
    ...
    /// Optional toggle resolver installed by a toggle engine-replay.
    /// When `Some(fn)`, the pre-resolve consults it for the evaluated
    /// toggle instead of running a nested replay (recursion guard).
    /// Live driver / sandbox leave this `None`.
    pub decide_toggle_override: Option<&'a DecideToggleOverrideFn<'a>>,
}
...
fn apply_stat_modifiers(
    ...
    if !crate::effects::has_modifier_keys(extras) {
        return;
    }
    ...
    // The secondary bite is the primary bite with a different base value; the
    // only thing that differs between them is that it applies no on-hit
    // ailments. So every modifier that reaches `damage` reaches it too, and a
    // new damage effect never has to be wired to the variant twice. Its own
    // `damage2` key stays available for a secondary-only modifier.
    eff.damage2 = esv(esv(eff.damage2, "damage2", extras, time), "damage", extras, time).max(0.0);
    ...
    eff.damage_taken_multiplier_on_breath = esv(
        eff.damage_taken_multiplier_on_breath,
...
/// Whether the post-4p effective-stats rebuild would change anything for one
/// side. Mirrors [`with_active_weight_bonuses_and_static_factor`] plus the
/// reflect overwrite and the modifier layer: when every arm is inert the
/// rebuild is a field-for-field copy of `stats`, so the caller keeps the
/// borrow instead of paying the clone.
fn side_needs_stage2_eff(
    ...
    has_modifiers
        || reflect != stats.has_reflect
        || (fortify_weight_bonus_until > 0.0 && fortify_weight_bonus_until > time)
        || (harden_active_until > 0.0 && harden_active_until > time)
        || (static_weight_factor > 0.0 && (static_weight_factor - 1.0).abs() > 1e-9)
}
...
pub(super) fn run_one_event_loop_iter(
    ...
    // Work meter: counts EVERY engine iteration, including the ones run inside
    // posture / bite-variant / rollout inner replays (which pass
    // bench_count=false). The bench counter above deliberately excludes those;
    // this one is the deterministic cost the summary reports.
    super::work_meter::note_engine_iter();
    // Reset per-iteration damage-kind bitmasks so
    ...
    // Reset healing accumulator + snapshot the
    ...
    // Flush last iter's effective healing into the whole-fight accumulator
    // before clearing it (drives the Effective-HP metric). The final iter's
    // value stays in iter_healing_taken and is added on top at summary time.
    state.a.total_healing_taken += state.a.iter_healing_taken;
    state.b.total_healing_taken += state.b.iter_healing_taken;
    state.a.iter_healing_taken = 0.0;
    state.b.iter_healing_taken = 0.0;
    // Reset raw-damage accumulators so
    ...
    // Mirror the user-window snapshot for BUILT-IN active
    ...
    // Drain any pending requests user effects wrote
    ...
    // FormSwap: reverse HP reconciliation when a temporary
    ...
    // Snapshot HP at the start of each iteration so
    ...
    let scheduler_flags = SchedulerPassiveFlags {
        ...
        has_any_oxygen_moisture: params.flags.has_any_oxygen_moisture,
    };
    ...
    // forever at t=0.
    let posture_snap_enabled = (params.decide_override.is_none()
        || params.decide_override_respects_schedule)
        // Posture<->toggle coordination: the toggle follower's inner replay freezes
        // posture (`ForcedOff`), so `posture_next_decision_at` is never advanced
        // inside it. Left enabled, the scheduler would snap `time` to that stale
        // timer and truncate the replay at the first posture-decision moment
        // (~5 s) - before any 15 s regen cycle, hiding the Warden harvest's value
        // so the follower always commits Off. Suppress the snap for that replay
        // (identified by a frozen posture + an installed toggle override). Gated
        // on `coordination_enabled()` so it engages only with the rollout on
        // (byte-identical off) and the experiment's coordination-disabled arm
        // still reproduces the conflict. The Fortify fork (also `ForcedOff` but
        // no toggle override) is untouched.
        && !(matches!(params.posture_policy_override, PosturePolicyMode::ForcedOff)
            && params.decide_toggle_override.is_some()
            && super::stance_bridge::coordination_enabled());
    ...
    #[cfg(test)]
    crate::composable::oracle::note_phase(selected_phase);
    ...
    // ~2.4x live loop on policy-on fights; zero on policy-off fights.
    ...
    if !matches!(params.posture_policy_override, PosturePolicyMode::ForcedOff) {
        ...
        // Else: standard periodic-decision gating via posture_next_decision_at.
        ...
        let a_fire = params.config.attacker_posture_policy_enabled
            && state.a.death_time.is_none()
            && !params.config.head_start_inert_a(state.time)
        ...
        if a_fire {
            let scripted = params
                .decide_override
                .and_then(|decide_fn| decide_fn(&state.a, &state.b, state.time, /* is_attacker */ true));
            let action = match scripted {
                Some(a) => a,
                None => super::stance_bridge::decide_stance_now(
        ...
        let b_fire = params.config.defender_posture_policy_enabled
            && state.b.death_time.is_none()
            && !params.config.head_start_inert_b(state.time)
            && match (params.decide_override.is_some(), params.decide_override_respects_schedule) {
                (true, false) => true,
                _ => b_scheduled,
            };
        if b_fire {
            let scripted = params
                .decide_override
                .and_then(|decide_fn| decide_fn(&state.b, &state.a, state.time, /* is_attacker */ false));
            let action = match scripted {
                Some(a) => a,
                None => super::stance_bridge::decide_stance_now(
    ...
    // Toggle rollout (flag-gated): resolve the held ON/OFF for Hunker /
    // Warden's Rage by a bounded engine replay at the ~5 s epoch, BEFORE
    // borrowing `state` for any phase context (same pre-resolve shape as the
    // Fortify rollout - the fork needs a whole-state clone). `None` => flag
    // off / inner replay / fast mode, and the phase keeps its pi-zero gate path.
    // Resolved once here and consumed at both the Hunker phase (below) and
    // the Warden's phase (delayed-activations cluster).
    let (hunker_forced_a, hunker_forced_b, warden_forced_a, warden_forced_b) =
        if selected_phase == OrderedEventPhase::ActiveAbilities {
            use super::toggle_replay_bridge::{resolve as toggle_resolve, ToggleKind};
            (
                toggle_resolve(state, params, true, ToggleKind::Hunker),
                toggle_resolve(state, params, false, ToggleKind::Hunker),
                toggle_resolve(state, params, true, ToggleKind::WardenRage),
                toggle_resolve(state, params, false, ToggleKind::WardenRage),
            )
        } else {
            (None, None, None, None)
        };
    ...
    if selected_phase == OrderedEventPhase::ActiveAbilities {
        // Fortify rollout (flag-gated): resolve the fire/skip decision by a
        // bounded engine fork BEFORE borrowing `state` for the phase context,
        // mirroring the bite-variant pre-resolve (the fork needs a whole-state
        // clone, which conflicts with the phase's disjoint `&mut` borrows).
        // `None` => the flag is off (or this is an inner replay) and the phase
        // uses its existing gate path. Under a decision cadence the resolve also
        // writes the committed plan onto `state.fortify_control`, which is why
        // it takes the state mutably.
        let fortify_rollout_a =
            super::fortify_rollout_bridge::resolve(state, params, /* self_is_attacker */ true);
        let fortify_rollout_b =
            super::fortify_rollout_bridge::resolve(state, params, /* self_is_attacker */ false);
        ...
        process_phase_3_activations(
            &mut ctx,
            params.flags.has_any_rewind,
            params.ability_policy,
            &mut state.fortify_control,
            params.ability_policy_mode,
            fortify_rollout_a,
            fortify_rollout_b,
        );
    }
    ...
    let b_has_defensive = state.b
        ...
        .map(|i| i.stacks > 0.0 && i.remaining_sec > 0.0)
        .unwrap_or(false);
    let has_modifiers_a = crate::effects::has_modifier_keys(&state.a.user_extras);
    let has_modifiers_b = crate::effects::has_modifier_keys(&state.b.user_extras);
    let needs_eff_override = params.flags.has_any_fortify
        ...
        || a_has_defensive
        || b_has_defensive
        // Any user_extras key under modifier.*
    ...
    //
    // Modifier keys are the ONLY thing this stage layers on. Without them the
    // copy equals `params` field for field, and stage 2 rebuilds from `params`
    // anyway - so the other `needs_eff_override` arms have nothing to do here.
    // Skipping it keeps two full stat clones and ~48 key allocations out of
    // every iteration of every Fortify / Harden / Reflect fight.
    if has_modifiers_a || has_modifiers_b {
    ...
    // Phase 4b + 4b-bis: Trap clusters. See `process_phase_4_traps_cluster`.
    {
        ...
        process_phase_4_traps_cluster(&mut ctx, params.flags.has_any_thorn_trap, params.flags.has_any_toxic_trap, &mut state.counters);
    }
    ...
    if needs_eff_override {
        ...
        let a_reflect = params.config.attacker_reflect
            && state.a.reflect_active_until > 0.0
            && state.a.reflect_active_until > state.time;
        let b_reflect = params.config.defender_reflect
            && state.b.reflect_active_until > 0.0
            && state.b.reflect_active_until > state.time;
        // A side whose windows are all shut, whose static factor is 1 and whose
        // reflect flag already matches keeps reading raw params: the rebuild
        // would reproduce them field for field, and it is a full stat clone in
        // the hot path of every Fortify fight.
        if side_needs_stage2_eff(
            params.attacker,
            state.a.fortify_weight_bonus_until,
            state.a.harden_active_until,
            state.time,
            a_static,
            a_reflect,
            has_modifiers_a,
            ...
            a_eff.has_reflect = a_reflect;
            apply_stat_modifiers(&mut a_eff, &state.a.user_extras, state.time);
            eff_a_stage2_owned = a_eff;
            eff_a = &eff_a_stage2_owned;
        } else {
            eff_a = params.attacker;
        }
        if side_needs_stage2_eff(
            params.defender,
            state.b.fortify_weight_bonus_until,
            state.b.harden_active_until,
            state.time,
            b_static,
            b_reflect,
            has_modifiers_b,
            ...
            b_eff.has_reflect = b_reflect;
            apply_stat_modifiers(&mut b_eff, &state.b.user_extras, state.time);
            eff_b_stage2_owned = b_eff;
            eff_b = &eff_b_stage2_owned;
        } else {
            eff_b = params.defender;
        }
    }
    ...
    // Oxygen / Moisture per-second drain tick (Compare-only global mode).
    // Dispatched alongside the status DOT phase: a global environmental DOT,
    // so it rides the StatusTicks boundary the scheduler marks due when
    // `oxy_moist_next_tick_at` is reached. Gated by `has_any_oxygen_moisture`
    // so off / all-immune runs never build the context.
    if selected_phase == OrderedEventPhase::StatusTicks && params.flags.has_any_oxygen_moisture {
        let mut ctx = PhaseContext {
            time: state.time,
            attacker: eff_a,
            defender: eff_b,
        ...
        process_phase_4_oxygen_moisture_tick(&mut ctx);
    }

    // Buffered natural-regen release. Runs EVERY iteration regardless of
    // `selected_phase`, and is placed after the status-decay gate (Phase 3) and
    // status DOT ticks (Phase 12) so a suppressor that decays below full block
    // THIS iteration is seen as cleared in the SAME iteration - the release is
    // then anchored to the clear (WR: instant; Bleed/Burn: ~1.5 s later), not to
    // the next 15 s regen boundary. Warden's Rage turn-off (Phase 4) is likewise
    // upstream of here. Because `regen_release_at` is always either INFINITY or
    // consumed here, and the scheduler folds it, no stale finite schedule can
    // stall the loop. See `process_regen_pending_release`.
    {
        let mut ctx = PhaseContext {
            time: state.time,
            attacker: eff_a,
    ...
    if selected_phase == OrderedEventPhase::Breath {
        // Ideal breath policy (flag-gated): pick each side's next burst-open
        // time by a bounded engine replay BEFORE borrowing `state` for the
        // phase context (the fork needs a whole-state clone, which conflicts
        // with the phase's disjoint `&mut` borrows). `None` => flag off / inner
        // replay / not the Ideal policy / not at a burst boundary, and the
        // fuel stepper keeps its committed / availability path.
        if let Some(open_at) =
            super::breath_ideal_bridge::resolve(state, params, /* self_is_attacker */ true)
        {
            state.a.breath_burst_open_at = open_at;
        }
        if let Some(open_at) =
            super::breath_ideal_bridge::resolve(state, params, /* self_is_attacker */ false)
        {
            state.b.breath_burst_open_at = open_at;
        }
        ...
        process_phase_14_15_breath(
            ...
            hunker_active_b,
            &mut state.counters,
            &mut state.breath_scheduler_ticks_a,
            &mut state.breath_scheduler_ticks_b,
        );
    }
...
#[cfg(test)]
mod secondary_bite_modifier_tests {
    use super::apply_stat_modifiers;
    use crate::policy::state::PolicyValue;
    use crate::contracts::SimpleCombatantStats;
    use std::collections::BTreeMap;

    fn damage_mul(value: f64) -> BTreeMap<String, PolicyValue> {
        let mut extras: BTreeMap<String, PolicyValue> = BTreeMap::new();
        extras.insert("modifier.damage.mul.src.value".into(), PolicyValue::Number(value));
        extras.insert("modifier.damage.mul.src.until".into(), PolicyValue::Number(100.0));
        extras
    }

    #[test]
    fn a_damage_modifier_reaches_the_secondary_bite() {
        let mut eff = SimpleCombatantStats::default();
        eff.damage = 100.0;
        eff.damage2 = 250.0;

        apply_stat_modifiers(&mut eff, &damage_mul(1.5), 0.0);

        assert_eq!(eff.damage, 150.0);
        assert_eq!(
            eff.damage2, 375.0,
            "the secondary bite differs from the primary only in applying no on-hit \
             ailments, so a damage modifier has to scale it identically"
        );
    }

    #[test]
    fn a_secondary_only_modifier_still_applies_on_top() {
        let mut eff = SimpleCombatantStats::default();
        eff.damage = 100.0;
        eff.damage2 = 200.0;
        let mut extras = damage_mul(1.5);
        extras.insert("modifier.damage2.mul.src.value".into(), PolicyValue::Number(2.0));
        extras.insert("modifier.damage2.mul.src.until".into(), PolicyValue::Number(100.0));

        apply_stat_modifiers(&mut eff, &extras, 0.0);

        assert_eq!(eff.damage, 150.0);
        assert_eq!(eff.damage2, 600.0);
    }
}
