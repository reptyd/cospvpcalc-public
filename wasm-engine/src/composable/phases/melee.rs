...
pub(in super::super) fn process_phase_10_11_melee(
    ...
    if (ctx.a.next_hit - ctx.time).abs() <= 1e-9 {
        // Tag iter mask - A is dealing a bite this iter,
        ...
        let (kill_secure_a, heal_save_a) = if expunge_eligible {
            ...
            // Scale Expunge heal by the variant's effective damage.
            // Previously the TS bridge substituted `damage = damage2`
            // before serialization in SecondaryOnly mode, which made this
            // line read damage2 implicitly. The bridge no longer mutates
            // `damage`; we re-derive variant damage from `bite_eff_a` to
        ...
        if ctx.record_trace && applied_melee_damage_a > 0.0 {
            // Surface variant in the timeline so primary vs. secondary
        ...
        // Skip on-hit offensive ailments entirely when this bite
        ...
        if expunge_fires_a {
            ...
            // Same variant-aware heal as the eligibility check above.
            let heal_amount = EXPUNGE_HEAL_FRACTION_OF_BONUS
                * bite_eff_a.damage
                * EXPUNGE_DAMAGE_PER_STACK
                * bleed_on_b;
    ...
    if (ctx.b.next_hit - ctx.time).abs() <= 1e-9 {
        ...
        let (kill_secure_b, heal_save_b) = if expunge_eligible_b {
            ...
            // Variant-aware heal on the B side (mirror of A's).
            let heal_amount = EXPUNGE_HEAL_FRACTION_OF_BONUS
                * bite_eff_b.damage
                * EXPUNGE_DAMAGE_PER_STACK
                * bleed_on_a;
