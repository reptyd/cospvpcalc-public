...
//!
//! Holds `process_phase_16_post_tick` and the passive Warden's Rage
//! controller it calls.

#![allow(clippy::too_many_arguments)]

use super::super::{
    compute_first_strike_active, policy_bridge, user_dispatch,
    user_status_dispatch, CombatSide, DamageCounters, PhaseContext,
};
...
/// loop iteration. The 20 arguments after `ctx` carry iteration-scope
/// state computed earlier in the loop body that this phase needs to read /
/// mutate.
#[allow(clippy::too_many_arguments)]
pub(in super::super) fn process_phase_16_post_tick(
    ...
    // Per-tick corpse pin. A dead side never casts again, so retire its
    // Healing Pulse timer - the cast gate blocks the re-arm, which would
    // otherwise leave a stale past timestamp for the scheduler to filter
    // every tick.
    ...
    // Damage-event triggers. Phase 9: also entered for a dynamic
    ...
    if track_damage_triggers || track_status_hooks {
        ...
        // Derive per-kind event flags from the iter mask.
        ...
        // Pre-mitigation totals seen by the
    ...
    // Reactive triggers: status apply, status expire, per-status decay and
    // restack, kill, first strike, heal, and active end.
    ...
    if track_damage_triggers || track_status_hooks {
        ...
        if let (Some(pre_a), Some(pre_b)) = (status_keys_a_pre, status_keys_b_pre) {
            ...
            // Collect the actual per-id sets, not just counts.
            ...
            if track_status_hooks {
                ...
                // landing on or expiring on the bearer (mirror of the ability
        ...
        // on_heal - dispatch if accumulator > 0.
        ...
        // on_active_end - diff the pre-iter snapshot
                ...
                || !ctx.a.iter_builtin_active_until_pre.is_empty())
        {
            ...
            // Also diff the built-in active windows.
    ...
    // Dispatch user (custom) abilities for both sides.
    if !ctx.attacker.user_ability_ids.is_empty() {
    ...
    // Passive Warden's Rage controller. Runs after all damage phases so it
    // sees the iteration's FINAL post-bite HP and damage. The manual
    // controller (Phase 4j) owns the authority flag `warden_rage_manual_on`;
    // this passive layer runs mechanically on top.
    if ctx.config.attacker_warden_rage {
        apply_passive_warden_rage(ctx.a, ctx.attacker.health, ctx.a.death_time.is_some());
    }
    if ctx.config.defender_warden_rage {
        apply_passive_warden_rage(ctx.b, ctx.defender.health, ctx.b.death_time.is_some());
    }
}

/// Passive Warden's Rage controller.
///
/// - Auto-ON: damage landed this iteration while `hp < max_hp` turns the
///   shared switch on, with no cooldown and no block. Once on it stays on
///   through iterations in which no damage landed, until the side reaches
///   full HP or the manual controller turns it off. Turning it back on takes
///   a damage event in that iteration, so a manual deactivation in an
///   iteration with no damage releases exactly one buffered regen tick before
///   the next bite turns the switch on again.
/// - Auto-OFF at full HP, but only while NO manual controller holds it
///   (`warden_rage_manual_on == false`); authority stays with the manual
///   path until an explicit manual deactivation.
/// - Both transitions zero the strength value, so the bite multiplier is
///   1.0 whenever the switch is off.
pub(in super::super) fn apply_passive_warden_rage(
    side: &mut CombatSide,
    max_hp: f64,
    is_dead: bool,
) {
    ...
    let max_hp = max_hp.max(1.0);
    let below_full = side.hp < max_hp;
    let took_damage = side.iter_raw_damage_taken > 0.0;

    // The on/off transitions here (and the manual ones in Phase 4j) are turned
    // into timeline window events by `sync_conditional_passive_events`, which
    // diffs the settled per-iteration state - so no event is logged for an
    // individual transition here, which runs once per iteration per side.
    if below_full {
        if !side.warden_rage_on && took_damage {
            side.warden_rage_on = true;
        }
        if side.warden_rage_on {
            side.warden_rage_stacks = wardens_rage_stacks_from_hp_ratio(side.hp / max_hp);
        }
    } else if side.warden_rage_on && !side.warden_rage_manual_on {
        // Full HP and no manual hold: the game cleanses Warden's Rage here.
        side.warden_rage_on = false;
        side.warden_rage_stacks = 0;
        side.warden_rage_tap_until = 0.0;
    }
}
