...
//!
//! All 24 `process_phase_*` functions live here, plus the local types
//! they depend on (`SchedulerStep`, `SchedulerPassiveFlags`). The driver
//! loop in `composable/mod.rs` calls these via `phases::process_phase_*`.
//!
...
#[cfg(test)]
pub(super) use post_tick::apply_passive_warden_rage;
...
/// side recent-damage logs. `push_damage_window` drops entries older
/// than this, so a policy reading a window longer than 30 seconds reads
/// a truncated log.
...
/// is identical for both variants; this selects the variant, which
/// decides whether the bite reads `damage` or `damage2`.
///
...
/// Build the bite-effective stats clone for a secondary bite.
...
/// a fully-shielded bite leaves no entry.
...
/// Record a damage event on a (dealer, victim) pair into
...
/// post-mitigation amount that landed. The bite site writes
/// `iter_raw_damage_*` itself before it runs its own pre-damage hook,
/// so it passes raw == applied here. For breath and DOT this helper is
