//! Phase processor functions extracted from the megafunction driver.
//!
...
/// Round 45 / B4: add `delta` to a cumulative counter living on a
...
/// Round 46 / B2: maximum sliding-window length retained on the per-
...
/// P3: resolve which bite variant (primary vs. secondary) the
...
/// Round 46 / B2: push a `(time, amount)` entry to a sliding-window
...
/// every per-iteration accumulator: combat-counters (B4), sliding-
/// window buffers (B2), raw-damage iteration totals (A10b).
///
/// `raw` is the pre-mitigation amount, `applied` is the engine's
/// post-mitigation amount that actually landed. For bite, the
/// pre-damage hook layer (A13) lives at a different layer so we pass
