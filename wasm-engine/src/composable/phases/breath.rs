...
/// healing from breath damage. The two halves are
...
pub(in super::super) fn process_phase_14_15_breath(
    ...
    breath_scheduler_ticks_a: &mut u32,
    breath_scheduler_ticks_b: &mut u32,
) {
    // Head Start: an inert side does not breathe. The scheduler parks
    // `next_breath` on the window boundary while the side is inert, so the
    // breath ticks below never match `time` until the side resumes.
    // Cocoon Ph2: breath lands no damage and applies no statuses on a target
    // inside its Phase 2 window, and a creature in its own
    // Ph2 cannot breathe (all abilities blocked). Both mirror the bite
    // reschedules in process_phase_10_11_melee: hold A's breath until the
    // relevant Ph2 window ends, then the cadence resumes.
    if (ctx.a.next_breath - ctx.time).abs() <= 1e-9 && ctx.b.in_cocoon_phase_2(ctx.time) {
        ctx.a.next_breath = ctx.b.cocoon_phase2_until;
    }
    if (ctx.a.next_breath - ctx.time).abs() <= 1e-9 && ctx.a.in_cocoon_phase_2(ctx.time) {
        ctx.a.next_breath = ctx.a.cocoon_phase2_until;
    }
    // Reflect response: the same hold covers breath, because Reflect returns
    // breath damage as it returns bite damage. Abilities are not held: Reflect
    // does not return them, so holding one would delay it for no reduction in
    // damage taken.
    if ctx.a.reflect_response_hold
        && (ctx.a.next_breath - ctx.time).abs() <= 1e-9
        && ctx.b.reflect_active_until > ctx.time
    {
        ctx.a.next_breath = ctx.b.reflect_active_until;
    }
    if ctx.b.reflect_response_hold
        && (ctx.b.next_breath - ctx.time).abs() <= 1e-9
        && ctx.a.reflect_active_until > ctx.time
    {
        ctx.b.next_breath = ctx.a.reflect_active_until;
    }
    // Phase 14: Breath A
    if (ctx.a.next_breath - ctx.time).abs() <= 1e-9 {
        if let Some(breath) = ctx.attacker_breath {
            // A is breathing onto B.
            ...
            tick_breath_side(
                ...
                "A",
                "B",
                ctx.config.attacker_breath_policy,
            );
            if ctx.record_trace {
                    ...
                    && muddy_after_a.zip(muddy_before_a).map(|(after, before)| after > before + 1e-9).unwrap_or(muddy_after_a.is_some())
                {
                    ctx.combat_log.push(crate::contracts::CombatLogEntry {
                        ...
                        description: Some("Cloud Breath applied Muddy (2s)".to_string()),
                        detail: None,
            ...
            // Breath damage reaches the combat counters, the
            // sliding-window logs and the raw-damage accumulators
            // through `record_damage_event`. Raw equals applied here:
            // `tick_breath_side` applies breath resistance and Hunker
            // internally and returns one figure. The pre-damage hooks
            // (`on_before_*_damage`) do not run on breath damage;
            // running them requires `tick_breath_side` to return the
            // pre-mitigation and post-mitigation amounts separately.
            ...
            // Life Leech healing from breath damage.
    ...
    // Cocoon Ph2: symmetric to the A-side breath holds above.
    if (ctx.b.next_breath - ctx.time).abs() <= 1e-9 && ctx.a.in_cocoon_phase_2(ctx.time) {
        ctx.b.next_breath = ctx.a.cocoon_phase2_until;
    }
    if (ctx.b.next_breath - ctx.time).abs() <= 1e-9 && ctx.b.in_cocoon_phase_2(ctx.time) {
        ctx.b.next_breath = ctx.b.cocoon_phase2_until;
    }
    // Phase 15: Breath B
    if (ctx.b.next_breath - ctx.time).abs() <= 1e-9 {
        if let Some(breath) = ctx.defender_breath {
            // B is breathing onto A.
            ...
            tick_breath_side(
                ...
                "B",
                "A",
                ctx.config.defender_breath_policy,
            );
            if ctx.record_trace {
                push_breath_heal_log(
                    ctx.combat_log,
            ...
            // Symmetric to the A->B breath accounting above.
...
/// flag so the driver can early-out when neither side has the
/// ability.
pub(in super::super) fn process_phase_15b_15c_post_breath_hooks(
    ...
    if has_any_rewind {
        if ctx.config.attacker_rewind {
            record_rewind_snapshot(&mut ctx.a.rewind_history, ctx.time, ctx.a.hp);
        }
        if ctx.config.defender_rewind {
            record_rewind_snapshot(&mut ctx.b.rewind_history, ctx.time, ctx.b.hp);
        }
    }
