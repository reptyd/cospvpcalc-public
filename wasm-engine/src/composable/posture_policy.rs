...
//!
//! Per decision (~5 s cadence): clone state x 3 candidates x replay
...
use super::{DefensivePinControl, OrderedEventPhase};
...
/// The interval in force. Reads the const in shipped builds; a `#[cfg(test)]`
/// override lets one test process price several grids without a rebuild.
#[cfg(not(test))]
#[inline(always)]
pub(super) fn decision_periodic_sec() -> f64 {
    DECISION_PERIODIC_SEC
}

#[cfg(test)]
thread_local! {
    static DECISION_PERIODIC_OVERRIDE: std::cell::Cell<Option<f64>> =
        const { std::cell::Cell::new(None) };
}

#[cfg(test)]
pub(super) fn decision_periodic_sec() -> f64 {
    DECISION_PERIODIC_OVERRIDE
        .with(|c| c.get())
        .unwrap_or(DECISION_PERIODIC_SEC)
}

#[cfg(test)]
pub(super) fn set_decision_periodic_override(v: Option<f64>) {
    DECISION_PERIODIC_OVERRIDE.with(|c| c.set(v));
}
...
/// fight (O(T^2) at the 900-s cap -> infeasible). The persistent
/// post-window cost a naive finite window drops (settled-Lay's x1.75
...
/// FEATURE FLAG - exact, decision-identical speedup of the depth-4 tree
/// search via a shared-prefix checkpoint/fork DFS. Default OFF: the tree
/// block runs the byte-identical 4-nested-loop path (each of the 256 paths
/// re-simulated from the decision moment). When `true`, the 256 paths are
/// enumerated by a DFS that advances the inner replay ONE decision interval
/// per tree node from a checkpointed `LoopState`, so a shared action-prefix
/// is simulated once and forked - producing the IDENTICAL per-leaf outcomes
/// (same engine, same float ops, same order) at a fraction of the work. The
/// decision-identity invariant (same committed action) is gated by
/// `oracle.rs::decision_identity_gate`.
pub(super) const POSTURE_TREE_CHECKPOINT: bool = true;

/// FEATURE FLAG - Fortify's activation decision computed by a bounded engine
/// ROLLOUT instead of the hand-utility candidate search. Default OFF: when
/// `false`, Fortify uses the existing `policy_bridge::should_activate_now` path
/// unchanged (byte-identical). When `true`, `fortify_rollout_bridge::resolve`
/// forks the live engine per Ideal-grid candidate delay (force-fire at `now+d`
/// vs a skip baseline), scores each over the bounded `compute_replay_horizon`
/// window (extrapolated by `project_terminal_state`) with
/// `compute_replay_fitness`, and fires only when the best fire beats the skip
/// baseline. Inner replays run Fortify on its ReallyFast gate
/// (`AbilityPolicyMode::GateOnly`) so the rollout never re-enters itself.
pub(super) const FORTIFY_ROLLOUT: bool = true;

/// FEATURE FLAG - exact, decision-identical pruning of the Fortify rollout's
/// candidate sweep. Default ON. The verdict `run_rollout` returns is the
/// conjunction "the lead sits on `d = 0`" AND "that lead clears the skip
/// baseline", and `d = 0` is scored first, so once either term fails no later
/// candidate can restore it - every remaining fork is scored only to be thrown
/// away. When `true` the sweep stops at the fork that falsifies the verdict and
/// visits the delays longest-first (`PRUNE_SWEEP_ORDER`) so the falsifying one
/// comes early; when `false` every candidate is scored in `IDEAL_GRID` order -
/// same verdict, ~12 forks per decision instead of ~3. Forks are pure (each
/// runs on its own `LoopState` clone), so pruning and reordering cannot change
/// what any surviving fork scores. Proven by
/// `composable::fortify_rollout_identity_tests`.
pub(super) const FORTIFY_ROLLOUT_SHORT_CIRCUIT: bool = true;

/// FEATURE FLAG - Hunker / Warden's Rage toggle state decided by a
/// bounded engine REPLAY (held-plan ablation) instead of the deleted
/// analytic `on_off_delta`. Default OFF: when `false`, both toggles
/// resolve through their pi-zero gate (`AlwaysOnIfEligibleTogglePolicy`)
/// exactly as the flag-off precision path does today - byte-identical.
/// When `true`, `toggle_replay_bridge::resolve` runs a `hold_on` vs
/// `hold_off` (plus a Warden's harvest) ablation at the posture ~5 s
/// decision cadence, commits the winning `ToggleCommit`, and holds it
/// between epochs. Engaged only for the precision modes
/// (SemiIdeal / Ideal / Extreme); ReallyFast / Fast keep the cheap
/// gate. Inner replays run every OTHER toggle at its pi-zero gate
/// (`AbilityPolicyMode::GateOnly`, posture `ForcedOff`) so the replay
/// never nests itself.
pub(super) const TOGGLE_ROLLOUT: bool = true;
...
pub(super) fn compute_replay_fitness(
    ...
    match (my_death, opp_death) {
        ...
        // 5x better. Discovered 2026-05-22 by the
...
pub(super) fn decide(
    ...
    _fortify_control: DefensivePinControl,
    _max_time_sec: f64,
) -> PostureAction {
