...
// phase fns above the megafunction).
// ---------------------------------------------------------------------------
...
/// Round 32 / A5: write the five `env.*` flags into a side's
...
/// Round 42 / A11: seed each attached user ability's active level into
...
/// megafunction's loop iterations), and `'phase` is the lifetime of
...
/// block scope inside the loop body lets the rest of the megafunction
...
fn simulate_composable_matchup_with_trace_control(
    ...
    // Sprint 5.3: fire `on_round_start` for every user ability on
