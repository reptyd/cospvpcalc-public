...
// ---------------------------------------------------------------------------
// Aerial Dodge (Compare-only "air miss" rule)
// ---------------------------------------------------------------------------

/// Which incoming-attack channel is rolling an Aerial Dodge. Each channel
...
/// Compare-only Aerial Dodge: does an incoming bite / breath tick LAND on
/// `target`? Returns true (lands, no dodge) unless the rule is active, the
/// target can still dodge (no Shredded Wings), and the roll misses.
///
/// - Rule off (`aerial_dodge_active == false`) -> always lands, no roll, no
///   state mutation, so off-path runs stay byte-identical.
/// - `Shredded_Wings` present -> the flier is grounded, can't dodge, always
///   lands.
/// - hit chance >= 100 (non-fliers are set to 100 at the bridge) -> always
///   lands; <= 0 -> always dodges. Neither draws from the roll state.
...
pub(super) fn aerial_dodge_incoming_lands(
    ...
    // Only creatures that can take to the air dodge - fliers and gliders. A
    // grounded target is always hit.
    let is_aerial = target_stats
        .identity
        .as_ref()
        .map(|i| i.is_aerial)
        .unwrap_or(false);
    ...
    if target.statuses.contains_key("Shredded_Wings") {
        return true;
    }
