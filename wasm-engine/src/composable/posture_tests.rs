...
mod action_gating {
    ...
    fn laying_blocks_bite() {
        ...
        assert_eq!(probe.posture_incoming_damage_mult(), 2.0);
    }
...
mod policy {
    ...
    use crate::composable::side::CombatSide;
    use crate::composable::simulate_composable_matchup_with_trace;
    use crate::composable::{DefensivePinControl, OrderedEventPhase};
    use crate::contracts::SimpleAbilityTimingMode;
    ...
    fn decide_returns_stay_when_no_clear_improvement() {
        ...
        // nothing (HP already at max) but incurs the x1.75 incoming
    ...
    fn high_dps_opponent_blocks_lay_decision() {
        // [POSTURE:phase3] [REF:compare_posture_policy]
        // If the opponent does very high DPS, the x1.75 incoming
    ...
    fn policy_lays_for_regen_ticks_korathos_full_regen_build_vs_golgaroth() {
        ...
        // 4 % x 10 750 x 3 = ~1290 HP at x1 mult, ~2580 at x2 mult
        // (lay) - a 1290-HP swing. Golga's bite into Korathos is
        // ~165 dmg (weight ratio + bite-damage% effects), so 8 s of
        // settled-lay penalty is ~165 x 0.75 x 6 = ~740. Net for lay
...
mod replay_primitive {
    ...
    fn decide_does_not_perturb_outer_sides() {
        ...
        let _action = decide(
            ...
            SimpleAbilityTimingMode::Ideal,
            &event_phase_order,
            crate::composable::DefensivePinControl::default(),
            /* max_time_sec */ 60.0,
        );
    ...
    fn forced_off_breaks_recursion_inside_iter() {
        ...
        let mut state = LoopState {
            ...
            fortify_control: crate::composable::DefensivePinControl::default(),
        };
    ...
    fn spirit_glare_breath() -> SimpleBreathProfile {
        SimpleBreathProfile {
            ...
            special_statuses: vec![
                SimpleAppliedStatus { status_id: "Burn_Status".to_string(), stacks: 1.0, ..Default::default() },
                SimpleAppliedStatus { status_id: "Fear_Status".to_string(), stacks: 1.0, ..Default::default() },
            ],
            self_heal_pct: 0.0,
            cleanse_stacks: 0.0,
...
/// (DEF / ATK x unaware / aware) configs so we can answer the
...
/// settled-Lay x1.75 incoming damage applies to Spirit Glare
...
fn diagnose_compare_realistic_posture_policy_opra_vs_gimon() {
    ...
    let attacker_breath = SimpleBreathProfile {
        ...
        special_kind: Some("spirit_glare".to_string()),
        special_statuses: vec![
            SimpleAppliedStatus { status_id: "Burn_Status".into(), stacks: 1.0, ..Default::default() },
            SimpleAppliedStatus { status_id: "Fear_Status".into(), stacks: 1.0, ..Default::default() },
        ],
