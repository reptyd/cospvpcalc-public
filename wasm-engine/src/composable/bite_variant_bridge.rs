...
use super::loop_iter::{
    run_one_event_loop_iter, AbilityPolicyMode, BiteVariantOverrideFn, IterHooks, LoopOutcome,
...
impl BiteVariantReplayer for LoopStateBiteVariantReplayer<'_> {
    fn replay_with_plan(
        ...
        // x ~12 decisions x ~5 ms ~ 15 s per posture call). Running
        // that under EVERY bite-variant replay (x120 per fight) is
        ...
        let inner_params = LoopParams {
            ...
            // With the Fortify or toggle rollout on, force those abilities to
            // their gate inside the bite-variant inner replay so they can't
            // nest a rollout. Both flags off => Normal => byte-identical.
