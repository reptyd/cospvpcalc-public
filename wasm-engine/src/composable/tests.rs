    ...
    fn cloud_breath_applies_muddy_status_deterministically() {
        ...
        let got = simulate_composable_matchup_with_trace(
            ...
            // The uptime-calibrated accumulator lands its first proc at
            // t=2.5; the window has to reach past it.
            3.0,
            true,
        );
        ...
        assert!(
            got.combat_log
                .as_ref()
                .is_some_and(|log| log.iter().any(|entry| entry.description.as_deref() == Some("Cloud Breath applied Muddy (2s)")))
        );
        assert!(
            got.combat_log
                .as_ref()
    ...
    fn heal_breath_self_heal_is_recorded_in_trace() {
        ...
        // The breath fires at 0.5 s but heals once per whole second, so the
        // heal entries are half the fired ticks. Breath Time is read from the
        // fired ticks; deriving it from the log would report half the seconds
        // the breath was actually running.
        let heal_entries = got
            .combat_log
            .as_ref()
            .map(|log| {
                log.iter()
                    .filter(|entry| entry.description.as_deref() == Some("Heal Breath heal"))
                    .count()
            })
            .unwrap_or(0);
        let fired = got
            .debug
            .as_ref()
            .map(|debug| debug.a.breath_fire_times.len())
            .unwrap_or(0);
        assert_eq!(heal_entries, 2, "one heal per second over the 2 s window");
        assert_eq!(fired, 4, "two fired ticks per second over the same window");
    }
    ...
    fn shadow_barrage_hits_and_payloads_are_traced_as_shadow_barrage() {
        ...
        assert!(
            log.iter().any(|entry| {
                // Burst-on-activation multiplies on-hit stacks by
                // the configured count (2 stacks x 2 hits = 4 stacks)
    ...
    #[test]
    fn direct_bite_weight_scale_downscales_a_lighter_attackers_ailments() {
        let mut a = simple_stats(10_000.0, 1.0, 100.0);
        a.weight = 100.0;
        a.on_hit_statuses = vec![status("Disease_Status", 4.0)];
        let mut b = simple_stats(10_000.0, 0.0, 100.0);
        ...
        assert!(
            log.iter().any(|entry| {
                entry.description.as_deref() == Some("Bite applied Disease (3)")
                    && entry.status_id.as_deref() == Some("Disease_Status")
            }),
            "a half-weight attacker scales its 4 Disease stacks by 0.75"
        );
    }
    ...
    fn all_four_trails_parallel_structure() {
        ...
        // With 4 trails at 2% max HP each (B has 1200 hp -> 24 dmg x 4 = 96/s),
    ...
    // drains 1.5x while overfilled, Reflux costs 25% base appetite. Defiled
    // Ground reduces drain by 20/50/80% @ level 1/2/3 (x 1.2 when opponent
    // is weakened).

    #[test]
    fn hunger_rule_runs_a_whole_matchup() {
    ...
    // future hit timings instead of a DPSxstep approximation. For any policy-
    ...
    #[test]
    fn really_fast_fortify_holds_the_opening_cast() {
        // The stack threshold itself is covered by the decision's unit tests;
        // what this pins end-to-end is that ReallyFast spends the opening
        // seconds letting pressure build instead of cleansing a fresh fight.
        let mut a = simple_stats(1500.0, 60.0, 2.0);
        a.starting_statuses = vec![status("Frostbite_Status", 40.0)];
        let b = simple_stats(2000.0, 55.0, 2.2);
        let mut config = ComposableAbilityConfig::default();
        config.attacker_fortify = true;

        let inside_hold = simulate_composable_matchup_with_trace(
            &a,
            &b,
            None,
            None,
        ...
        let inside_log = inside_hold.combat_log.as_ref().expect("combat log");
        assert!(
            !inside_log
                .iter()
                .any(|entry| entry.description.as_deref() == Some("Fortify activated")),
            "ReallyFast Fortify must not fire inside the opening hold"
        );

        let past_hold = simulate_composable_matchup_with_trace(
            &a,
            &b,
            None,
            None,
        ...
        let first = first_fortify_time(&past_hold).expect("Fortify must fire past the opening hold");
        assert!(
            first >= 8.0,
            "ReallyFast Fortify fired at {first}, inside the opening hold"
        );
    }
    ...
    fn hunters_curse_and_unbridled_rage_stack_multiplicatively_on_bite_damage() {
        ...
        // Sanity: HC alone equals x2.0 vs an active-less baseline (already
    ...
    // Burn; attacker HP capped DOWN to 15% (if currently higher). Cooldown 300s.
    //
    ...
    fn self_destruct_unit_caps_attacker_hp_down() {
        ...
        let defender = simple_stats(2000.0, 50.0, 2.0);
        let profile = sd_profile();
        // Attacker at 40% HP (above the 15% cap) -> gets capped DOWN to 15% = 150.
        let mut att_hp = 400.0;
        let mut def_hp = 2000.0;
        let mut att_statuses = BTreeMap::new();
        let mut def_statuses = BTreeMap::new();
        let mut cooldown = 0.0;
        ...
        assert!((att_hp - 150.0).abs() < 1e-9, "att_hp capped DOWN to 15% = 150, got {}", att_hp);
    }
    ...
    fn self_destruct_unit_leaves_low_hp_alone() {
        ...
        let defender = simple_stats(2000.0, 50.0, 2.0);
        let profile = sd_profile();
        // Attacker at 2% HP (below the 15% cap) -> untouched.
        let mut att_hp = 20.0;
        let mut def_hp = 2000.0;
        let mut att_statuses = BTreeMap::new();
        let mut def_statuses = BTreeMap::new();
    ...
    #[test]
    fn cocoon_phase_one_bite_carries_no_damage_buff() {
        // F1 regression: Cocoon_Damage_Status is seeded at activation (Ph1
        // start), but the +15% Cocoon Rage buff is a Phase-3 effect. A bite
        // landing inside Ph1 [T, T+5) must deal baseline damage, not 1.15x.
        // Pre-fix this bite was buffed because the multiplier only checked the
        // stack count, not the phase window.
        let a = simple_stats(1000.0, 100.0, 1.0);
        let b = simple_stats(1_000_000.0, 80.0, 1.0);
        let mut cfg = ComposableAbilityConfig::default();
        cfg.attacker_cocoon = true;
        let res = crate::composable::simulate_composable_matchup_with_trace(
            &a, &b, None, None, SimpleAbilityTimingMode::Fast, &cfg, 15.0, true,
        );
        let log = res.combat_log.expect("trace log");
        let cocoon_t = log
            .iter()
        ...
        let p1_bite = log
            ...
            .map(|e| e.damage)
            .expect("a bite must land inside Phase 1");
        // Baseline bite, no Cocoon: same combatants, so the unbuffed melee
        // damage is identical (no buffs are wired in either run).
        let bare = crate::composable::simulate_composable_matchup_with_trace(
            &a, &b, None, None, SimpleAbilityTimingMode::Fast,
            &ComposableAbilityConfig::default(), 15.0, true,
        );
        let bare_bite = bare
            .combat_log
            .expect("trace log")
        ...
        let ratio = p1_bite / bare_bite;
        assert!(
            (ratio - 1.0).abs() < 1e-3,
            "Phase 1 bite must NOT carry the Ph3 +15% buff: p1_bite={p1_bite}, baseline={bare_bite}, ratio={ratio}"
        );
    }

    #[test]
    fn cocoon_phase_two_suppresses_dot_damage() {
        // F2 regression: while sealed in Phase 2 the bearer is
        // ImmuneToAilmentDamage - DoT ticks deal 0 damage but the status
        // persists. Direct unit test of the tick handler's `cocoon_invul`
        // gate (mirrors the `laying`/Hypothermia gate test).
        ...
        let make = || {
            ...
            statuses.insert(
                ...
                SimpleStatusInstance {
                    ...
                    // Far-future decay so the single tick under test keeps its
                    // stacks - isolates the damage gate from decay.
        ...
        // Cocoon Ph2 -> 0 damage, status still present.
        let mut statuses = make();
        let mut hp = 10_000.0;
        let mut dealt = 0.0;
        handle_simple_dot_ticks_full(
            0.0, 10_000.0, 0.0, &mut hp, &mut statuses, &mut dealt, false, None, 1.0, false, true, 1.0, 1.0,
        );
        assert!(
            (hp - 10_000.0).abs() < 1e-9,
            "Cocoon Ph2 must nullify the DoT damage tick: hp={hp}"
        );
        assert!(
            statuses.contains_key("Burn_Status"),
            "the DoT status must persist through Phase 2"
        );

        // Not in Ph2 -> the tick lands its damage.
        let mut statuses = make();
        let mut hp = 10_000.0;
        let mut dealt = 0.0;
        handle_simple_dot_ticks_full(
            0.0, 10_000.0, 0.0, &mut hp, &mut statuses, &mut dealt, false, None, 1.0, false, false, 1.0, 1.0,
        );
        assert!(
            hp < 10_000.0,
            "outside Phase 2 the DoT tick must deal damage: hp={hp}"
        );
    }

    #[test]
    fn cocoon_phase_two_blocks_opponent_breath() {
        // F3 regression: a target sealed in Phase 2 is ImmuneToDamageFrom
        // Players - opponent breath lands no damage during [T+5, T+10). Pre-fix
        // breath had no cocoon gate and kept ticking through the invincibility
        // window. The breath is held to Ph2 end, mirroring the bite reschedule.
        let a = simple_stats(10_000.0, 10.0, 3.0);
        let b = simple_stats(1_000_000.0, 350.0, 0.5);
        let breath_b = SimpleBreathProfile {
            dps_pct: 10.0,
            capacity: 60.0,
            regen_rate: 10.0,
            crit_chance_pct: 0.0,
        ...
        cfg.attacker_cocoon = true;
        let res = crate::composable::simulate_composable_matchup_with_trace(
            &a, &b, None, Some(&breath_b), SimpleAbilityTimingMode::Fast, &cfg, 15.0, true,
        );
        let log = res.combat_log.expect("trace log");
        let cocoon_t = log
            .iter()
        ...
        assert!(
            p1_breaths > 0,
            "Phase 1 [{cocoon_t}, {}) is normal play - opponent breath must land",
            cocoon_t + 5.0
        );
        let p2_breaths = log
            .iter()
        ...
        assert_eq!(
            p2_breaths, 0,
            "Phase 2 [{}, {}) invincibility must block all opponent breath",
            cocoon_t + 5.0,
            cocoon_t + 10.0,
        );
    }
    ...
    // Bleed stacks on the target at bite time; bonus damage = D_normal x
    // (1 + 0.05 x bleed); heal owner for flat 0.5 x baseAttack x 0.05 x bleed.
    ...
    /// Smoke test: a user-defined ability attached to the
    ...
    /// abilities are *live* in combat - the spec / UI / dispatcher
    /// infrastructure existed already, but the engine integration
    /// landed here.
    #[test]
    ...
    /// on_round_start trigger fires at `t = 0`,
    ...
    /// on_tick trigger fires every interval_sec. With a
    ...
    fn user_ability_on_tick_fires_periodically() {
        ...
        // ticks once per loop iteration when due -
        ...
        // we should fire many ticks. 50 dmg per tick x 10+ ticks.
        assert!(
            extra_damage >= 400.0,
    ...
    /// on_take_damage fires when the actor takes damage
    ...
    fn user_ability_on_take_damage_reflects() {
        ...
        // ~15 bites in 30s x 100 reflect = 1500 extra hp lost on
    ...
    /// ModifyStat damage +mul lands in user_extras and
    ...
    /// ModifyStat on `damage2` flows to the
    ...
    /// ModifyStat on `hunker_reduction_pct`
    ...
    /// ModifyStat on `breath_resistance` flows
    ...
    /// ModifyStat on `unbreakable_damage_cap_pct`
    ...
    /// ModifyStat on `first_strike_pct` flows
    ...
    /// ModifyStat on `bite_cooldown` now drives
    ...
    /// ModifyStat on the berserk cadence fields
    ...
    /// ModifyStat on `plushie_reflect_avg_pct`
    ...
    /// ModifyStat on `first_strike_hp_ratio_threshold`
    ...
    /// ModifyStat on `quick_recovery_hp_ratio_threshold`
    /// reaches the live regen tick (effective_hp_regen_multiplier) via eff.
    /// A is a regen tank under steady fire: without Quick Recovery its base
    /// regen can't outpace the fire and it dies; with it, the low-HP boost
    /// ramps up as A drops below 40% HP until regen matches the incoming
    /// damage, so A stabilises at low HP and survives the whole fight.
    #[test]
    fn user_ability_modify_stat_quick_recovery_saves_low_hp() {
        ...
        // per 15s tick (267/s). B deals 350/s - above A's base regen, so without
        // the boost A dies. Quick Recovery ramps regen up below 40% HP (toward 2x
        // at 0 HP); the boost catches the incoming fire at low HP, so A stabilises
        // above 0 and survives.
    ...
    /// A permanent FormSwap raises max
    ...
    /// The `on_heal` trigger now fires from built-in
    ...
    /// fail if life leech did not feed the accumulator.
    #[test]
    ...
    /// The `on_active_end` trigger now fires when a
    ...
    /// Would fail if only `user.*` windows fired the trigger.
    #[test]
    ...
    /// baseline takes the full breath. Would fail if breath bypassed
    ...
    fn user_ability_on_before_take_damage_absorbs_breath() {
        ...
        assert!(
            ...
            "on_before_take_damage shield must absorb breath via the pre-damage hook; \
             shielded_taken={shielded_taken}, baseline_taken={baseline_taken}",
        );
    }

    /// The pre-damage hook now fires on the
    ...
    /// no-ability baseline bleeds out normally. Would fail if DOT
    /// bypassed the hook.
    #[test]
    fn user_ability_on_before_take_damage_absorbs_dot() {
        ...
        let baseline_taken = 1_000_000.0 - baseline.final_hp_b;
        let shielded_taken = 1_000_000.0 - with_shield.final_hp_b;
        // 5 Bleed stacks at 2 flat/stack/tick, decaying a stack each tick,
        // deal ~20 total over the fight - enough to prove a real DOT for the
        // shield to absorb below.
        assert!(
            baseline_taken > 10.0,
        ...
        assert!(
            shielded_taken < baseline_taken * 0.1,
            "on_before_take_damage shield must absorb DOT via the pre-damage hook; \
             shielded_taken={shielded_taken}, baseline_taken={baseline_taken}",
        );
    }

    /// The pre-damage hook now fires on
    ...
    /// the baseline takes it. Would fail if the hook were bypassed.
    #[test]
    fn user_ability_on_before_take_damage_absorbs_damage_trail() {
        ...
        assert!(
            ...
            "on_before_take_damage shield must absorb the damage trail via the \
             pre-damage hook; shielded_taken={shielded_taken}, baseline_taken={baseline_taken}",
        );
    }
    ...
    /// fail if reflected damage bypassed the hook.
    #[test]
    fn user_ability_on_before_take_damage_absorbs_reflect() {
        ...
        assert!(
            ...
            "on_before_take_damage shield must absorb reflected damage via the \
             pre-damage hook; shielded_loss={shielded_loss}, baseline_loss={baseline_loss}",
        );
    }

    /// A user ability whose on_fire chains another user
    ...
    fn user_ability_trigger_ability_chains_on_fire() {
        ...
        // 30s / 5s cooldown = 6 fires x 200 dmg = 1200 extra dmg
    ...
    /// Reentrancy guard: A triggers B triggers A - the
    ...
    /// Dispatch-time integration - a spec with `scaling`
    ...
    /// Per-fight active-level override flows through
    ...
    /// Out-of-range overrides silently fall back to
    ...
    /// Passive shield - a defender with
    ...
    /// Damage amplifier - attacker with
    ...
    /// `event.raw_damage` and `event.prevented_damage`
    ...
    /// combat.bites_dealt counter. An ability gated on
    ...
    /// Batch-level `when` gate. When set and falsy,
    ...
    /// `opp.fired_count.<id>` mirrors `self.fired_count`,
    ...
    /// A defender that fires a 5000-dmg counterattack
    ...
    /// PushExtra + ClearExtraArray + the .length / .sum /
    ...
    /// Ratio preserves the HP fraction, so HP scales back up toward the restored
    /// max and B sits near full again. Absolute keeps the current HP, so B stays
    /// near the capped value.
