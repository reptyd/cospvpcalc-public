...
//!
//! Status apply inside `apply_yolk_bomb` goes through
//! `apply_statuses_with_trace`, so when a trace sink is supplied the
//! apply pushes a "Yolk Bomb applied <Status> (N)" entry per status.
//! Stack-count claims are verified by inspecting BTreeMap state after a
//! direct call; cooldown is verified via the simulation trace.
...
use crate::spec_constants::YOLK_BOMB_COOLDOWN_SEC;
...
fn harmful_value_applies_two_slow_plus_four_value_stacks_to_opponent() {
    ...
    // Use "Burn" (harmful) - engine routes Slowed x 2 + Burn x 4 to
    // defender.
    ...
    let mut log: Vec<crate::contracts::CombatLogEntry> = Vec::new();
    apply_yolk_bomb(
        0.0, Some("Burn"),
        &attacker, &defender,
    ...
    // Effect application now emits a timeline event (the trap/area
    // effect-logging fix): "<ability> applied <Status> (N)".
    assert!(
        log.iter()
            .any(|e| e.description.as_deref() == Some("Yolk Bomb applied Burn (4)")),
        "Yolk Bomb must log its Burn application to the timeline: {:?}",
...
fn beneficial_value_routes_full_effect_to_user() {
    // [REF:ability_yolk_bomb]
    // Bullet 2 (beneficial branch): "Beneficial values route the full effect
    // to the user, harmful values route it to the opponent." Use "Healing
    // Pulse" - engine routes Slowed x 2 + Healing_Pulse_Status x 4 to
    // attacker.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000.0);
    let mut atk_st: BTreeMap<String, SimpleStatusInstance> = BTreeMap::new();
...
fn fortify_value_grants_immunity_window_instead_of_status() {
    // [REF:ability_yolk_bomb]
    // Bullet 3: "When the value is Fortify, Yolk Bomb grants a 12 second
    // status-immunity window and the 5% weight bonus that goes with it,
    // instead of applying a status." Engine: SelfFortify branch sets
    // `fortify_immune_until = time + 12.0` and weight bonus, then applies
    // Slow_Status x 2 to user only - no Fortify status is added to attacker's
    // statuses.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000.0);
    let mut atk_st: BTreeMap<String, SimpleStatusInstance> = BTreeMap::new();
    ...
    // Slowed x 2 still applied to user.
    let slow = atk_st.get("Slow_Status").map(|s| s.stacks).unwrap_or(0.0);
    assert!(
        (slow - 2.0).abs() < 1e-9,
...
fn cooldown_thirty_seconds() {
    // [REF:ability_yolk_bomb]
    // Bullet 6: "It has a 30 second cooldown."
    ...
    assert!(
        (gap - YOLK_BOMB_COOLDOWN_SEC).abs() < 1.0,
        "second Yolk Bomb activation must land ~{YOLK_BOMB_COOLDOWN_SEC} s after the first: gap={gap}, times={activations:?}"
    );
}
