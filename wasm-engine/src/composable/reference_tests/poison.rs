...
//!   `compute_simple_dot_damage(_, "Poison_Status", stacks) =
//!   max_hp * (0.2 + 0.05 * stacks) / 100`.
//! - 3 s tick cadence: `statuses.rs:13` returns `Some(3.0)` for
//!   `Poison_Status` from `status_tick_sec`.

use crate::spec_constants::{POISON_DOT_BASE_PCT_MAX_HP, POISON_DOT_PER_STACK_PCT_MAX_HP};
...
fn damage_starts_at_zero_point_two_percent_and_increases_by_zero_point_zero_five_per_stack() {
    ...
    let base = compute_simple_dot_damage(max_hp, "Poison_Status", 0.0);
    let expected_base = max_hp * POISON_DOT_BASE_PCT_MAX_HP / 100.0;
    assert!(
        (base - expected_base).abs() < 1e-9,
        "post-decay 0 Poison stacks must deal base {POISON_DOT_BASE_PCT_MAX_HP}% maxHP ({expected_base} on 10000): got {base}"
    );
    // 1 stack -> 0.2% + 0.05% = 0.25% -> 25 dmg.
    let one = compute_simple_dot_damage(max_hp, "Poison_Status", 1.0);
    let expected_one =
        max_hp * (POISON_DOT_BASE_PCT_MAX_HP + POISON_DOT_PER_STACK_PCT_MAX_HP) / 100.0;
    assert!(
        (one - expected_one).abs() < 1e-9,
        "1 Poison stack must deal {expected_one} (base + per-stack on 10000): got {one}"
    );
    // 10 stacks -> 0.2% + 0.5% = 0.7% -> 70 dmg.
    let ten = compute_simple_dot_damage(max_hp, "Poison_Status", 10.0);
    let expected_ten =
        max_hp * (POISON_DOT_BASE_PCT_MAX_HP + 10.0 * POISON_DOT_PER_STACK_PCT_MAX_HP) / 100.0;
    assert!(
        (ten - expected_ten).abs() < 1e-9,
        "10 Poison stacks must deal {expected_ten} (base + 10×per-stack on 10000): got {ten}"
    );
