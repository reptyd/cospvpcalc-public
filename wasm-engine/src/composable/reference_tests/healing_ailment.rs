...
//! sees it.
//!
//! Engine path: `phases/phase4.rs::process_phase_4_healing_ailment_tick`
//! fires a heal tick every `HEALING_AILMENT_TICK_SEC = 15.0` seconds
//! while the side carries `Healing_Ailment` with stacks > 0, healing
//! `HEALING_AILMENT_HEAL_PCT_PER_TICK = 7.0` % maxHP at its base rate.
//! The game's HealingPulse adds a flat +7 to HealthRegen after the
//! base-regen multipliers, so no regen multiplier scales it: status
//! regen mods (Bleed x0, ...), Quick Recovery, Harden and the Compare
//! regen bonus are all bypassed, and it keeps firing while bleed/burn
//! have zeroed natural regen. Only the recipient's posture multiplies
//! the pulse (1.5x sitting, 2x laying). The tick
//! scheduler `healing_ailment_next_tick_at` is seeded only by the
//! Healing Pulse cast path - so these tests deliver the status via
//! `attacker_healing_pulse` (Once-at-start) on side A.
//!
//! The tick is also gated by `is_external_healing_blocked` (Heartbroken)
//! - see `composable/reference_tests/heartbroken.rs`.

use super::super::config::ComposableAbilityConfig;
use super::super::simulate_composable_matchup_with_trace;
use super::default_combatant;
use crate::contracts::{SimpleAbilityTimingMode, SimpleAppliedStatus, SimpleCombatantStats};
use crate::spec_constants::{HEALING_AILMENT_HEAL_PCT_MAX_HP, HEALING_AILMENT_TICK_SEC};
...
fn fires_a_discrete_heal_every_fifteen_seconds_within_coverage_window() {
    ...
    assert!(
        (t - HEALING_AILMENT_TICK_SEC).abs() < 1.0,
        "first Healing Ailment tick must land near t={HEALING_AILMENT_TICK_SEC} ({HEALING_AILMENT_TICK_SEC} s cadence): got t={t}"
    );
}
...
fn each_heal_restores_seven_percent_max_hp() {
    // [REF:status_healing_ailment]
    // Bullet 2: "Each heal restores 7% of the target's max HP at its base
    // rate." Strategy: 30 000 max HP attacker, pre-wound via biter; observe
    ...
    let expected = 30_000.0 * HEALING_AILMENT_HEAL_PCT_MAX_HP / 100.0;
    assert!(
        (healed - expected).abs() < 1.0,
        "Healing Ailment heal must be {HEALING_AILMENT_HEAL_PCT_MAX_HP}% maxHP (= {expected} on 30000): got {healed}"
    );
}

#[test]
fn heal_survives_bleed_regen_disable() {
    // [REF:status_healing_ailment]
    // The flat +7 is added on top of natural health regeneration, so a
    // regen-disabling ailment does NOT suppress it: the tick still fires
    // and heals the full 7% maxHP even while Bleed has collapsed the regen
    // multiplier to zero.
    // Strategy: attacker carries Bleed x 10 at start (zeroes regen through
    // t~30) AND Healing Pulse Once-at-start (applies Healing_Ailment x 10
    // at t=0); the t~15 tick must still heal 7% maxHP.
    ...
    assert!(
        first_tick.is_some(),
        "Healing Ailment must still fire under Bleed (regen-disable does not touch the flat +7%)"
    );
    let healed = first_tick.unwrap().healing.unwrap_or(0.0);
    let expected = 10_000.0 * HEALING_AILMENT_HEAL_PCT_MAX_HP / 100.0;
    assert!(
        (healed - expected).abs() < 1.0,
        "under Bleed the pulse must still heal {HEALING_AILMENT_HEAL_PCT_MAX_HP}% maxHP (= {expected}): got {healed}"
    );
}
...
fn stacks_act_as_duration_more_stacks_extend_the_window() {
    // [REF:status_healing_ailment]
    // Bullet 6: "Stacks act as duration: 10 stacks corresponds to about 30
    // seconds of coverage (2 heal ticks). More stacks extend the window
    // proportionally." Engine: each Healing_Ailment stack decays 1 per 3 s;
    // the tick at t=30 with a 10-stack carrier sees post-decay stacks=0 and
    // skips, so the integer result is 1 tick on a 10-stack carrier across a 50
    // s window. The Healing Pulse Normal-mode cast re-applies 10 stacks every
    // 90 s, so a 16-second window captures the t=0 cast, the t=15 tick, and
    // nothing else; a 95-second window also captures the second cast at t=90
    // and a following tick. We assert the proportional-extension shape: a
    // longer simulation window observes strictly more ticks.
    let attacker = passive_combatant(10_000.0);
    let mut biter = default_combatant();
    biter.damage = 50.0;
    biter.bite_cooldown = 1.0;
