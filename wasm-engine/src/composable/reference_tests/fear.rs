...
use crate::spec_constants::FEAR_OUTGOING_DAMAGE_REDUCTION_PCT;
...
fn reduces_outgoing_damage_by_forty_five_percent() {
    // [REF:status_fear]
    // Bullet 1: "Fear reduces outgoing melee damage by 45% while it is
    // active."
    ...
    let expected = 1.0 - FEAR_OUTGOING_DAMAGE_REDUCTION_PCT / 100.0;
    assert!(
        (ratio - expected).abs() < 1e-9,
        "Fear must multiply outgoing damage by {expected} (-{FEAR_OUTGOING_DAMAGE_REDUCTION_PCT}%): got ratio {ratio} (with={with_fear}, base={baseline})"
    );
}
...
fn strength_does_not_stack() {
    // [REF:status_fear]
    // Bullet 3: "The number of stacks only affects how long Fear lasts."
    let mut atk = default_combatant();
    atk.damage = 100.0;
    let def = default_combatant();
