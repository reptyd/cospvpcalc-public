...
fn outgoing_melee_damage_halved_when_on() {
    // [REF:ability_hunker]
    // Bullet 1: "While Hunker is on, the user's melee damage is multiplied by
    // 0.5." `apply_hunker_to_damage(damage, true) == damage * 0.5`.
    let raw = 200.0_f64;
...
fn incoming_damage_reduced_by_hunker_value_percent() {
    // [REF:ability_hunker]
    // Bullet 3: "While Hunker is on, incoming direct damage is reduced by the
    // user's Hunker value." `apply_hunker_to_incoming(damage, pct, true) ==
    // damage * (1 - pct/100)`.
    let raw = 100.0_f64;
...
fn example_hunker_forty_reduces_incoming_by_forty_percent() {
    // [REF:ability_hunker]
    // Bullet 4: "For example, Hunker 40 reduces incoming direct damage by
    // 40%."
    let raw = 250.0_f64;
...
fn incoming_reduction_applies_to_both_bite_and_breath() {
    // [REF:ability_hunker]
    // Bullet 5: "This incoming reduction applies to direct bite damage and
    // direct breath damage." Two simulations isolating each damage path: (a)
    // Bite-only attacker -> defender's outgoing bite to A is reduced. (b)
    // Breath-only attacker -> defender's outgoing breath to A is reduced. In
    // both cases A holds Hunker; final HP under Hunker must exceed the
    // no-Hunker baseline (less damage taken).
...
fn stays_on_without_active_window_or_cooldown() {
    // [REF:ability_hunker]
    // Bullet 7: "Hunker has no timed window and no cooldown. It stays on until
    // the policy turns it off or actives are disabled."
...
fn re_toggle_takes_five_seconds_to_take_hold() {
    // [REF:ability_hunker]
    // Bullet 8: "If Hunker is turned off and then back on, the new Hunker
    // effect takes 5 seconds to take hold; the very first activation in a
    // fight has no delay." `resolve_hunker_effect_starts_at` returns
    // `current_time` for the first activation (`prior_activation_count == 0`)
    // and `current_time + HUNKER_EFFECT_DELAY_SEC` for any later flip
