...
//! cast applies `Healing_Ailment x HEALING_PULSE_STACKS_PER_CAST = 10`
...
//! self-only and `next_healing_pulse` is set to +inf.

use super::super::config::ComposableAbilityConfig;
use super::super::simulate_composable_matchup_with_trace;
use super::default_combatant;
use crate::contracts::{SimpleAbilityTimingMode, SimpleCombatantStats};
use crate::spec_constants::HEALING_PULSE_COOLDOWN_SEC;
...
fn normal_mode_applies_ten_healing_ailment_to_both_sides_at_t_zero() {
    ...
    // Bullet 2: "In Normal mode each activation applies 10 stacks of Healing
    // Ailment to the user and the opponent, because the in-game radius covers
    // both in a stand-and-fight exchange." Plus Normal mode bullet 3:
    // "Normal: the user casts at t=0..." Engine: Phase 4d-ter applies
    // Healing_Ailment x 10 to both A and B at the first eligible tick.
    // Healing_Ailment heals max-HP % every 15s while stacks > 0; first heal
    // observable as a downstream event, but the activation log entry alone is
    // sufficient proof of the t=0 cast.
...
fn normal_mode_recasts_every_ninety_seconds() {
    // [REF:compare_healing_pulse]
    // Bullet 3: "Normal: the user fires at the start of the fight and again
    // every 90 seconds of cooldown for the rest of the fight."
    ...
    assert!(
        (gap - HEALING_PULSE_COOLDOWN_SEC).abs() < 1.0,
        "second Healing Pulse cast must land ~{HEALING_PULSE_COOLDOWN_SEC} s after the first: gap={gap}, times={casts:?}"
    );
}
...
fn once_at_start_casts_only_once_self_only() {
    // [REF:compare_healing_pulse]
    // Bullet 4: "Once at start: the user fires a single time at the start of
    // the fight, targeting only the user - the opponent does not receive
    // Healing Ailment." Engine: SelfFortify-like single-shot, then
    // `next_healing_pulse = f64::INFINITY` blocks reschedules. Run a long
    // window and confirm exactly one cast event AND the defender does NOT
    // carry Healing_Ailment downstream effects (no defender-side heal log).
    let attacker = passive_combatant(10_000_000.0);
    let mut defender = default_combatant();
    defender.damage = 1.0;
    defender.bite_cooldown = 5.0;
