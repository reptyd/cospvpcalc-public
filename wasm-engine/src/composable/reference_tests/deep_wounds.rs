...
fn blocks_natural_bleed_decay_while_active() {
    ...
    // Bullets 1 + 2: "Deep Wounds blocks natural Bleed decay while it is
    // active." + "Bleed stacks already on the creature are held at their
    // count until Deep Wounds expires; Bleed applied during the window
    // is held too." Strategy: pre-seed attacker with Bleed x 5 +
    // Deep_Wounds x 5 (= 15 s of decay block). Run 12 s window. Compare
    // cumulative Bleed DoT damage to a control with only Bleed x 5 (decay
    // active). With decay blocked, all 4 Bleed DoT ticks (t=3, 6, 9, 12) fire
    // at full 5 stacks -> bigger total Bleed damage.
