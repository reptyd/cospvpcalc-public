...
fn capacity_is_ten_seconds_of_firing() {
    ...
    // fire spends 1 unit." Capacity is in seconds (10 s of firing). The
    // self-heal rides the 1 s regen loop, so with the user wounded each second
    // we observe exactly 10 self-heal events across the first burst before it
    // exhausts.
    ...
    assert_eq!(
        burst_heals.len(),
        10,
        "first Heal Breath burst fires 10 s; self-heal lands once per second (10 heals): {burst_heals:?}"
    );
}
...
fn self_heals_once_per_second_while_firing() {
    // [REF:ability_heal_breath]
    // The fire tick runs at 2 Hz (drives the cleanse), but the self-heal
    // rides the 1 s regen loop: the first heal lands at t=1.0 (one second
    // of accumulated firing) and successive heals are 1.0 s apart.
    let mut attacker = passive_combatant(1_000_000.0);
    attacker.bite_cooldown = 1000.0;
    let mut defender = passive_combatant(10_000_000.0);
    defender.damage = 100.0;
    ...
    assert!(
        heal_times.len() >= 2,
        "expected at least 2 self-heal events in 3 s window, got: {heal_times:?}"
    );
    assert!(
        (heal_times[0] - 1.0).abs() < 1e-9,
        "first Heal Breath self-heal must land at t=1.0 (one second of firing), got {}",
        heal_times[0]
    );
    for pair in heal_times.windows(2) {
        ...
        assert!(
            (gap - 1.0).abs() < 1e-9,
            "Heal Breath self-heal spacing must be 1.0 s, got {gap}: {heal_times:?}"
        );
    }
}
...
fn each_tick_heals_three_percent_max_hp() {
    // [REF:ability_heal_breath]
    // Bullet 5: "It heals the user for 3% of max HP per second while firing -
    // the self-heal runs on the 1-second regen loop, not on the 2-per-second
    // fire tick." Wound the attacker enough that the heal does not clamp at
    // full HP, then assert the per-tick `healing` field equals 3% of maxHP.
...
fn each_tick_removes_half_stack_of_negative_status() {
    // [REF:ability_heal_breath]
    // Bullet 6: "Each fire tick also removes 0.5 stacks of removable negative
    // statuses from the user (the cleanse stays on the fire tick)." Direct
    // unit test of `heal_simple_status_stacks` (called from the engine each
    // tick with `breath.cleanse_stacks` as the budget).
...
fn cleanse_order_is_poison_then_burn_then_bleed_then_corrosion() {
    // [REF:ability_heal_breath]
    // Bullet 7: "That cleanse works in a fixed order: Poison, Burn, Bleed,
    // then Corrosion."
