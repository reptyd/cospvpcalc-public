...
//!
//! Engine path: arming in `phases/phase4.rs` (Phase 4l) - arms
//! automatically as soon as `time >= spite_cooldown_until` and
//! `!spite_armed`, logging one "Spite activated" event. Charge ramp +
//! consume in `phases/melee.rs` (`charge_ratio = (time - activation) /
//! 5`, clamped 0..1; bonus = value x ratio): the next bite while
//! `spite_armed` clears the flag. The consume does NOT log a second
//! activation - the bite is tagged "Spite-charged" and its doubled
//! on-hit statuses log as "Spite applied <X>" (`spite_status_mult =
//! 2.0` while armed).
...
use crate::spec_constants::SPITE_COOLDOWN_SEC;
...
fn cooldown_twenty_seconds() {
    ...
    // re-arm cycle. Only arming logs "Spite activated" (the consume is
    // silent), so the activation events directly mark each arm. Still
    // group by unique timestamps for robustness against any same-tick
    // duplicate, then read the arming-cycle gap.
    ...
    assert!(
        (gap - SPITE_COOLDOWN_SEC).abs() < 1.0,
        "second Spite arming must land ~{SPITE_COOLDOWN_SEC} s after the first: gap={gap}, unique={unique_times:?}"
    );
}
...
fn fully_charged_bite_uses_one_plus_value_multiplier() {
    ...
    // Engine: `bonus_mult = 1.0 + value x clamp((time - arm_time)/5, 0..1)`.
    //
...
fn charged_hit_doubles_offensive_ailments() {
    ...
    //
    // Setup: attacker has on-hit Bleed x 1. First bite (consumes
    // Spite) applies Bleed x 2.
    ...
    // The doubled application is tagged as Spite-sourced so the timeline shows
    // which statuses the Spite bite buffed ("Spite applied Bleed", not "Bite
    // applied Bleed").
    let description = first_bleed_apply.description.as_deref().unwrap_or("");
    assert!(
        description.starts_with("Spite applied"),
        "Spite-doubled status must log a Spite-sourced application: description={description}"
    );
}

#[test]
fn consume_tags_the_bite_and_does_not_log_a_second_activation() {
    // [REF:ability_spite]
    // Bullet 4 (display): the consuming bite no longer logs a second "Spite
    // activated" event. Instead the bite carries the "Spite-charged" tag, so a
    // single fight cycle yields exactly ONE activation event (the arm) and a
    // tagged bite - not a duplicate marker at the bite.
    let mut a = melee_attacker(1_000_000.0, 50.0, 1.0);
    a.on_hit_statuses = vec![applied_status("Bleed_Status", 1.0)];
    let defender = passive_combatant(10_000_000.0);
    ...
    let result = simulate_composable_matchup_with_trace(
        ...
        SimpleAbilityTimingMode::ReallyFast,
        &spite_attacker_cfg(1.0),
        // Stop before the 20 s re-arm so exactly one cycle is in the trace.
        3.0, true,
    );
    let log = result.combat_log.expect("trace");
    ...
    assert_eq!(
        activation_count, 1,
        "one cycle must log exactly one Spite activation (arm only, consume is silent): {activation_count}"
    );
    ...
    assert!(
        ...
        "the bite that consumes Spite must be tagged 'Spite-charged'"
    );
}
