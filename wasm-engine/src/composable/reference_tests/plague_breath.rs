...
use crate::spec_constants::{PLAGUE_BREATH_CRIT_MULTIPLIER, PLAGUE_BREATH_PER_HIT_MULTIPLIER};
...
fn plague_breath_profile() -> SimpleBreathProfile {
    ...
    // Disease 100% x 1 stack = 1.0 deterministic per tick.
...
fn capacity_is_five_seconds_of_firing() {
    ...
    // fire spends 1 unit." 5 s x 2 ticks/s = 10 ticks before the burst
    // exhausts.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = plague_breath_profile();
...
fn damage_formula_matches_spec() {
    ...
    // 2) / 100) × 0.25 × 1.125 × (1 - breath resistance)."
    ...
    let expected = base
        * PLAGUE_BREATH_PER_HIT_MULTIPLIER
        * PLAGUE_BREATH_CRIT_MULTIPLIER
...
fn applies_disease_secondary_status() {
    // [REF:ability_plague_breath]
    // Bullet 5: "Every damage tick applies 1 stack of Disease - the
    // model does not roll Disease's 100% chance for 1 stack, it applies
    // the average of that roll." Verify Disease is applied during firing.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = plague_breath_profile();
    let result = simulate_composable_matchup_with_trace(
...
#[test]
fn each_tick_applies_the_disease_stacks_the_entry_states() {
    // [REF:ability_plague_breath]
    // "Every damage tick applies 1 stack of Disease".
    let applied = stacks_from_one_breath_tick(&plague_breath_profile());
    assert_tick_stacks(&applied, "Disease_Status", 1.0);
}
