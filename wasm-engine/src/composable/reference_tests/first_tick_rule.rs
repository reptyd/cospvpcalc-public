...
fn regen_half_overrides_first_regen_tick_to_compare_delay() {
    ...
    // Bullets 1 + 2 (regen branch): "First Tick Rule can apply to ailments,
    // regeneration, or both." + "When it is enabled, the first tick uses the
    // chosen delay instead of its normal starting time: 15 seconds for
    // regeneration, 3 seconds for a freshly applied ailment." Engine: with
    // attacker_compare_first_tick_regen=true and delay_sec=2, the first regen
    // tick fires at t=2 instead of t=15.
...
fn ailments_half_overrides_first_dot_tick_scheduling() {
    // [REF:compare_first_tick_rule]
    // Bullets 1 + 2 (ailments branch): with
    ...
    //
    // Setup: attacker takes a Burn x 5 stack on bite. With rule on
