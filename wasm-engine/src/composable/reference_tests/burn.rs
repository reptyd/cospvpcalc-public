...
//!   `compute_simple_dot_damage(_, "Burn_Status", stacks) =
...
use crate::spec_constants::{BURN_DOT_BASE_PCT_MAX_HP, BURN_DOT_PER_STACK_PCT_MAX_HP};
...
fn damage_formula_base_plus_per_stack() {
    ...
    let max_hp = 10_000.0;
    let one = compute_simple_dot_damage(max_hp, "Burn_Status", 1.0);
    let expected_one =
        max_hp * (BURN_DOT_BASE_PCT_MAX_HP + BURN_DOT_PER_STACK_PCT_MAX_HP) / 100.0;
    assert!(
        (one - expected_one).abs() < 1e-9,
        "1 Burn stack must deal {expected_one} (base + per-stack on 10000): got {one}"
    );
    let zero = compute_simple_dot_damage(max_hp, "Burn_Status", 0.0);
    let expected_zero = max_hp * BURN_DOT_BASE_PCT_MAX_HP / 100.0;
    assert!(
        (zero - expected_zero).abs() < 1e-9,
        "post-decay 0 Burn stacks must deal base only ({BURN_DOT_BASE_PCT_MAX_HP}% maxHP = {expected_zero} on 10000): got {zero}"
    );
    let ten = compute_simple_dot_damage(max_hp, "Burn_Status", 10.0);
    let expected_ten =
        max_hp * (BURN_DOT_BASE_PCT_MAX_HP + 10.0 * BURN_DOT_PER_STACK_PCT_MAX_HP) / 100.0;
    assert!(
        (ten - expected_ten).abs() < 1e-9,
        "10 Burn stacks must deal {expected_ten} (base + 10×per-stack on 10000): got {ten}"
    );
}
...
fn one_stack_moving_versus_stationary_five_x_ratio() {
    ...
    let max_hp = 10_000.0;
    let stationary = compute_simple_dot_damage(max_hp, "Burn_Status", 0.0);
    let moving = compute_simple_dot_damage(max_hp, "Burn_Status", 1.0);
...
fn ten_stack_moving_versus_stationary_gap_shrinks_to_about_one_point_one() {
    // [REF:status_burn]
    // Bullet 4 (tail): "The gap shrinks as stacks grow (1.108x at
    // ten stacks)."
    let max_hp = 10_000.0;
    let stationary = compute_simple_dot_damage(max_hp, "Burn_Status", 9.0);
    let moving = compute_simple_dot_damage(max_hp, "Burn_Status", 10.0);
    let ratio = moving / stationary;
    assert!(
        (ratio - 1.1).abs() < 0.02,
