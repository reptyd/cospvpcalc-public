...
//!   `compute_simple_dot_damage(_, "Hypothermia_Status", _) =
...
//!
//! The immunity bullet is a TS-side gate (`isWeatherImmune` in
//! `src/engine/weather.ts`); `src/engine/weather.test.ts` asserts it and
//! carries this entry's marker.

use super::super::config::ComposableAbilityConfig;
use super::super::simulate_composable_matchup_with_trace;
use super::default_combatant;
use crate::contracts::{SimpleAbilityTimingMode, SimpleAppliedStatus, SimpleCombatantStats};
use crate::spec_constants::HYPOTHERMIA_DOT_PCT_MAX_HP;
...
fn deals_zero_point_seven_five_percent_max_hp_every_three_seconds_stacks_independent() {
    ...
    let dmg_1 = compute_simple_dot_damage(10_000.0, "Hypothermia_Status", 1.0);
    let dmg_5 = compute_simple_dot_damage(10_000.0, "Hypothermia_Status", 5.0);
    let dmg_30 = compute_simple_dot_damage(10_000.0, "Hypothermia_Status", 30.0);
    let expected = 10_000.0 * HYPOTHERMIA_DOT_PCT_MAX_HP / 100.0;
    assert!(
        (dmg_1 - expected).abs() < 1e-9,
        "Hypothermia {HYPOTHERMIA_DOT_PCT_MAX_HP}% maxHP at any stacks ({expected} on 10000): got dmg_1={dmg_1}"
    );
    assert!(
        (dmg_5 - expected).abs() < 1e-9 && (dmg_30 - expected).abs() < 1e-9,
...
fn laying_nullifies_hypothermia_damage_any_source() {
    ...
    handle_simple_dot_ticks_full(
        0.0, 10_000.0, 0.0, &mut hp, &mut statuses, &mut dealt, false, None, 1.0, true, false, 1.0, 1.0,
    );
    assert!(
    ...
    let dot = 10_000.0 * HYPOTHERMIA_DOT_PCT_MAX_HP / 100.0;
    assert!(
        (hp - (10_000.0 - dot)).abs() < 1e-6,
        "standing must take {HYPOTHERMIA_DOT_PCT_MAX_HP}% maxHP ({dot} of 10000): hp={hp}"
    );
}
