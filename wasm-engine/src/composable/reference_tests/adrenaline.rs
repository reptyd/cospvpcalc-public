...
use crate::spec_constants::{ADRENALINE_COOLDOWN_SEC, ADRENALINE_DURATION_SEC};
...
fn lasts_thirty_seconds() {
    ...
    assert!(
        (duration - ADRENALINE_DURATION_SEC).abs() < 1e-9,
        "expected {ADRENALINE_DURATION_SEC} s duration, got {duration}"
    );
}
...
fn base_cooldown_ninety_seconds() {
    ...
    assert!(
        (cooldown - ADRENALINE_COOLDOWN_SEC).abs() < 1e-9,
        "expected {ADRENALINE_COOLDOWN_SEC} s base cooldown, got {cooldown}"
    );
}
...
fn bite_damage_multiplied_by_one_point_two_when_active() {
    ...
    // damage. damage_with - damage_without should equal 20 x M for some
