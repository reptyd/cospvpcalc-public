...
//! applies Burn x 1 + Fear x 1 to the target via the breath path.
...
use crate::spec_constants::{SPIRIT_GLARE_CRIT_MULTIPLIER, SPIRIT_GLARE_PER_HIT_MULTIPLIER};
...
fn spirit_glare_profile() -> SimpleBreathProfile {
    ...
    // Per-tick secondaries: Burn x 1 and Fear x 1.
...
fn damage_formula_matches_spec() {
    ...
    // 2) / 100) × 1.0 × 1.0 × (1 - breath resistance)."
    ...
    let expected = base
        * SPIRIT_GLARE_PER_HIT_MULTIPLIER
        * SPIRIT_GLARE_CRIT_MULTIPLIER
...
fn capacity_is_ten_seconds_and_fires_until_empty() {
    // [REF:ability_spirit_glare]
    // Bullet 3: "Capacity is 10 seconds of firing. Once Spirit Glare starts it
    // fires until the bar is empty." 60 s window: long enough for one full
    // burst (no startup delay + 10 s firing = 10 s) but well shorter than the
    // 120 s cooldown.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = spirit_glare_profile();
...
fn no_startup_delay() {
    // [REF:ability_spirit_glare]
    // Bullet 4: "It has no startup delay." First damage tick lands at t=0.5
    // (the 0.5 s tick interval) - not at t=3.5 (Solar Beam's 3 s startup) or
    // any later boundary.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = spirit_glare_profile();
...
fn one_hundred_twenty_second_cooldown() {
    // [REF:ability_spirit_glare]
    // Bullet 5: "It has a 120 second cooldown instead of a capacity
    // that refills."
    let attacker = passive_combatant(1_000.0);
...
fn each_damage_tick_applies_one_burn_and_one_fear() {
    // [REF:ability_spirit_glare]
    // Bullet 6: "Each damage tick also applies 1 stack of Burn and 1 stack of
    // Fear." The breath path applies `special_statuses` per tick. Verify both
...
fn each_tick_applies_the_stacks_the_entry_states() {
    ...
    // a full stack of each, not a fraction of one.
    let applied = stacks_from_one_breath_tick(&spirit_glare_profile());
    assert_tick_stacks(&applied, "Burn_Status", 1.0);
    assert_tick_stacks(&applied, "Fear_Status", 1.0);
}
