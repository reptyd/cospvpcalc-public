...
//! an incoming bite / breath tick lands. It is gated at both bite sides in
//! `phases/melee.rs` (dodge => the swing whiffs and advances one normal
//! cooldown) and in `breath.rs::fire_breath_damage`.
...
#[test]
fn a_creature_that_does_not_fly_dodges_on_the_same_chance() {
    // [REF:compare_aerial_dodge]
    ...
    let lands = (0..8)
        ...
        .count();
    assert_eq!(lands, 2, "25% lands 2 of 8 on a creature that does not fly: got {lands}");
}

#[test]
fn shredded_wings_leaves_a_creature_that_does_not_fly_dodging() {
    // [REF:compare_aerial_dodge]
    // "Shredded Wings stops a flying creature from dodging. A creature that
    // does not fly keeps its dodge."
    let mut stats = default_combatant();
