...
use crate::spec_constants::{SOLAR_BEAM_CRIT_MULTIPLIER, SOLAR_BEAM_PER_HIT_MULTIPLIER};
...
fn damage_formula_matches_spec() {
    // [REF:ability_solar_beam]
    // Bullets 2 + 3: "Breath damage per tick is calculated as (((target max HP
    // × (1 + min(attacker effective weight / defender effective weight, 3))) /
    // 2) / 100) × 1.5 × 1.0 × (1 - breath resistance)." Solar Beam DOES use
    // the standard weight + breath-resistance scaling (unlike Heliolyth's
    // Judgement which ignores them).
    ...
    let expected = base
        * SOLAR_BEAM_PER_HIT_MULTIPLIER
        * SOLAR_BEAM_CRIT_MULTIPLIER
...
fn capacity_is_ten_seconds_and_fires_until_empty() {
    // [REF:ability_solar_beam]
    // Bullet 3: "Capacity is 10 seconds of firing. Once Solar Beam starts it
    // fires until the bar is empty." The tick count alone cannot say this:
    // capacity drains per tick, so it stays at 20
    // however long those 20 ticks are spread over. The claim is seconds, so
    // measure seconds - the batch has to span the capacity from the first
    // tick's interval to the last.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = solar_beam_profile();
    ...
    let last = *times.last().expect("at least one tick");
    let interval = breath.capacity / times.len() as f64;
    assert!(
        (last - first + interval - breath.capacity).abs() < 1e-9,
        "capacity {} must buy {} seconds of firing, but the batch ran {first}..{last}",
        breath.capacity,
        breath.capacity
    );
    assert!(
        (first - (breath.auto_fire_delay_sec + interval)).abs() < 1e-9,
        "firing must start the moment the startup delay ends: first tick at {first}"
    );
}
...
fn three_second_startup_delay() {
    // [REF:ability_solar_beam]
    // Bullet 4: "It has a 3 second startup delay before firing begins."
    // Engine: auto_fire_delay_sec = 3.0. Plus the 0.5 s tick interval = first
    // tick lands at t=3.5.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = solar_beam_profile();
...
fn one_hundred_twenty_second_cooldown() {
    // [REF:ability_solar_beam]
    // Bullet 5b: "It has a 120 second cooldown instead of a capacity
    // that refills."
    ...
    // "A second batch happens sometime before 200 s" would hold for any
    // cooldown between roughly 90 and 180 s. The re-arm is deterministic,
    // so pin it: the cooldown runs from the arm at t=0, then the startup
    // delay and one tick interval before the batch's first damage tick.
    ...
    let interval = late[1] - late[0];
    let expected = breath.auto_fire_cooldown_sec + breath.auto_fire_delay_sec + interval;
    assert!(
        (late[20] - expected).abs() < 1e-9,
        "the second batch must open at cooldown {} + startup {} + one interval = {expected}, \
         got {}",
