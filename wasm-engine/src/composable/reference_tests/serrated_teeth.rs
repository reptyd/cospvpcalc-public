...
//! `on_hit_statuses` (Deep_Wounds_Status x 10) on the carrier
...
fn applies_ten_deep_wounds_on_direct_hit() {
    ...
    // Bullet 1: "Serrated Teeth applies 10 stacks of Deep Wounds when the user
    // lands a bite." Setup: A holds Serrated Teeth (modeled as Deep_Wounds x
    // 10 in on_hit_statuses). A bites B -> B receives a Deep_Wounds_Status
