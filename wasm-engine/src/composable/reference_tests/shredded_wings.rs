...
//!
//! Modeled: Shredded Wings grounds a flier so it can no longer dodge under the
//! Aerial Dodge rule - every incoming bite and breath then lands on it. With
//! the Aerial Dodge rule off the status has no separate combat effect. Test
//! body carries the [REF:status_shredded_wings] marker for the coverage gate.
...
#[test]
fn shredded_wings_grounds_a_flier_so_attacks_always_land() {
    // [REF:status_shredded_wings]
    // A flier at 0% hit chance would dodge everything, but Shredded Wings
    // removes the dodge, so every incoming bite and breath lands.
