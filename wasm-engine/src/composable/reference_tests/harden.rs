...
use crate::spec_constants::{HARDEN_COOLDOWN_SEC, HARDEN_DURATION_SEC, HARDEN_WEIGHT_BONUS_MULTIPLIER};
...
fn lasts_thirty_seconds_during_active_window() {
    ...
    let harden_active_until = HARDEN_DURATION_SEC;

    let inside = with_active_weight_bonuses(&attacker, 0.0, harden_active_until, 0.0, harden_active_until - 0.001);
    let boundary = with_active_weight_bonuses(&attacker, 0.0, harden_active_until, 0.0, harden_active_until);
    let after = with_active_weight_bonuses(&attacker, 0.0, harden_active_until, 0.0, harden_active_until + 0.001);

    assert!(
        (inside.weight - original_weight * HARDEN_WEIGHT_BONUS_MULTIPLIER).abs() < 1e-9,
        "Harden weight bonus must apply just before the 30 s boundary: expected {}, got {}",
        original_weight * HARDEN_WEIGHT_BONUS_MULTIPLIER,
        inside.weight,
    );
    assert!(
...
fn cooldown_one_hundred_twenty_seconds() {
    ...
    assert!(
        (gap - HARDEN_COOLDOWN_SEC).abs() < 1.0,
        "second Harden activation must land ~{HARDEN_COOLDOWN_SEC} s after the first, got {gap}: {activations:?}"
    );
}
...
fn weight_bonus_is_thirty_five_percent_during_active_window() {
    // [REF:ability_harden]
    // Bullet 3: "While Harden is active, the user's effective combat weight is
    // multiplied by 1.35." active_runtime::with_active_weight_bonuses applies
    // the 1.35 multiplier while time < harden_active_until.
    let attacker = passive_combatant(1_000.0);
    let original_weight = attacker.weight;
...
fn active_window_multiplies_passive_regen_by_one_point_two_five() {
    // [REF:ability_harden]
    // Bullet 4: "While Harden is active, passive health regeneration is
    // multiplied by 1.25." Pin HP at the t=15 regen tick to be identical
    // across both runs. Damage source is starting Poison_Status
    // (weight-independent and does not modify regen multiplier; Bleed_Status
    // is unsuitable because it disables regen entirely per
    // hp_regen_multiplier_from_statuses). Both sides have 0 melee damage and
    // very long bite cooldowns, so Harden's weight bonus does not change
    // incoming damage. With 20 Poison stacks the attacker has taken enough
    // damage by t=15 that the regen heal does not cap at max HP in either run,
    // isolating the 1.25x multiplier. Mirrors the regression test
