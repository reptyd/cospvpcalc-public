...
use crate::spec_constants::DROWSY_AREA_COOLDOWN_SEC;
...
fn cooldown_sixty_seconds() {
    ...
    assert!(
        (gap - DROWSY_AREA_COOLDOWN_SEC).abs() < 1e-6,
        "second Drowsy Area activation must be {DROWSY_AREA_COOLDOWN_SEC} s after the first, got {gap}: {times:?}"
    );
}
