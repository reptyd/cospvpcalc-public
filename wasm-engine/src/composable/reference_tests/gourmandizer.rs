...
fn dynamic_path_reads_current_hunger_when_rule_is_on() {
    // [REF:compare_gourmandizer]
    // Bullet 5: "With hunger rules on, the bonus updates from the current fill
    // as the meter drains." The dynamic helper takes (current_hunger,
    // appetite_base). With appetite_base=100 and current_hunger=125 ->
    // fill_pct=125% -> 1.15x. After drain to 110 -> fill_pct=110% -> 1.0 + 0.15 *
    // 0.4 = 1.06.
