...
//!
//! Engine path: `current_simple_bite_cooldown_with_statuses` reads
//! `Frostbite_Status` and adds
//! `FROSTBITE_BITE_COOLDOWN_INCREASE_PCT_PER_STACK/100 * effective_stacks` to
//! the multiplier, where `effective_stacks = ceil(remaining_sec / 3)`.

use super::default_combatant;
use crate::combat::current_simple_bite_cooldown_with_statuses;
use crate::contracts::SimpleStatusInstance;
use crate::spec_constants::FROSTBITE_BITE_COOLDOWN_INCREASE_PCT_PER_STACK;
...
fn increases_bite_cooldown_by_two_percent_per_stack() {
    ...
    // Bullet 1: "Frostbite increases bite cooldown by 2.5% per stack while it
    // is active."
    ...
    let expected_1 = 2.0 * (1.0 + FROSTBITE_BITE_COOLDOWN_INCREASE_PCT_PER_STACK / 100.0 * 1.0);
    assert!(
        (cd_1 - expected_1).abs() < 1e-9,
        "1 Frostbite stack must yield 2.0 × (1 + {FROSTBITE_BITE_COOLDOWN_INCREASE_PCT_PER_STACK}%×1) = {expected_1} s cooldown: got {cd_1}"
    );

    // 10 stacks -> +2% per stack x 10
    ...
    let expected_10 = 2.0 * (1.0 + FROSTBITE_BITE_COOLDOWN_INCREASE_PCT_PER_STACK / 100.0 * 10.0);
    assert!(
        (cd_10 - expected_10).abs() < 1e-9,
        "10 Frostbite stacks must yield 2.0 × (1 + {FROSTBITE_BITE_COOLDOWN_INCREASE_PCT_PER_STACK}%×10) = {expected_10} s cooldown: got {cd_10}"
    );
}
...
fn strength_scales_directly_with_stacks() {
    // [REF:status_frostbite]
    // Bullet 2: "Its strength scales directly with stacks, fractional ones
    // included: half a stack is +1.25%."
    let mut stats = default_combatant();
