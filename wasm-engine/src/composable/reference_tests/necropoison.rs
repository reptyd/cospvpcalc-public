...
fn does_not_disable_already_active_abilities_helper_reads_state_only() {
    // [REF:status_necropoison]
    // Bullet 1: "Necropoison blocks new active ability activations at 10
    // stacks and above." Engine: the `is_actives_disabled_by_necro` helper is
    // a pure status check, consulted only at *activation* call sites in
    // composable/mod.rs (Phase 4f-onwards) before flipping a `*_active_until`
    // field. Once an ability is active, its tick / ongoing behaviour does not
    // re-check this helper. The pure- helper invariant is the only meaningful
    // Rust assertion: the helper is stateless and reads `Necropoison_Status`
    // from the status map without inspecting any `*_active_until` field.
    let mut s: BTreeMap<String, SimpleStatusInstance> = BTreeMap::new();
