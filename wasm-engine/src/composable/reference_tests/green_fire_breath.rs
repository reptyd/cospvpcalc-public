//! Reference: ability_green_fire_breath
//!
//! Covers each testable bullet in the "Green Fire Breath" entry. Each
//! test body starts with the [REF:ability_green_fire_breath] marker so
...
use crate::spec_constants::{GREEN_FIRE_BREATH_CRIT_MULTIPLIER, GREEN_FIRE_BREATH_PER_HIT_MULTIPLIER};
use std::collections::BTreeMap;

fn green_fire_breath_profile() -> SimpleBreathProfile {
...
fn damage_formula_matches_spec() {
    ...
    let breath = green_fire_breath_profile();
    let mut chain = 0.0;
    ...
    let expected = base
        * GREEN_FIRE_BREATH_PER_HIT_MULTIPLIER
        * GREEN_FIRE_BREATH_CRIT_MULTIPLIER
        * (1.0 - defender.breath_resistance);
    assert!(
        (actual - expected).abs() < 1e-9,
        "Green Fire Breath per-tick damage mismatch: expected {expected}, got {actual}"
    );
}
...
fn applies_burn_secondary_status() {
    ...
    assert!(
        burn_present,
        "Green Fire Breath must apply Burn_Status as a secondary effect"
    );
}
...
fn each_tick_applies_the_burn_stacks_the_entry_states() {
    // [REF:ability_green_fire_breath]
    // "Every damage tick applies 0.375 stacks of Burn". The test above proves
    // the tick lands; this is how much of it lands.
    let applied = stacks_from_one_breath_tick(&green_fire_breath_profile());
