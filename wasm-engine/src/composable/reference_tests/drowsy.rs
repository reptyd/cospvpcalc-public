...
use crate::spec_constants::DROWSY_BITE_COOLDOWN_INCREASE_PCT;
...
fn increases_bite_cooldown_by_thirty_five_percent() {
    ...
    let expected = 2.0 * (1.0 + DROWSY_BITE_COOLDOWN_INCREASE_PCT / 100.0);
    assert!(
        (cd - expected).abs() < 1e-9,
        "Drowsy must add +{DROWSY_BITE_COOLDOWN_INCREASE_PCT}% to bite cooldown (2.0 × {} = {expected}): got {cd}",
        1.0 + DROWSY_BITE_COOLDOWN_INCREASE_PCT / 100.0
    );
}
...
fn strength_does_not_stack() {
    // [REF:status_drowsy]
    // Bullet 3: "The number of stacks only affects how long Drowsy lasts."
    let mut stats = default_combatant();
    stats.bite_cooldown = 2.0;
