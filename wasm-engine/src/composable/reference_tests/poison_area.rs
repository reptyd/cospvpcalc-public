...
//! activation applies Poison_Status x 5 to the opponent and re-arms
...
use crate::spec_constants::POISON_AREA_COOLDOWN_SEC;
...
fn applies_five_stacks_of_poison_to_opponent_on_activation() {
    ...
    // Bullet 2: "Each time it fires it applies 5 stacks of Poison to the
    // opponent."
...
fn cooldown_fifteen_seconds() {
    // [REF:compare_poison_area]
    // Bullet 3: "The ability has a 15 second cooldown."
    let attacker = passive_combatant(1_000_000.0);
    let defender = passive_combatant(10_000_000_000.0);
    let result = simulate_composable_matchup_with_trace(
        &attacker, &defender, None, None,
    ...
    assert!(
        (gap - POISON_AREA_COOLDOWN_SEC).abs() < 1.0,
        "second Poison Area activation must land ~{POISON_AREA_COOLDOWN_SEC} s after the first: gap={gap}, times={activations:?}"
    );
}
