...
use crate::spec_constants::{TOXIN_BREATH_CRIT_MULTIPLIER, TOXIN_BREATH_PER_HIT_MULTIPLIER};
...
fn toxin_breath_profile() -> SimpleBreathProfile {
    ...
    // tick. crit 5% -> x1.05 pseudo-crit. Poison 75% x 0.75 stacks
...
fn capacity_is_fifteen_seconds_of_firing() {
    // [REF:ability_toxin_breath]
    // Bullet 2: "Capacity is 15 seconds of firing - one second of continuous
    // fire spends 1 unit." Engine: capacity drains at 1 unit per second of
    // firing -> 15 s of continuous firing before exhaustion.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = toxin_breath_profile();
    let result = simulate_composable_matchup_with_trace(
...
fn damage_formula_matches_spec() {
    ...
    // 100) × 0.25 × 1.025 × (1 - breath resistance)."
    ...
    let expected = base
        * TOXIN_BREATH_PER_HIT_MULTIPLIER
        * TOXIN_BREATH_CRIT_MULTIPLIER
...
fn listed_secondary_is_poison_at_zero_point_five_six_two_five_per_tick() {
    // [REF:ability_toxin_breath]
    // Bullet 5: "Every damage tick applies 0.5625 stacks of Poison - the
    // model does not roll Poison's 75% chance for 0.75 stacks, it
    // applies the average of that roll." The TS bridge collapses
    // probability x stacks into a single deterministic apply per tick. Verify
    // (a) the breath profile carries Poison_Status x 0.5625 and (b) Poison DoT
    // actually lands on the defender during a sustained burn.
    let breath = toxin_breath_profile();
    ...
    assert!(
        poison_dots.len() >= 4,
        "Toxin Breath must produce Poison DoT ticks during a 15 s burst: {poison_dots:?}"
    );
    // That the DoT fires at all would hold for any payload size, including
    // one the engine rounded up to a whole stack. Poison's damage scales
    ...
    let per_window =
        (status_decay_sec("Poison_Status") / simple_breath_tick_sec(&breath)) * s.stacks - 1.0;
    for (k, (time, damage)) in poison_dots.iter().take(4).enumerate() {
        let expected_stacks = per_window * (k as f64 + 1.0);
        let expected =
            compute_simple_dot_damage(defender.health, "Poison_Status", expected_stacks);
        assert!(
            (damage - expected).abs() < 1e-6,
            "the Poison tick at t={time} must stand on {expected_stacks} stacks \
...
fn pseudo_crit_multiplier_is_one_point_zero_two_five() {
    ...
    // ratio must be exactly 1.025 (5% chance x 1.5x crit = 1 + 0.05x0.5).
    let mut attacker = default_combatant();
