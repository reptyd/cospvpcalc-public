...
//!   `compute_simple_dot_damage(_, "Heat_Wave_Status", _) =
...
//!   `Burn_Status x 2.0` which the caller re-applies to the target.
...
use crate::spec_constants::HEAT_WAVE_DOT_PCT_MAX_HP;
...
fn deals_one_percent_max_hp_every_three_seconds_stacks_independent() {
    ...
    let dmg_1 = compute_simple_dot_damage(10_000.0, "Heat_Wave_Status", 1.0);
    let dmg_5 = compute_simple_dot_damage(10_000.0, "Heat_Wave_Status", 5.0);
    let dmg_30 = compute_simple_dot_damage(10_000.0, "Heat_Wave_Status", 30.0);
    let expected = 10_000.0 * HEAT_WAVE_DOT_PCT_MAX_HP / 100.0;
    assert!(
        (dmg_1 - expected).abs() < 1e-9,
        "Heat Wave {HEAT_WAVE_DOT_PCT_MAX_HP}% maxHP at any stacks ({expected} on 10000): got dmg_1={dmg_1}"
    );
    assert!(
...
fn each_tick_applies_two_burn_stacks() {
    ...
    // to the same target."
    // Strategy: pre-seed attacker with Heat_Wave_Status x 5 (= 15 s
    // duration, ~5 ticks). Run 16 s. Attacker must accumulate Burn
    // DoT events (proof that Burn x 2 was applied each tick).
