...
use crate::spec_constants::CURSED_SIGIL_COOLDOWN_SEC;
...
fn cooldown_eighty_five_seconds() {
    ...
    assert!(
        (gap - CURSED_SIGIL_COOLDOWN_SEC).abs() < 1e-6,
        "second Cursed Sigil activation must be {CURSED_SIGIL_COOLDOWN_SEC} s after the first, got {gap}: {times:?}"
    );
}
