...
use super::super::sandbox::{SandboxAutomationMode, SandboxRuntime};
...
use crate::spec_constants::{
    REFLUX_COOLDOWN_SEC, REFLUX_PRIMARY_PCT_MAX_HP, REFLUX_SECONDARY_PCT_MAX_HP,
};
...
fn impact_deals_five_percent_target_max_hp() {
    ...
    // The puddle's first tick lands at impact too (the AOE ticks on
    // spawn), so isolate the impact via its own trace event rather than
    // total damage dealt.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000.0);
    ...
    let expected = defender.health * REFLUX_PRIMARY_PCT_MAX_HP / 100.0;
    assert!(
        (impact.damage - expected).abs() < 1e-6,
        "Reflux impact must deal {REFLUX_PRIMARY_PCT_MAX_HP}% of target maxHP: expected {expected}, got {}",
        impact.damage
    );
}
...
fn impact_applies_two_slowed() {
    // [REF:ability_reflux]
    // Bullet 3: "The impact also applies 2 stacks of Slowed to the target."
    // Engine: `apply_incoming_statuses_to_target_with_fortify_immunity`
    // applies `Slow_Status` x 2 right after the impact damage
    // (composable/mod.rs:3658-3666). The status apply itself does not push a
    // combat_log entry, but the engine emits a "Reflux impact" ability event
    // whose `detail` field documents the secondary Slow apply ("5% maxHP
    // direct hit + Slow 2"). That detail is the observable channel.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000.0);
    let result = simulate_composable_matchup_with_trace(
        &attacker, &defender, None, None,
...
#[test]
fn impact_applies_exactly_two_slowed_stacks() {
    // [REF:ability_reflux]
    // Bullet 3 on the count: "The impact also applies 2 stacks of Slowed to the
    // target." The detail string the test above reads is a literal the engine
    // writes out; the stacks are a second, separate literal, so the two can part
    // company. This reads the stacks off the target instead, one step past the
    // impact and before anything has decayed.
    let mut rt = SandboxRuntime::new(
        passive_combatant(1_000.0),
        passive_combatant(10_000.0),
        None,
        None,
        reflux_attacker_cfg(),
        SimpleAbilityTimingMode::ReallyFast,
    ...
    rt.step_to_time(5.001);
    let slowed = rt
        .snapshot_view()
        .side_b
        .statuses
        .iter()
        .find(|s| s.id == "Slow_Status")
        .map(|s| s.stacks)
        .unwrap_or(0.0);
    assert!(
        (slowed - 2.0).abs() < 1e-9,
        "the Reflux impact must apply 2 stacks of Slowed, got {slowed}"
    );
}
...
fn puddle_lasts_ten_seconds_after_impact() {
    // [REF:ability_reflux]
    // Bullet 4: "After the impact, Reflux leaves a puddle for 10 seconds."
    // Engine: `reflux_puddle_until = time + 10.0` at impact (t=5). Puddle
    // expires at t=15. Verify via the trace: NO "Reflux puddle tick" events at
    // t > 15.
    //
...
fn puddle_ticks_once_per_second() {
    // [REF:ability_reflux]
    // Bullet 5: "The puddle ticks once per second." Game AOETickCount=10: the
    // AOE ticks on spawn, so the first tick lands at impact (t=5) and the
    // handler schedules the rest at +1 s while `puddle_until > time` (strict).
    // With puddle_until=15, ticks land at t=5, 6, ..., 14 - 10 ticks total.
    // The boundary tick at t=15 is gated out (puddle_until > 15 is false).
    let attacker = passive_combatant(1_000.0);
    ...
    assert_eq!(
        puddle_ticks.len(),
        10,
        "Reflux puddle must produce 10 ticks (1 Hz, t=5..14, AOETickCount=10): {puddle_ticks:?}"
    );
...
fn each_puddle_tick_deals_one_point_five_percent_max_hp_and_applies_half_corrosion() {
    // [REF:ability_reflux]
    // Bullet 6: "Each puddle tick deals direct damage equal to 1.5% of the
    // target's max HP and applies 0.5 stacks of Corrosion." Engine:
    // `puddle_damage = defender.health * 0.015` per tick;
    // `apply_incoming_statuses_to_target_with_fortify_immunity` adds 0.5
    // stacks of Corrosion_Status per tick.
    //
    ...
    // applied_puddle = defender.health x 0.015 (clamped by remaining
    ...
    let expected_per_tick = defender.health * REFLUX_SECONDARY_PCT_MAX_HP / 100.0;
    for tick in &puddle_ticks {
        assert!(
            (tick.damage - expected_per_tick).abs() < 1e-6,
            "each puddle tick must deal {REFLUX_SECONDARY_PCT_MAX_HP}% maxHP = {expected_per_tick}: got {}",
            tick.damage
        );
    }
...
fn cooldown_one_hundred_twenty_seconds() {
    // [REF:ability_reflux]
    // Bullet 8: "It has a 120 second cooldown." Engine: `reflux_cooldown_until
    // = time + 120.0` set at impact time (t=5). Re-arming gated until t=125;
    // second impact at t=130; second puddle ends at t=140.
    let attacker = passive_combatant(1_000.0);
    ...
    assert!(
        (gap - (5.0 + REFLUX_COOLDOWN_SEC)).abs() < 1.0,
        "second Reflux arm must land ~{} s after the first (5 s charge + {REFLUX_COOLDOWN_SEC} s cooldown): \
         got {gap}: {charge_starts:?}",
        5.0 + REFLUX_COOLDOWN_SEC
    );
}
