...
use crate::spec_constants::LICH_MARK_COOLDOWN_SEC;
...
fn payload_can_be_any_documented_id() {
    // [REF:ability_lich_mark]
    // Bullet 5: "The payloads are Blessing's Boon, Malice's Mark, Slowed,
    // Drowsy, Necropoison, Poison, Bad Omen, Water Regeneration, Flowering,
    // Broken Bones, Stolen Speed, Blurred Vision, and Gale." Engine:
    // `payload_status_id` is `Option<String>` set per side via config. Any
    // well-formed status id flows through `convert_lich_mark_pending`. Verify
    // with a second documented payload (Necropoison_Status) - different from
    // the default Bad_Omen used by the other tests.
    let attacker_stats = melee_attacker(1_000.0, 50.0, 1.0);
    let defender_stats = passive_combatant(10_000.0);
    let mut attacker = CombatSide::new(&attacker_stats, None);
    let mut defender = CombatSide::new(&defender_stats, None);
...
fn cooldown_thirty_seconds() {
    ...
    assert!(
        (gap - LICH_MARK_COOLDOWN_SEC).abs() < 1.0,
        "second Lich Mark activation must land ~{LICH_MARK_COOLDOWN_SEC} s after the first, got {gap}: {activations:?}"
    );
}
