...
#[test]
fn the_activation_is_logged_when_it_comes_on_not_on_its_first_tick() {
    // [REF:ability_aura]
    // The aura is an active ability its owner keeps on, so a fight it walks in
    // with starts with it running. The log entry used to be written inside the
    // first tick, three seconds later, which read as "activated at 3s".
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000.0);
    let cfg = aura_attacker_config();
    let result = simulate_composable_matchup_with_trace(
    ...
    let activation = log
        .iter()
        .find(|entry| {
            entry.description.as_deref() == Some("Aura (Corrosion) activated")
        })
        .expect("the aura announces itself");
    assert!(
        activation.time.abs() < 1e-9,
        "the aura is on from the start, so its activation belongs at 0 s, got {}",
        activation.time,
    );
}
