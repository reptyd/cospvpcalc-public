...
fn fixed_cooldown_overrides_status_modifiers() {
    ...
    // Bullet 3: "That fixed cooldown overrides normal bite-cooldown changes
    // from statuses and traits." Engine: with `compare_air_rule_cooldown_sec =
    // 1.5`, even with Sticky_Teeth_Status (+65% cooldown) and Drowsy_Status
    // (+35%) present, the helper returns 1.5 unchanged.
