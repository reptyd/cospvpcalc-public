...
//!
//! Covers each testable bullet in the "Fixed Bite Cadence" entry.
...
//!
//! Engine path: `current_simple_bite_cooldown_with_statuses` in `combat.rs` -
//! when `stats.compare_air_rule_cooldown_sec > 0.0` the helper returns
//! `max(0.1, this)` and returns before every modifier below it.
...
fn fixed_cooldown_overrides_status_modifiers() {
    ...
    // Bullet 4: "A fixed cadence is not changed by any status or ability that
    // would otherwise move the bite cooldown." Engine: with
    // `compare_air_rule_cooldown_sec = 1.5`, even with Sticky_Teeth_Status
    // (+65% cooldown) and Drowsy_Status (+35%) present, the helper returns 1.5
    // unchanged.
    ...
    assert!(
        (cd - 1.5).abs() < 1e-12,
        "a fixed cadence must lock bite cooldown at 1.5 regardless of statuses: got {cd}"
    );
}
...
fn disabled_zero_cooldown_falls_back_to_normal_calc() {
    // [REF:compare_special_air_pvp_rule]
    // Bullet 3: "A side can instead keep its own bite cooldown while the other
    // side is on a fixed cadence." Engine: with
    // `compare_air_rule_cooldown_sec = 0.0`, the helper applies status
    // multipliers normally - Sticky_Teeth gives +65% (multiplier 1.65), so a
    // 2.0 s base cooldown becomes 3.3 s.
