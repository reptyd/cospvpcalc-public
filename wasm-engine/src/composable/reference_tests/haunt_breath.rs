...
use crate::spec_constants::{HAUNT_BREATH_CRIT_MULTIPLIER, HAUNT_BREATH_PER_HIT_MULTIPLIER};
...
fn haunt_breath_profile() -> SimpleBreathProfile {
    ...
    // Poison 75% x 1 stack, Shock 10% x 0.5 and Tunnel Vision 25% x 0.5,
    // as expected stacks per tick.
    breath.special_statuses = vec![
        applied_status("Poison_Status", 0.75),
        applied_status("Shock_Status", 0.05),
        applied_status("Tunnel_Vision_Status", 0.125),
    ];
    breath
}
...
fn capacity_is_ten_seconds_of_firing() {
    ...
    // exactly 20 ticks before the burst exhausts and waits for regen.
    let attacker = passive_combatant(1_000.0);
    let defender = passive_combatant(10_000_000.0);
    let breath = haunt_breath_profile();
    let result = simulate_composable_matchup_with_trace(
...
fn damage_formula_matches_spec() {
    ...
    // 2) / 100) × 0.75 × 1.175 × (1 - breath resistance)." The 0.75 is the
    // per-tick percent (dps 1.5 / 2 ticks/sec); 1.35 is the 35% pseudo-crit.
    // Encoded as dps_pct=1.5, crit_chance_pct=35.
    ...
    let expected = base
        * HAUNT_BREATH_PER_HIT_MULTIPLIER
        * HAUNT_BREATH_CRIT_MULTIPLIER
...
#[test]
fn applies_all_three_secondary_statuses() {
    // [REF:ability_haunt_breath]
    // Bullet 5: "Every damage tick applies 0.75 stacks of Poison, 0.05
    // stacks of Shock and 0.125 stacks of Tunnel Vision - the model does
    // not roll Poison's 75% chance for 1 stack, Shock's 10% chance for
    // 0.5 stacks, or Tunnel Vision's 25% chance for 0.5 stacks, it
    ...
    assert!(
        shock_present,
        "Haunt Breath must apply Shock_Status as a secondary effect"
    );
    ...
    assert!(
        tunnel_present,
        "Haunt Breath must apply Tunnel_Vision_Status as a secondary effect"
    );
}
...
fn each_tick_applies_the_stacks_the_entry_states() {
    // [REF:ability_haunt_breath]
    // "Every damage tick applies 0.75 stacks of Poison, 0.05 stacks of Shock
    // and 0.125 stacks of Tunnel Vision". The three sizes are two orders of
    // magnitude apart, so presence alone says almost nothing about them.
    let applied = stacks_from_one_breath_tick(&haunt_breath_profile());
    assert_tick_stacks(&applied, "Poison_Status", 0.75);
    assert_tick_stacks(&applied, "Shock_Status", 0.05);
    assert_tick_stacks(&applied, "Tunnel_Vision_Status", 0.125);
}
