...
fn offensive_application_on_bite_applies_torn_ligaments() {
    ...
    // Bullet 1: "On an offensive carrier, Ligament Tear applies Torn Ligaments
    // to the target when the user lands a bite." Plus bullet 4: "Ligament Tear
    // applies Torn Ligaments." Setup: A holds Ligament Tear as offensive
    // (on_hit_statuses with Torn_Ligaments). A bites B -> B receives
    // Torn_Ligaments.
...
fn defensive_application_on_being_bitten_applies_torn_ligaments() {
    // [REF:ability_ligament_tear]
    // Bullet 3: "On a defensive carrier it runs the other way, applying Torn
    // Ligaments to the attacker when the user is bitten." Setup: A holds
    // Ligament Tear as defensive (on_hit_taken_statuses with Torn_Ligaments).
    // When B bites A, the engine applies Torn_Ligaments to B (the biter).
...
fn breath_does_not_trigger_ligament_tear() {
    // [REF:ability_ligament_tear]
    // Bullet 2: "Breath does not trigger Ligament Tear." The engine's
    // CombatSide initialises `next_hit = 0.0`, so a bite event fires at t=0
    // even with `bite_cooldown = 1000`. With `damage = 0` that first bite
    // contributes no damage, but `on_hit_statuses` still apply (the engine
    // consumes them on every bite event regardless of damage). With a long
    // bite_cooldown, only that single t=0 bite fires within a 5 s window, so
    // the Torn_Ligaments apply count must equal exactly 1 - proving the breath
    // path does NOT add additional applies.
    let mut a = passive_combatant(10_000.0);
    a.on_hit_statuses = vec![applied_status(TORN_LIGAMENTS_STATUS_ID, 1.0)];
