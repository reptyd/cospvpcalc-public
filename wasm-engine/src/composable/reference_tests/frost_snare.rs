...
use crate::spec_constants::FROST_SNARE_COOLDOWN_SEC;
...
fn cooldown_two_hundred_five_seconds() {
    ...
    assert!(
        (gap - FROST_SNARE_COOLDOWN_SEC).abs() < 1e-6,
        "second Frost Snare activation must be {FROST_SNARE_COOLDOWN_SEC} s after the first, got {gap}: {times:?}"
    );
}
