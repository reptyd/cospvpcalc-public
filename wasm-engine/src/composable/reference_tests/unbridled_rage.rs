...
use crate::spec_constants::{UNBRIDLED_RAGE_COOLDOWN_SEC, UNBRIDLED_RAGE_DURATION_SEC};
...
fn lasts_thirty_seconds() {
    ...
    // exceed baseline (x1.3). Use damage totals at t=15 as proxy.
    ...
    // `unbridled_rage_active_until = activation_time + 30.0`
    // (phases/phase4.rs) and emits "Unbridled Rage deactivated" at the
    // first loop event at/after the window end. The 0.5 s bite cadence
    // keeps events flowing so the close lands within a sub-second of the
    // window end. Measure deactivation minus activation so the planned-
    // fire arm delay does not bias the window length.
    let timed = simulate_composable_matchup_with_trace(
        &attacker, &defender, None, None,
        SimpleAbilityTimingMode::ReallyFast,
        &ur_attacker_cfg(),
        60.0, true,
    );
    let timed_log = timed.combat_log.expect("trace");
    let event_time = |desc: &str| -> f64 {
        timed_log
            .iter()
            .find(|e| e.description.as_deref() == Some(desc))
            .unwrap_or_else(|| panic!("missing event: {desc}"))
            .time
    };
    let window = event_time("Unbridled Rage deactivated") - event_time("Unbridled Rage activated");
    assert!(
        (window - UNBRIDLED_RAGE_DURATION_SEC).abs() < 0.6,
        "Unbridled Rage active window must last ~{UNBRIDLED_RAGE_DURATION_SEC} s, got {window}"
    );
}
...
fn cooldown_one_hundred_twenty_seconds() {
    ...
    assert!(
        (gap - UNBRIDLED_RAGE_COOLDOWN_SEC).abs() < 1.0,
        "second Unbridled Rage activation must land ~{UNBRIDLED_RAGE_COOLDOWN_SEC} s after the first: gap={gap}, times={activations:?}"
    );
}
...
fn while_active_bite_damage_is_multiplied_by_one_point_three() {
    // [REF:ability_unbridled_rage]
    // Bullet 3: "While Unbridled Rage is active, the user's melee damage is
    // multiplied by 1.3." Verify by reading the per-bite damage during the
    // active window versus a baseline run with UR off.
...
fn does_not_increase_breath_damage() {
    // [REF:ability_unbridled_rage]
    // Bullet 4: "It boosts melee damage only and does not increase breath
    // damage." Engine: `melee_multiplier_a *= 1.3` is in the melee branch
    // only; the breath path (`compute_simple_breath_damage_*`) does not read
    // any `unbridled_rage_active_until`. Verify per-tick breath damage is
    // identical with and without UR active.
