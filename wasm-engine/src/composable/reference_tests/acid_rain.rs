...
//!   `compute_simple_dot_damage(_, "Acid_Rain_Status", _) =
...
//!   `Poison_Status x 2.0` which the caller re-applies to the target.
...
use crate::spec_constants::ACID_RAIN_DOT_PCT_MAX_HP;
...
fn deals_three_percent_max_hp_every_three_seconds_stacks_independent() {
    ...
    let dmg_1 = compute_simple_dot_damage(10_000.0, "Acid_Rain_Status", 1.0);
    let dmg_5 = compute_simple_dot_damage(10_000.0, "Acid_Rain_Status", 5.0);
    let dmg_30 = compute_simple_dot_damage(10_000.0, "Acid_Rain_Status", 30.0);
    let expected = 10_000.0 * ACID_RAIN_DOT_PCT_MAX_HP / 100.0;
    assert!(
        (dmg_1 - expected).abs() < 1e-9,
        "Acid Rain {ACID_RAIN_DOT_PCT_MAX_HP}% maxHP at any stacks ({expected} on 10000): got dmg_1={dmg_1}"
    );
    assert!(
...
fn weather_acid_rain_is_a_single_permanent_stack() {
    // [REF:status_acid_rain]
    // Bullet 5 + note: "As a weather effect it is a single permanent stack for
    // the whole fight." A normal 1-stack DoT would decay after one 3 s
    // interval; the weather stack must keep ticking far beyond that. Late
    // ticks (after several decay windows) prove permanence.
    let attacker = passive_combatant(10_000_000.0);
    let defender = passive_combatant(10_000_000.0);
    let result = simulate_composable_matchup_with_trace(
        &attacker,
