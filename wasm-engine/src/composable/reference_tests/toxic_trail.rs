...
fn gated_by_trails_compare_only_toggle() {
    ...
    // Bullet 1: "In Compare, Toxic Trail runs only with the Trails rule turned
    // on for the user."
...
fn does_not_apply_above_hp_threshold() {
    // [REF:ability_toxic_trail]
    // Bullet 2: "Toxic Trail activates while the user's current HP is at or
...
fn segment_is_eternal_while_threshold_holds() {
    ...
    // Always-active trail ticks every DAMAGE_TRAIL_TICK_SEC=2 s for 2% maxHP
    // each tick. First tick at t=2, so a 7 s window observes ticks at t=2, 4,
    // 6 = 3 ticks = 6%. The >=5% bound proves the segment kept ticking (a
    // single despawn would leave <=4%).
    let attacker = passive_combatant(1_000.0);
    ...
    assert!(
        lost_pct >= 0.05,
        "Toxic Trail segment must keep ticking for the full window: defender lost {:.2}% maxHP (need >=5%)",
        lost_pct * 100.0
    );
}
