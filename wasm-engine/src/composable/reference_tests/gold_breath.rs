...
use crate::spec_constants::{GOLD_BREATH_CRIT_MULTIPLIER, GOLD_BREATH_PER_HIT_MULTIPLIER};
use std::collections::BTreeMap;

fn gold_breath_profile() -> SimpleBreathProfile {
    let mut breath = default_breath();
    // Per Reference: per-tick = 0.5 x 0.25. Encoded as dps_pct=0.5
    // (0.5 x 0.5 = 0.25), crit_chance_pct=0.
    breath.dps_pct = 0.5;
    breath.capacity = 20.0;
    breath.regen_rate = 1.8;
    breath.crit_chance_pct = 0.0;
    // Blurred Vision 30% and Freeze 5%, both single-stack, as expected
    // stacks per tick.
    breath.special_statuses = vec![
        applied_status("Blurred_Vision_Status", 0.3),
...
fn damage_formula_matches_spec() {
    ...
    // dps_pct(0.5) x the universal per-tick halving = the spec's 0.25
    // per-hit multiplier; crit factor is 1.0 (0% pseudo-crit).
    let expected = base
        * GOLD_BREATH_PER_HIT_MULTIPLIER
        * GOLD_BREATH_CRIT_MULTIPLIER
...
fn each_tick_applies_the_stacks_the_entry_states() {
    // [REF:ability_gold_breath]
    // "Every damage tick applies 0.3 stacks of Blurred Vision and 0.05 stacks
    // of Freeze".
    let applied = stacks_from_one_breath_tick(&gold_breath_profile());
    assert_tick_stacks(&applied, "Blurred_Vision_Status", 0.3);
