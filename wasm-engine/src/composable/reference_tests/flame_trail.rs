...
fn applies_burn_to_opponent_below_threshold() {
    ...
    assert!(
        ...
        "Flame Trail must deal damage to the opponent (2% of max HP per 2 s tick)"
    );
}

#[test]
fn override_flips_no_move_facetank_off_while_trail_active() {
    // [REF:ability_flame_trail] [REF:compare_trails]
    // Bullets 8 + 9: "While any of the user's trail abilities is active, No
    // Move Facetank is forced off." + "The previous No Move Facetank setting
    // returns as soon as no trail is active."
    //
