...
//! the owner is bitten) carrying Shredded_Wings x value. The bite
...
fn offensive_application_on_bite_applies_shredded_wings() {
    ...
    // Bullet 1: "On an offensive carrier, Wing Shredder applies Shredded Wings
    // to the target when the user lands a bite." Plus bullet 4: "Wing Shredder
    // applies Shredded Wings." Setup: A holds Wing Shredder as offensive
    // (`on_hit_statuses` with Shredded_Wings x value). A bites B -> B receives
    // Shredded_Wings.
...
fn defensive_application_on_being_bitten_applies_shredded_wings() {
    // [REF:ability_wing_shredder]
    // Bullet 3: "On a defensive carrier it runs the other way, applying
    // Shredded Wings to the attacker when the user is bitten." Setup: A holds
    // Wing Shredder as defensive (`on_hit_taken_statuses` with Shredded_Wings
    // x value). When B bites A, the engine applies Shredded_Wings to B (the
    // biter).
...
fn breath_does_not_trigger_wing_shredder() {
    // [REF:ability_wing_shredder]
    // Bullet 2: "Breath does not trigger Wing Shredder." The engine's
    ...
    // still consumes `on_hit_statuses`. With a long bite_cooldown, only that
    // single t=0 bite fires within a 5 s window, so the Shredded_Wings apply
    // count must equal exactly 1 - proving the breath path does NOT add
    // additional applies.
    let mut a = passive_combatant(10_000.0);
    a.on_hit_statuses = vec![applied_status(SHREDDED_WINGS_STATUS_ID, 2.0)];
