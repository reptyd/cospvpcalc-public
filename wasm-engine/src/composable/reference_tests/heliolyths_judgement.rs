...
use crate::spec_constants::HELIOLYTHS_JUDGEMENT_PCT_MAX_HP;
...
fn per_tick_damage_is_one_point_six_percent_of_max_hp() {
    ...
    let expected = max_hp - 20.0 * (HELIOLYTHS_JUDGEMENT_PCT_MAX_HP / 100.0) * max_hp;
    assert!(
...
fn one_hundred_twenty_second_cooldown() {
    // [REF:ability_heliolyths_judgement]
    let attacker = passive_combatant(1_000.0);
    // Defender HP large enough to survive two full batches (32% x 2 = 64%).
