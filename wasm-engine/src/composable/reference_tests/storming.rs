...
use crate::spec_constants::STORMING_INCOMING_DAMAGE_INCREASE_PCT;
...
fn storming_status_contributes_ten_percent_incoming() {
    ...
    assert!(
        (incoming_damage_pct_from_statuses(&storming_statuses()) - STORMING_INCOMING_DAMAGE_INCREASE_PCT).abs() < 1e-9,
        "Storming_Status must contribute +{STORMING_INCOMING_DAMAGE_INCREASE_PCT}% incoming damage"
    );
    assert!(
...
fn storming_amplifies_bite_damage_by_ten_percent() {
    ...
    let amp = 1.0 + STORMING_INCOMING_DAMAGE_INCREASE_PCT / 100.0;
    assert!(
        base > 0.0 && (with_storming - base * amp).abs() < 1e-6,
        "Storming must amplify bite damage by exactly {STORMING_INCOMING_DAMAGE_INCREASE_PCT}%: base={base}, with_storming={with_storming}"
    );
}
...
fn storming_amplifies_breath_damage_by_ten_percent() {
    ...
    assert!(
        base > 0.0 && (with_storming - base * amp).abs() < 1e-6,
        "Storming must amplify breath damage by exactly {STORMING_INCOMING_DAMAGE_INCREASE_PCT}%: base={base}, with_storming={with_storming}"
    );
}
