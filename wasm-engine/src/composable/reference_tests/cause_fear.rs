...
use crate::spec_constants::CAUSE_FEAR_COOLDOWN_SEC;
...
fn cooldown_one_hundred_twenty_seconds() {
    ...
    assert!(
        (gap - CAUSE_FEAR_COOLDOWN_SEC).abs() < 1e-6,
        "second Cause Fear activation must be {CAUSE_FEAR_COOLDOWN_SEC} s after the first, got {gap}: {times:?}"
    );
}
