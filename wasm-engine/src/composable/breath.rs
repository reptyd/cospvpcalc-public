...
/// Record that `actor` fired one breath tick at `time`. Called from every
/// path that spends a tick, including ones that land nothing on the target,
/// so the firing time Compare reports is the time the breath was actually
/// running rather than the time it happened to produce a log entry.
fn record_breath_fire(actor: &mut CombatSide, time: f64, record_trace: bool) {
    if record_trace {
        actor.breath_fire_times.push(time);
    }
}
...
fn tick_breath_lance(
    ...
    if actor.lance_armed_until <= 0.0 {
        ...
        record_breath_fire(actor, time, record_trace);
...
fn fire_breath_damage(
    ...
    // Dodge Chance: the target can move clear of a breath tick. On a dodge the
