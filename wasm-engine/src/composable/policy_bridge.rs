...
//! `composable::CombatSide` (narrow public API surface).
...
use crate::policy::decisions::{
    ...
    rewind::RESTORED_HP_DELTA_KEY,
    wardens_rage::CURRENT_STATE_EXTRA_KEY as WR_ON_KEY,
    AdrenalineDecision, CocoonDecision, FortifyDecision, GuardiansPassageDecision, HunkerDecision,
    HuntersCurseDecision, LifeLeechDecision, ReflectDecision, RewindDecision, UnbridledRageDecision,
    WardensRageDecision,
};
...
fn decision_registry() -> &'static DecisionRegistry {
    ...
    REG.get_or_init(|| {
        ...
        reg.register(Box::new(GuardiansPassageDecision::new()));
...
pub fn build_policy_side(
    ...
    cooldowns.insert(
        decisions::guardians_passage::GUARDIANS_PASSAGE_DECISION_ID.to_string(),
        side.guardians_passage_cooldown_until,
    );
    cooldowns.insert(
    ...
    // Merge per-side user-ability state. Built-in-only
    ...
    let mut policy_side = PolicySide {
        ...
        // Surface the committed posture as a label so
        ...
        // Snapshot the sliding-window logs so
...
/// Extras builder for the Rewind decision's snapshot input.
/// `restored_hp_delta` is the *post-cap* HP gain Rewind would produce if fired
/// now (can be negative if the snapshot HP is below current HP). Game Rewind
/// restores HP only, so HP is the sole input - there is no status term.
pub fn rewind_extras(restored_hp_delta: f64) -> [(&'static str, PolicyValue); 1] {
...
pub fn user_should_activate_now(
    ...
    time: f64,
    mode: TimingMode,
    // scaling.<key> entries are injected here so
    // `Expr::Var { path: "scaling.<key>" }` resolves during the
    // decision (utility / is_available) eval. Empty map => no
    // scaling entries visible (legacy single-level behavior).
