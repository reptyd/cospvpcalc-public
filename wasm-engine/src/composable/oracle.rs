...
mod tests {
    ...
    /// counted by the `#[cfg(test)]` probe in `run_one_event_loop_iter`. That
    /// probe also counts the iterations inside the policy's inner replays,
    /// which the `bench_count` counter does not. Reports, at a long-running
    /// near-tie point, the ratio of (live policy on) to (no policy) iterations:
    /// the policy's compute multiplier.
    ///
