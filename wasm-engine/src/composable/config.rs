...
pub struct ComposableAbilityConfig {
    ...
    /// Compare-only Dodge Chance rule master switch. When set, each incoming
    /// bite / breath tick on a side rolls against that side's
    /// `aerial_dodge_hit_chance_pct` (100 = always hit; Shredded Wings forces
    /// 100 on a flying side). Off (default) is inert -> byte-identical.
    #[serde(default)]
    pub aerial_dodge_active: bool,
    /// Roll style for the dodge. false (default) = the deterministic even
