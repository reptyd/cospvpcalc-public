...
pub struct CombatSide {
    ...
    /// Clock time of every breath tick this side actually fired, recorded
    /// only under `record_trace`. A fired tick covers `runtime_breath_tick_sec`
    /// of firing whether or not it produced damage or healing, which the
    /// combat log cannot show: a heal breath heals once per second but fires
    /// twice, and a fully dodged or fully absorbed damage tick writes no
    /// damage entry either. Compare's Breath Time reads these.
