//! User-ability dispatch into the live combat loop.
//!
...
//! `on_tick`) wire in against the same dispatcher shape - see the
//! on_* trigger handlers below.
...
/// Build the `scaling.<key>` extras snapshot for a
/// spec at its effective active level. Returns an empty map if the
/// spec has no scaling (legacy single-level specs and level-1-only specs).
///
...
/// Resolve the effective active level for a spec on
...
pub fn drain_due_scheduled_effects_for_caster(
    ...
    // Drain due entries; push back not-yet-due.
...
/// Drain side-extras requests written by custom-ability effects that
...
pub fn dispatch_user_actives_for_caster(
    ...
    for id in &ids {
        ...
        // scaling.<key> entries injected into both the
        // decision PolicyState (here) and the apply PolicyState (below).
        // Active level honors the per-fight Compare
        ...
        let apply_state = PolicyState {
            ...
            time,
            // scaling visible inside Conditional / Expr-
        ...
        let mut ctx = EffectContext {
            ...
            // Heal accumulators for OnHeal trigger.
            ...
            chain_depth: 0,
            // Stamp source onto ModifyStat keys so two
...
/// Fire any due `on_tick` triggers for the caster side.
...
/// Fire each attached user ability's `on_round_start`
...
///
/// `firing_ability_id` is stamped onto `ModifyStat` keys so two
/// abilities don't collide on the same modifier slot.
...
/// The user half of the defender chain, called from
/// `damage_pipeline::resolve_incoming_damage`. Fires
/// `on_before_deal_damage` on the dealer THEN `on_before_take_damage`
/// on the victim, then returns the damage amount left for the built-in
/// reductions.
///
...
/// already exist so users can branch on damage kind there.
#[allow(clippy::too_many_arguments)]
pub fn run_user_pre_damage_hooks(
