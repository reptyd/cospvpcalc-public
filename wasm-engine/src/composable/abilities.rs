...
use super::status_helpers::{apply_status_delta, apply_statuses_with_trace};
...
fn resolve_yolk_bomb_routing(value: Option<&str>) -> YolkBombRouting<'_> {
    // 2026-05-18: expanded from a 15-entry hardcoded match to the
...
pub(super) fn apply_yolk_bomb(
    ...
    let slow = SimpleAppliedStatus {
        ...
        stacks: YOLK_BOMB_SLOW_STACKS, ..Default::default() };
    match routing {
        YolkBombRouting::SelfFortify => {
            ...
            apply_statuses_with_trace(
                ...
                source_label, attacker_hp, source_label, "Yolk Bomb",
                combat_log,
            );
        }
        YolkBombRouting::SelfStatus(status_id) => {
            let statuses = [
                SimpleAppliedStatus {
                    ...
                    stacks: YOLK_BOMB_VALUE_STACKS, ..Default::default() },
                slow,
            ];
            apply_statuses_with_trace(
        ...
        YolkBombRouting::EnemyStatus(status_id) => {
            let opp_label = if source_label == "A" { "B" } else { "A" };
            let statuses = [
                SimpleAppliedStatus {
                    status_id: status_id.to_string(),
                    stacks: YOLK_BOMB_VALUE_STACKS, ..Default::default() },
            ...
            apply_statuses_with_trace(
                ...
                source_label, attacker_hp, opp_label, "Yolk Bomb",
                combat_log,
            );
        }
