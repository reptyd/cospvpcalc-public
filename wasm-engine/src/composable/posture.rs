...
//!
//! | posture | health_regen x | neg.ailment decay x | incoming bite/breath x |
//! |---------|----------------|---------------------|-----------------------|
...
/// Statuses whose recovery is accelerated by Defiled Ground and the
/// Darkstar plushie while the holder is settled sit/lay. This is the
/// Defiled-Ground "recoverable ailment" set - a strict subset of
/// `is_negative_ailment`. The recovery accelerators (Defiled Ground
/// level, Darkstar) only touch these; other negative ailments still
/// strip at the plain posture rate.
pub fn is_defiled_ground_recoverable(status_id: &str) -> bool {
    matches!(
        status_id,
        "Bad_Omen"
            | "Bleed_Status"
...
mod tests {
    ...
    fn settled_multipliers_match_spec() {
        ...
        assert_eq!(settled_incoming_damage_mult(Posture::Laying), 2.0);
    }

    #[test]
    fn defiled_ground_recoverable_is_subset_of_negative_ailments() {
        // The recovery accelerators only ever touch the DG-recoverable
        // set, so every member must also be a negative ailment (otherwise
        // the posture-decay multiplier and the recovery bonus would key on
        // different status families).
        for key in [
            "Bad_Omen",
            "Bleed_Status",
            "Burn_Status",
            ...
            assert!(is_defiled_ground_recoverable(key), "{key} must be recoverable");
            assert!(is_negative_ailment(key), "{key} must be a negative ailment");
        }
        // A negative ailment outside the recoverable list is NOT accelerated.
        assert!(is_negative_ailment("Shock_Status"));
        assert!(!is_defiled_ground_recoverable("Shock_Status"));
    }
