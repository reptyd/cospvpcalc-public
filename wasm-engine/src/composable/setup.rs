...
pub(super) fn populate_combat_sides_and_flags(
    ...
    // True iff the Oxygen/Moisture mode armed a tick on either side. Reading
    // the seeded timers keeps the immunity gate in one place (the seed match
    // above), so an all-immune matchup leaves this false and the run is inert.
    let has_any_oxygen_moisture =
        a.oxy_moist_next_tick_at.is_finite() || b.oxy_moist_next_tick_at.is_finite();
