...
//!
//! Extracted from the megafunction in `mod.rs` so the Sandbox runtime
...
pub(super) fn populate_combat_sides_and_flags(
    ...
    // Round 32 / A5: seed environment flags into each side's user_extras
    ...
    // Round 42 / A11: seed the per-fight active level for each user
