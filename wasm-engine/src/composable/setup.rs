...
//!
//! [`derive_loop_flags`] holds the other half: the [`ComposableLoopFlags`]
//! bundle the main event loop reads to skip branches their abilities don't
//! use. It reads the stats and the config and writes nothing, so a caller that
//! needs the flags again for an already-started fight - the Sandbox, whenever
//! a setting changes - can have them without the seeding above.
//!
...
//! duplicating ~380 LoC of conditional setup.
//!
...
pub(super) struct ComposableLoopFlags {
    ...
    /// True when the global Oxygen / Moisture drain mode is on AND at least one
    /// side armed a per-second tick (non-immune). Gates the scheduler fold and
    /// the per-second tick phase so off / all-immune stays byte-identical.
    pub has_any_oxygen_moisture: bool,
...
/// How often a standing Hunker decision is reconsidered, by timing mode.
/// The deeper modes look again more often; every other mode shares the
/// half-second cadence.
pub(crate) fn hunker_cadence_for(policy: SimpleAbilityTimingMode) -> f64 {
    match policy {
...
fn attacker_has_damage_trail(config: &ComposableAbilityConfig) -> bool {
    config.attacker_flame_trail_value > 0.0
        || config.attacker_frost_trail_value > 0.0
        || config.attacker_plague_trail_value > 0.0
        || config.attacker_toxic_trail_value > 0.0
        || (config.attacker_trail_status_id.is_some() && config.attacker_trail_value > 0.0)
}

fn defender_has_damage_trail(config: &ComposableAbilityConfig) -> bool {
    config.defender_flame_trail_value > 0.0
        || config.defender_frost_trail_value > 0.0
        || config.defender_plague_trail_value > 0.0
        || config.defender_toxic_trail_value > 0.0
        || (config.defender_trail_status_id.is_some() && config.defender_trail_value > 0.0)
}

/// Read the flag bundle off the stats and the config without touching either
/// side. The event loop needs it once per fight, and the Sandbox needs it
/// again whenever a setting changes mid-fight - at which point re-running the
/// `time = 0` seeding below would put the fight back to its opening state.
pub(super) fn derive_loop_flags(
    ...
    let hunker_decision_cadence_sec =
        hunker_cadence_for(hunker_eff_policy_a).min(hunker_cadence_for(hunker_eff_policy_b));
    ...
    let has_any_damage_trail =
        attacker_has_damage_trail(config) || defender_has_damage_trail(config);
    ...
    let has_any_active_ability = has_any_fortify
        ...
        || config.attacker_guardians_passage
        || config.defender_guardians_passage
    ...
    // The Oxygen / Moisture mode arms a per-second tick only on a side whose
    // pool for the selected mode is non-zero; all-immune leaves it inert.
    let has_any_oxygen_moisture = match config.oxygen_moisture_mode.as_deref() {
        Some("ground") => attacker.moisture_time > 0.0 || defender.moisture_time > 0.0,
        Some("underwater") => attacker.oxygen_time > 0.0 || defender.oxygen_time > 0.0,
        _ => false,
    };
...
pub(super) fn populate_combat_sides_and_flags(
    ...
    let flags = derive_loop_flags(attacker, defender, ability_policy, config);
    // Starting HP is gated on Warden's Rage on purpose. The control exists for
    // that ability, whose damage scales with HP already lost, so a fight that
    // opens at full health never shows what it is worth. Nothing else reads a
    ...
    // compareFirstTickMode is "regen" or "both".
    ...
    a.compare_dark_star = config.attacker_compare_dark_star;
    ...
    a.compare_thirst = if config.attacker_compare_starting_thirst > 0.0 {
        compare_hunger::normalize_compare_hunger(config.attacker_compare_starting_thirst)
    } else {
        a.compare_appetite_base
    };
    for side in [&mut *a, &mut *b] {
        side.compare_season_hunger_interval = if config.compare_season_hunger_interval > 0.0 {
            config.compare_season_hunger_interval
        } else {
            1.0
        };
        side.compare_season_thirst_interval = if config.compare_season_thirst_interval > 0.0 {
            config.compare_season_thirst_interval
        } else {
            1.0
        };
    }
    a.reflect_response_hold = config.attacker_reflect_response_hold;
    b.reflect_response_hold = config.defender_reflect_response_hold;
    a.compare_has_hunger = !config.attacker_compare_has_no_hunger;
    a.compare_has_thirst = !config.attacker_compare_has_no_thirst;
    ...
    a.compare_plushie_thirst_drain_multiplier =
        if config.attacker_compare_plushie_thirst_drain_multiplier > 0.0 {
            config.attacker_compare_plushie_thirst_drain_multiplier
        } else {
            a.compare_plushie_drain_multiplier
        };
    ...
    b.compare_dark_star = config.defender_compare_dark_star;
    ...
    b.compare_plushie_thirst_drain_multiplier =
        if config.defender_compare_plushie_thirst_drain_multiplier > 0.0 {
            config.defender_compare_plushie_thirst_drain_multiplier
        } else {
            b.compare_plushie_drain_multiplier
        };
    ...
    b.compare_thirst = if config.defender_compare_starting_thirst > 0.0 {
        compare_hunger::normalize_compare_hunger(config.defender_compare_starting_thirst)
    } else {
        b.compare_appetite_base
    };
    b.compare_has_hunger = !config.defender_compare_has_no_hunger;
    b.compare_has_thirst = !config.defender_compare_has_no_thirst;
    ...
    // runtime.hasHealingStep ? HEALING_STEP_TICK_SEC : +inf.
    ...
    if attacker_has_damage_trail(config) {
        a.damage_trail_next_tick_at = Some(DAMAGE_TRAIL_TICK_SEC);
    }
    if defender_has_damage_trail(config) {
    ...
    // Defiled Ground Weakness: the afflicted side stands on the opponent's
    // defiled ground and carries DefiledSickly. In game the ability re-applies
    // it continuously with an effectively unbounded stack limit (EnemyAilment
    // Limit 9e9), so it never lapses - modeled here as a single permanent
    // (no_decay) Sickly_Status. The ailment carries both modeled effects: the
    // -20% HealthRegen (effects registry) and the hunger-drain bump (read from
    // its presence in `advance_side_hunger`).
    if a.compare_defiled_ground_weakness_enabled {
        crate::statuses::seed_permanent_status(0.0, &mut a.statuses, "Sickly_Status");
    }
    if b.compare_defiled_ground_weakness_enabled {
        crate::statuses::seed_permanent_status(0.0, &mut b.statuses, "Sickly_Status");
    }

    // Seed the Oxygen / Moisture drain mode (Compare-only). The global mode
    // selects which per-side pool drains; a side whose relevant-mode stat is
    // 0 is immune (no pool, no tick armed). The first tick is at t=1; the pool
    // starts at the stat value and counts down 1/sec. Off / unknown leaves the
    // `oxy_moist_next_tick_at = INFINITY` default, so the scheduler is inert
    // and the run is byte-identical.
    match config.oxygen_moisture_mode.as_deref() {
        Some("ground") => {
            if attacker.moisture_time > 0.0 {
                a.moisture_remaining = attacker.moisture_time;
                a.oxy_moist_next_tick_at = 1.0;
            }
            if defender.moisture_time > 0.0 {
                b.moisture_remaining = defender.moisture_time;
                b.oxy_moist_next_tick_at = 1.0;
            }
        }
        Some("underwater") => {
            if attacker.oxygen_time > 0.0 {
                a.oxygen_remaining = attacker.oxygen_time;
                a.oxy_moist_next_tick_at = 1.0;
            }
            if defender.oxygen_time > 0.0 {
                b.oxygen_remaining = defender.oxygen_time;
                b.oxy_moist_next_tick_at = 1.0;
            }
        }
        _ => {}
    ...
    // Aerial Dodge (Compare-only): seed the global toggle + roll style onto
    // both sides; the per-side hit chance rides on the stats. Off leaves
    // `aerial_dodge_active = false`, so the incoming-attack gate returns
    // "lands" without touching roll state and the run is byte-identical.
    if config.aerial_dodge_active {
        a.aerial_dodge_active = true;
        b.aerial_dodge_active = true;
        a.aerial_dodge_real_random = config.aerial_dodge_real_random;
        b.aerial_dodge_real_random = config.aerial_dodge_real_random;
        if config.aerial_dodge_real_random {
            let seed_bits = config.aerial_dodge_seed.to_bits();
            a.aerial_dodge_rng = super::aerial_dodge_mix_seed(seed_bits, 0);
            b.aerial_dodge_rng = super::aerial_dodge_mix_seed(seed_bits, 1);
        }
    }
    ...
    if flags.has_any_rewind {
        if config.attacker_rewind {
            record_rewind_snapshot(&mut a.rewind_history, 0.0, a.hp);
        }
        if config.defender_rewind {
            record_rewind_snapshot(&mut b.rewind_history, 0.0, b.hp);
        }
    }
