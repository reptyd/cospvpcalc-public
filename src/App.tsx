...
// Beta design - the experimental next-gen site shell. Lazy so its chrome +
// CSS never ship to the default bundle; only loaded once a dev-unlocked user
...
function loadCreatureData(): Promise<CreatureDataRuntime> {
  if (!creatureDataPromise) {
    // Load the catalog AND install the custom-creature catalog bridge together.
    // Installing the bridge here drains every custom creature restored/buffered
    // before the catalog was ready (boot restore + share-link ephemerals) into
    // the live roster. This is the lazy seam that keeps the ~1.2 MB catalogs off
    // the boot main thread - they parse only once something actually needs them.
    creatureDataPromise = Promise.all([
      import("./engine/creatureData"),
      import("./engine/customCreatureCatalogBridge"),
    ]).then(([module, bridge]) => {
      bridge.ensureCustomCreatureCatalogBridge();
      return {
...
function App() {
  ...
  // Beta is the default design now; classic is the explicit opt-out. A fresh
  // visitor (no stored value) gets beta. The old `cos.compareDesign` key is
  // intentionally not read - under the previous scheme it was "default" for
  // nearly everyone, which would wrongly force classic here.
  const storedCompareBeta = safeReadLocalStorage("cos.uiDesign") !== "classic";
  ...
  const [compareDesignBeta, setCompareDesignBeta] = useState(storedCompareBeta);
  ...
  useEffect(() => {
    if (importedMatchModeRef.current) return;
    safeWriteLocalStorage("cos.uiDesign", compareDesignBeta ? "beta" : "classic");
  }, [compareDesignBeta]);
  ...
  // Site-wide design. Beta is the default everyone sees (compareDesignBeta
  // defaults to true via `cos.uiDesign !== "classic"`); the always-visible
  // Settings "Beta design" toggle lets any visitor opt out to the classic
  // layout. No dev unlock required - the toggle's flag is authoritative.
  const designBeta = compareDesignBeta;
  ...
  const pageRouter = (
    <AppPageRouter
      ...
      compareDesignBeta={designBeta}
      importedMatchMode={importedMatchMode}
  ...
  return (
    <BestBuildsBattleSettingsProvider>
      <div className="app" data-design={designBeta ? "beta" : undefined}>
        <a className="skip-link" href="#main-content">
          Skip to main content
        </a>
        {designBeta ? (
          <Suspense fallback={<div className="beta-shell-fallback">Loading…</div>}>
            <BetaShell
              page={page}
              setPage={setPage}
              ...
              trueDeveloperMode={trueDeveloperMode}
              setSiteDesignBeta={setCompareDesignBeta}
              buildHash={buildHash}
          ...
          <>
            ...
            <AppHero
              ...
              buildHash={buildHash}
            />
            <main id="main-content">{pageRouter}</main>
            <AppFooter />
            {trueDeveloperMode ? <AdSlot position="bottom" /> : null}
            {/* Floating bottom-left switch back to the new (beta) default
                design - the classic-shell counterpart to the beta sidebar's
                "Classic design" button. Fixed so it rides the viewport. */}
            <button
              type="button"
              className="classic-design-switch"
              onClick={() => setCompareDesignBeta(true)}
              title="Switch to the new design"
              aria-label="Switch to the new design"
            >
