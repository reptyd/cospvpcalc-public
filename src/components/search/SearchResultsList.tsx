...
export function SearchResultsList({
  ...
  // Beta-only extras (the classic page passes none, so its layout is unchanged):
  // hide the built-in <details> picker (the beta page renders its own popover),
  // and offer a per-card "add to Best Builds pool" toggle.
  showColumnPicker?: boolean;
  pooledSet?: Set<string>;
      ...
      {results.length === 0 ? (
        ...
        <div className="search-results-grid">
          {results.map(({ creature, effects }) => {
            ...
            const pooled = pooledSet?.has(creature.name) ?? false;
            return (
              <div key={creature.name} className="search-result-card">
                {onTogglePool ? (
                  <button
                    type="button"
                    className={`search-result-pool${pooled ? " is-pooled" : ""}`}
                    onClick={() => onTogglePool(creature.name)}
                    aria-pressed={pooled}
                    title={pooled ? "Remove from Best Builds pool" : "Add to Best Builds pool"}
                  >
                    {pooled ? <Check size={12} strokeWidth={2.6} aria-hidden="true" /> : <Plus size={12} strokeWidth={2.6} aria-hidden="true" />}
                    Pool
                  </button>
                ) : null}
...
function MatchBadges({
  ...
  // The user's noisy "Offensive Bleed: - x 8" came from leaving
