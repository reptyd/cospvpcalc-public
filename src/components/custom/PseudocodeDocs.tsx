    ...
    <div className="ce-docs-backdrop" onClick={onClose}>
      ...
      >
        <header className="ce-docs-header">
          ...
          <button className="ce-btn ce-btn-ghost ce-docs-close" onClick={onClose}>
            Close <span className="ce-docs-close-esc">(Esc)</span>
          </button>
        </header>
...
function DocsContent({
  ...
  // Ability levels + scaling tables (skip if single-level)
  ...
  on_before_take_damage:    ...     // pre-damage hook (VICTIM).
                                    // Write set_extra self damage_override = N
                                    // to replace the incoming amount.
  on_before_deal_damage:    ...     // DEALER pre-damage hook (fires before victim hook)
...
5. If best.utility > threshold:
     if best.delay <= threshold: return Now
        ...
        <ul>
          ...
          <li>
            <strong>Sliding windows</strong> read the buffer at the
...
combat.bites_dealt            # cumulative bites dealt by self
...
scaling.<key>                 # spec.scaling[<key>][active_level-1]
...
env.air_rule_active           # "Fixed Bite Cadence" enabled
...
self.extras.<key>.length      # numbered-key array length
self.extras.<key>.sum         # sum of array entries
self.extras.<key>.last        # most recently pushed
self.extras.<key>.<i>         # literal-index read
...
self.damage_taken_last.<N>    # post-mitigation damage TAKEN in last N
                              # seconds (bite-only; engine keeps a 30s buffer)
self.damage_dealt_last.<N>    # same for damage DEALT

opp.*                         # mirror of every self.* path. opp is an
opponent.*                    # alias for opponent.
...
apply [<Id1> x<n>, <Id2> x<n>, ...] to (self|opp)   # array form
clear (self|opp) <Status_Id>                       # remove specific status
clear (self|opp) [<Id1>, <Id2>, ...]               # array form
...
push_extra (self|opp) <key> <Expr>           # append to a numbered-key array
                                              # stored as <key>.length + <key>.<i>;
                                              # cap 256 entries
clear_array (self|opp) <key>                 # remove all <key>.<i> + <key>.length`}</Code>
        <p className="hint">
          <strong>Arrays.</strong> Read back with{" "}
...
schedule <secs> as <name>:                    # named - cancellable
  <effects>

cancel_schedule <name>                        # drop matching queued
reschedule <name> <secs>                      # move due time`}</Code>
        <p className="hint">
          <strong>Named schedules.</strong> Adding{" "}
...
trigger <ability_id>                          # chain another user ability`}</Code>
  ...
  force_fire: self.hp_ratio < 0.2`}</Code>
      ...
      <Section title="Vocabulary - exact values you can plug in">
        ...
        <p>
          <strong>Custom statuses.</strong> Define your own
        ...
        <p>
          ...
          every numeric combat field (the full set routes through a
        ...
        <p className="hint">
          ...
          <code>self.damage_taken_last.5</code> (last-5s damage),{" "}
          <code>opp.damage_dealt_last.10</code>. <code>opp.</code> is an
          alias for <code>opponent.</code>
        </p>
...
combat.bites_dealt            # bites dealt by self
...
ability user.scaled_strike "Scaled Strike"
  ...
  on_fire:
    ...
    cooldown self user.scaled_strike for 5`}</Code>
      ...
      <Section title="Damage-modify hooks - shields, parries, amplifiers">
        <p>
...
on_before_deal_damage:
  if opp.hp_ratio < 0.3:
    set_extra self damage_override = event.damage_taken * 3.0`}</Code>
        ...
        <ul>
          ...
          <li>
            <strong>Every damage source routes through the hooks.</strong>{" "}
            Bite, breath, DOT, lance (aura +
      ...
      <Section title="Ability levels &amp; scaling">
        <p>
  ...
  on_fire:
    ...
    cooldown self user.scaled_strike for 5`}</Code>
      ...
      <Section title="Batch-level gate - `when` field">
        <p>
      ...
      <Section title="Custom breath weapons">
        <p>
          ...
          the same <code>SimpleBreathProfile</code> path either way - this is
        ...
        <p>
          ...
          custom status - so an authored breath can drive an
          authored status, e.g. a custom DoT.
        </p>
      </Section>

      <Section title="Custom statuses - the Status DSL">
        <p>
  ...
  bite_cooldown_mult 1.2`}
        ...
        <Subhead>Programmable statuses - hooks, expressions &amp; teardown</Subhead>
        <p>
...
Per-iteration aggregation (after all damage phases that iteration):
  ...
  8.  on_heal           - cumulative heal_amount > 0 this iter
...
The ability never silently disables.`}</Code>
  ...
  4. Session default ("Ability timing mode" in Compare)`}</Code>
        ...
        <Subhead>9. Pre-damage shield doesn't seem to absorb</Subhead>
        <ul>
          <li>
            <strong>All sources now route.</strong> Bite,
        ...
        <Subhead>10. Sliding-window gate never fires</Subhead>
        <ul>
          <li>
        ...
        <Subhead>11. Level override doesn't apply</Subhead>
        <ul>
          <li>
