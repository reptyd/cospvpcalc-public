...
/**
 * Constructor-coverage lock for user statuses (Phase 8 / G6) — the
 * status-editor twin of `abilityPaletteCoverage.test.ts`.
 *
 * The design contract is "Constructor coverage = 100% of the schema":
...
/**
 * Block-constructor coverage (Phase 9): every `"blocks"` schema field — the
...
/**
 * Code↔Visual parity lock (Phase 8 dual-mode). Both editor modes back the
...
describe("Status Code↔Visual round-trip (parity lock)", () => {
  ...
  const fullSpec: UserStatusSpec = {
    ...
    // Phase 9 programmable extensions. Hook batch names normalize to the block
