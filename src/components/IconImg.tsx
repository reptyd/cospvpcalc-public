...
// `responsive` opts the icon into a mobile size bump (see `.icon-img--responsive`
// in App.css) - use it for content portraits, not small inline chrome glyphs.
export function IconImg({ src, alt, size, responsive = false }: { src: string | null; alt: string; size: number; responsive?: boolean }) {
  ...
  return (
    <img
      className={responsive ? "icon-img icon-img--responsive" : "icon-img"}
      src={url}
      alt={alt}
      loading="lazy"
      decoding="async"
      width={size}
      height={size}
      style={{ "--icon-base": `${size}px`, flexShrink: 0 } as CSSProperties}
      onError={() => {
        if (!primarySrc) {
          setFailed(true);
          return;
