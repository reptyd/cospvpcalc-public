 ...
 *
 *  - Rows are populated from `creature.userAbilityIds` x the
...
export function UserAbilityTimingOverridesPanel({
  ...
  // No custom abilities attached to either creature -> nothing to override,
  // so the panel is hidden entirely (it used to render an empty "none
  // attached" discoverability stub, which was pure noise in the common case).
  if (totalAttached === 0) return null;
