...
export function getViewDetails(summary: SimulationSummary, mode: CompareResultViewMode, side: "A" | "B") {
  ...
  // Breath Time in seconds = (fired ticks up to the cutoff) x BREATH_TICK_SEC,
  // taken from the engine's own record of when the breath fired. The combat
  // log cannot give this: a heal breath fires twice a second but heals once,
  // and a dodged or fully absorbed damage tick writes no damage entry, so
  // counting log entries reports less firing than happened.
  const firedTicks = (summary.debug?.[side]?.breathFireTimes ?? []).filter(
    (time) => time <= cutoffTime + 1e-9,
  ).length;
  const breathTimeSec = firedTicks * BREATH_TICK_SEC;
