...
// that can be active simultaneously on the same combatant. A future version
// will read the canonical catalog directly; for now it hardcodes here so the
...
export function formatTempBuffHpRegen(
  ...
  return (
    <div className="panel-block">
      <div className="card-title">
        <IconImg src={iconUrl} alt={stats?.name ?? title} size={64} responsive />
        <h3>{title}</h3>
      </div>
        ...
        <ul className="stat-list">
          ...
          <li>
            ...
            {compareAirRuleEnabled && typeof compareAirRuleCooldownSec === "number"
              ? `${compareAirRuleCooldownSec} (Fixed Bite Cadence)`
              : stats.biteCooldown}
          </li>
          {compareAirRuleEnabled && typeof compareAirRuleCooldownSec === "number" ? (
