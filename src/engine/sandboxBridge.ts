...
export type SandboxPosture = "standing" | "sitting" | "laying";
export type SandboxBiteVariant = "primary" | "secondary";
...
export type SandboxSideView = {
  ...
  /** Settled posture: `"standing"` / `"sitting"` / `"laying"`. */
  postureCurrent: SandboxPosture;
  /** Posture being transitioned to (equals `postureCurrent` when settled). */
  posturePending: SandboxPosture;
  postureTransitioning: boolean;
  /** Seconds until the in-flight transition settles (`null` when settled). */
  postureTransitionRemaining: number | null;
  postureRegenMult: number;
  postureDecayMult: number;
  postureIncomingDamageMult: number;
  /** Whether this side has a secondary attack (`damage2 > 0`). */
  hasSecondary: boolean;
  /** Appetite units left on each meter, or null when the creature has no such
   *  meter. A meter stops at zero; what an empty bar goes on consuming is the
   *  starving stack count and is not part of this number. */
  hunger: number | null;
  thirst: number | null;
  /** Full size of both meters - the game sizes them off one appetite stat. */
  appetiteBase: number;
};

export type SandboxMeter = "hunger" | "thirst";
...
export type SandboxLogEntryView = {
  ...
  eventType: string;
  description: string;
  /** HP swing of the event (0 for non-damaging). Present on engine log
   * entries; omitted on UI-built manual-action entries. */
  damage?: number;
  healing?: number | null;
  /** Acting side's HP right after the event - the timeline "HP N" readout. */
  actorHpAfter?: number;
  detail?: string | null;
};
...
/** The part of a create payload that describes the fight rather than the run:
 *  everything a live sandbox can be handed without starting over. */
export type SandboxReconfigurePayload = Omit<
  SandboxCreatePayload,
  "maxTimeSec" | "automationMode" | "recordTrace"
>;
...
type RustSandboxModule = {
  ...
  sandbox_apply_meter_js: (id: bigint, payload: unknown) => unknown;
  ...
  sandbox_set_automation_mode_js: (id: bigint, payload: unknown) => unknown;
  sandbox_reconfigure_js: (id: bigint, payload: unknown) => unknown;
  sandbox_force_posture_js: (id: bigint, payload: unknown) => unknown;
  ...
  sandbox_set_bad_omen_outcomes_js: (id: bigint, payload: unknown) => unknown;
...
async function loadSandboxModule(): Promise<RustSandboxModule | null> {
  ...
  if (
    ...
    typeof m.sandbox_apply_meter_js !== "function" ||
    ...
    typeof m.sandbox_force_posture_js !== "function" ||
    ...
    typeof m.sandbox_set_bad_omen_outcomes_js !== "function" ||
...
export async function sandboxApplyMeter(
  id: bigint,
  side: SandboxSide,
  meter: SandboxMeter,
  value: number,
  ...
  const mod = await requireSandbox();
  return unwrapJsValue<SandboxView>(mod.sandbox_apply_meter_js(id, { side, meter, value }));
}
...
export async function sandboxForceBite(
  id: bigint,
  side: SandboxSide,
  variant: SandboxBiteVariant = "primary",
): Promise<SandboxView> {
  const mod = await requireSandbox();
  return unwrapJsValue<SandboxView>(mod.sandbox_force_bite_js(id, { side, variant }));
}
...
export async function sandboxReconfigure(
  id: bigint,
  payload: SandboxReconfigurePayload,
): Promise<SandboxView> {
  const mod = await requireSandbox();
  // Same null-stripping as createSandbox: a breath turned off arrives as null,
  // which serde_wasm_bindgen rejects outright.
  return unwrapJsValue<SandboxView>(mod.sandbox_reconfigure_js(id, stripNullsForWasm(payload)));
}

export async function sandboxSetAutomationMode(
  id: bigint,
  mode: SandboxAutomationMode,
): Promise<SandboxView> {
  const mod = await requireSandbox();
  return unwrapJsValue<SandboxView>(mod.sandbox_set_automation_mode_js(id, { mode }));
}

export async function sandboxForcePosture(
  id: bigint,
  side: SandboxSide,
  target: SandboxPosture,
): Promise<SandboxView> {
  const mod = await requireSandbox();
  return unwrapJsValue<SandboxView>(mod.sandbox_force_posture_js(id, { side, target }));
}
...
/// Replace the random Bad Omen follow-up batch on a live runtime. Bad Omen
/// re-rolls its on-expiry debuff every simulation; the Sandbox hook pushes a
/// fresh batch here on every backward seek so rewinding + re-advancing shows a
/// different follow-up each pass (Best Builds pins a deterministic outcome and
/// never calls this; the debug Roulette pins via the config instead).
export async function sandboxSetBadOmenOutcomes(
  id: bigint,
  outcomes: RustBadOmenOutcome[],
): Promise<SandboxView> {
  const mod = await requireSandbox();
  return unwrapJsValue<SandboxView>(mod.sandbox_set_bad_omen_outcomes_js(id, { outcomes }));
}
