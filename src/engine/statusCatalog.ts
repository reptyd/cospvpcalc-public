...
//! inconsistency reported on 2026-05-18.
//!
...
const NAME_TO_ENGINE_ID: Record<string, string> = {
  ...
  "Thirsty": "Thirsty_Status",
  "Frostbite": "Frostbite_Status",
  "Gale": "Water_Gale_Status",
  "Guardian's Seal": "Guardian_Seal_Status",
  ...
  "Newborn": "Newborn_Status",
  "Poison": "Poison_Status",
  "Refreshed": "Refreshed_Status",
  "Satiated": "Satiated_Status",
  "Spring Water": "Spring_Water_Status",
  ...
  "Tunnel Vision": "Tunnel_Vision_Status",
  ...
  "Aggressive (Bear)": "Aggressive_Bear_Status",
...
//     per-stack decay (explicit values filled in later).
//   - status_max_stacks (Sticky Teeth = 5) -> defaultMaxStacks.
//
// The backfill leaves `effect: { kind: "meta_marker" }` as a placeholder
// for almost every entry; real EffectShape values are filled once
...
const NAME_TO_EFFECT_META: Record<string, EffectMeta> = {
  ...
  "Aggressive": {
    ...
    defaultDurationSec: 1, defaultTickSec: null, defaultMaxStacks: 10,
    stackRule: "stacking",
    // Compare emote buff: +25% multiplicative on outgoing damage. The emote
    // grants 10 stacks that come off one a second, which is the game's shape
    // (`Aggression`, MaxStack 10) and totals the same 10 seconds. The
    // magnitude is flat - combat.rs reads the id, not the stack count - so
    // the stacks are a lifetime, not a strength. Positive twin of Scared;
    // this row drives polarity (Fortify does NOT cleanse a positive),
    // category, the per-stack second and the 10-stack ceiling.
    effect: { kind: "stat_modifier", mods: { damagePct: 25 }, opStyle: "add_pct" },
    sources: ["compare_toggle"],
  },
  "Aggressive (Bear)": {
    polarity: "positive", category: "stat_buff",
    defaultDurationSec: 1, defaultTickSec: null, defaultMaxStacks: 10,
    stackRule: "stacking",
    // Bear-plushie variant: +37.5% outgoing damage for 10 seconds
    // (Bear formula: 25 * 1.1 + 10 = 37.5).
    effect: { kind: "stat_modifier", mods: { damagePct: 37.5 }, opStyle: "add_pct" },
    sources: ["compare_toggle"],
  },
  "Bad Omen": {
  ...
  // Hungry and Thirsty are not applied by anything - the engine keeps their
  // stack count equal to how much an empty meter has gone on consuming, and
  // drops them when it refills. Hence no sources: no picker should offer them.
  "Hungry": {
    polarity: "negative", category: "dot_damage",
    defaultDurationSec: null, defaultTickSec: 3, defaultMaxStacks: null,
    stackRule: "stacking",
    effect: {
      kind: "composite",
      parts: [
        // Percent-of-max-HP DoT, so the value lives in
        // compute_simple_dot_damage; this part just claims the tick channel.
        { kind: "meta_marker" },
        { kind: "stat_modifier", mods: { healthRegenPct: -100 }, opStyle: "add_pct" },
      ],
    },
  ...
  "Newborn": {
    ...
    effect: {
      kind: "composite",
      parts: [
        { kind: "stat_modifier", mods: { healthRegenPct: 50 }, opStyle: "add_pct" },
        { kind: "stat_modifier", mods: { hungerIntervalMult: 1.25, thirstIntervalMult: 1.25 }, opStyle: "mult" },
      ],
    },
    sources: ["compare_toggle"],
  ...
  "Satiated": {
    polarity: "positive", category: "stat_buff",
    defaultDurationSec: 300, defaultTickSec: null, defaultMaxStacks: null,
    stackRule: "non_stacking",
    effect: { kind: "stat_modifier", mods: { hungerIntervalMult: 1.3 }, opStyle: "mult" },
    sources: ["compare_toggle"],
    displayGroup: "compare_temp_buff",
  },
  "Spring Water": {
    ...
    defaultDurationSec: 300, defaultTickSec: null, defaultMaxStacks: null,
    stackRule: "non_stacking",
    effect: { kind: "stat_modifier", mods: { thirstIntervalMult: 1.3 }, opStyle: "mult" },
    sources: ["compare_toggle"],
    displayGroup: "compare_temp_buff",
  },
  "Territory": {
    ...
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: null,
    stackRule: "non_stacking",
    effect: { kind: "stat_modifier", mods: { hungerIntervalMult: 1.2 }, opStyle: "mult" },
    sources: ["compare_toggle"],
    displayGroup: "compare_temp_buff",
  },
  "Blessing's Boon": {
  ...
  "Clean Water": {
    ...
    effect: {
      kind: "composite",
      parts: [
        { kind: "stat_modifier", mods: { healthRegenPct: 20 }, opStyle: "add_pct" },
        { kind: "stat_modifier", mods: { hungerIntervalMult: 1.2, thirstIntervalMult: 1.2 }, opStyle: "mult" },
      ],
    },
  ...
  "Corrosion": {
    polarity: "negative", category: "dot_damage",
    // Game caps Corrosion at 90 stacks - the point where its weight reduction
    // saturates (7.5% base + 1% per stack = the 97.5% cap). Past that only the
    // remaining duration grew, so the cap costs nothing but the runaway count.
    defaultDurationSec: null, defaultTickSec: 3, defaultMaxStacks: 90,
    stackRule: "stacking", effect: { kind: "meta_marker" },
    sources: ["yolk_bomb_enemy", "lich_mark_payload", "ability_apply"],
  },
  "Deep Wounds": {
    polarity: "negative", category: "stat_debuff",
    // Game caps Deep Wounds at the per-application Limit=10 (Serrated Teeth and
    // Gore Charge both apply 10). Presence-only effect (blocks Bleed
    // decay/healing), so the cap bounds how long the block persists.
  ...
  "Disease": {
    ...
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: null,
    stackRule: "stacking",
    // Flat health regen reduction: x0.75 regardless of stack count. The game's
    // Disease.OnGet returns HealthRegen * StaminaHealPercent (0.75) and ignores
    // the stack count for regen - only its hunger/thirst drain scales with
    // stacks. So this is a single flat -25% modifier, not a per-stack one.
  ...
  "Frostbite": {
    ...
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: null,
    stackRule: "stacking",
    // Its bite-cooldown and stamina halves live in the engine; the hunger half
    // is a x0.9 on the drain interval, so an iced creature eats through its
    // appetite about 11% faster. Thirst is untouched.
    effect: {
      kind: "composite",
      parts: [
        { kind: "meta_marker" },
        { kind: "stat_modifier", mods: { hungerIntervalMult: 0.9 }, opStyle: "mult" },
      ],
    },
    sources: ["yolk_bomb_enemy", "lich_mark_payload", "ability_apply"],
  },
  ...
  "Guardian's Seal": {
    polarity: "positive", category: "stat_buff",
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: 3,
    stackRule: "stacking",
    // Flat -85% on everything that reaches the sealed side's HP. Stacks are
    // duration only; the magnitude lives in the damage pipeline
    // (composable/damage_pipeline.rs), this row drives polarity, the 3-stack
    // cap and the pickers.
    effect: { kind: "stat_modifier", mods: { damageTakenPct: -85 }, opStyle: "add_pct" },
    sources: ["compare_toggle", "ability_apply"],
  },
  "Healing Ailment": {
    polarity: "positive", category: "stat_buff",
  ...
  "Heartbroken": {
    polarity: "negative", category: "stat_debuff",
    // Game caps Heartbroken at Grim Lariat's per-application Limit=10 (each cast
    // applies 8, so a re-cast reaches 16 without the cap). Presence-only effect
    // (blocks ability healing), so the cap bounds how long the block persists.
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: 10,
    stackRule: "stacking", effect: { kind: "meta_marker" },
    sources: ["yolk_bomb_enemy", "lich_mark_payload", "ability_apply"],
  },
  ...
  "Sticky Teeth": {
    polarity: "negative", category: "stat_debuff",
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: 5,
    stackRule: "stacking", effect: { kind: "meta_marker" },
    sources: ["yolk_bomb_enemy", "lich_mark_payload", "ability_apply"],
  },
  "Stolen Speed": {
  ...
  "Aftershock": {
    polarity: "negative", category: "stat_debuff",
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: null,
    // -20% outgoing damage, flat regardless of stack count (stacks extend the
    // duration, they do not deepen the penalty). Applied to the ENEMY via Yolk
    // Bomb (Zeoarex) and Lich Mark (Garluhmoat). The runtime magnitude lives in
    // combat.rs outgoing_damage_pct_from_statuses, like Scared/Aggressive.
    stackRule: "stacking", effect: { kind: "stat_modifier", mods: { damagePct: -20 }, opStyle: "add_pct" },
    sources: ["ability_apply"],
  },
  "Ashy Lungs": {
    polarity: "negative", category: "neutral_marker",
  ...
  "Radiation": {
    polarity: "negative", category: "dot_damage",
    defaultDurationSec: null, defaultTickSec: 3, defaultMaxStacks: 99,
    stackRule: "stacking",
    // Radiation is a fully-fledged ailment whose signature effect - lowering
    // the target's positive ailment blocks for every OTHER ailment - lives in
    // the Rust engine (statuses.rs apply_status_application_in_place). Its
    // 0.5% max-HP-per-tick DOT is in compute_simple_dot_damage. Both are
    // engine-side, so the catalog only carries the tick cadence (3 s), the
    // 99-stack cap, and a meta_marker effect (no declarative modifier to emit).
  ...
  "Scared": {
    ...
    defaultDurationSec: 1, defaultTickSec: null, defaultMaxStacks: 10,
    stackRule: "stacking",
    // -50% multiplicative on outgoing damage. Ten stacks a second apart, the
    // shape the game's `Cower` carries, totalling the same 10 seconds.
  ...
  "Scared (Bear)": {
    ...
    defaultDurationSec: 1, defaultTickSec: null, defaultMaxStacks: 10,
    stackRule: "stacking",
    // Bear-plushie variant: -45% outgoing damage for 10 seconds
    // (softer than plain Scared's -50%; Bear formula -50 * 1.1 + 10 = -45,
    // matching combat.rs Scared_Bear_Status => -45.0).
    effect: { kind: "stat_modifier", mods: { damagePct: -45 }, opStyle: "add_pct" },
    sources: ["compare_toggle", "ability_apply"],
  },
  "Sickly": {
    polarity: "negative", category: "stat_debuff",
    defaultDurationSec: null, defaultTickSec: null, defaultMaxStacks: null,
    // Game AilmentData Sickly / DefiledSickly are Stackable = false.
    stackRule: "non_stacking",
    // -20% multiplicative on passive health regen while active, plus a x0.8 on
    // both drain intervals - the game's Sickly is what carries the Defiled
    // Ground weakness.
    effect: {
      kind: "composite",
      parts: [
        { kind: "stat_modifier", mods: { healthRegenPct: -20 }, opStyle: "add_pct" },
        { kind: "stat_modifier", mods: { hungerIntervalMult: 0.8, thirstIntervalMult: 0.8 }, opStyle: "mult" },
      ],
    },
    sources: ["ability_apply"],
  },
...
export type StatusCatalogEntry = {
  ...
  stackRule: EffectStackRule;
  /** Machine-readable effect content. The backfill uses
   *  `{ kind: "meta_marker" }` placeholder for most entries; real
   *  EffectShape values are filled once consumers need them. */
  effect: EffectShape;
...
function buildCatalog(): StatusCatalogEntry[] {
  ...
  for (const entry of STATUS_REFERENCE_DRAFTS) {
    ...
    // A system rule documents how the statuses behave, not a status - there is
    // nothing for a picker to apply.
    if (entry.isSystemRule) continue;
