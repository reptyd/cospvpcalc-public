...
const NAME_TO_EFFECT_META: Record<string, EffectMeta> = {
  ...
  "Aggressive": {
    ...
    // Compare emote buff: +25% multiplicative on outgoing damage for 10
    // seconds. In-game the emote applies 10 one-second stacks; with a flat
    // +25% bonus that nets to a single 10-second window, which is how the
    // engine models it (one non-stacking instance, defaultDurationSec=10).
    // Positive twin of Scared. Magnitude lives in combat.rs
    // (outgoing_damage_pct_from_statuses); this row drives polarity (Fortify
    // does NOT cleanse a positive), category, and the 10 s duration.
  ...
  "Radiation": {
    ...
    // Radiation path (generalized to a subtype-driven Aura mechanic). Tracked in the catalog so
