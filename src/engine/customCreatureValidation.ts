...
import { clampBlockFraction } from "./effectDerivation";
import { CUSTOM_ICON_RESULT_MAX_BYTES, dataUrlByteLength, isCustomIconDataUrl } from "../shared/imageIcon";
...
function normalizeFractionList(raw: unknown, warnings: string[]): FractionEntry[] {
  ...
  for (const entry of raw) {
    ...
    if (!base) continue;
    const fraction = clampBlockFraction(base.numeric);
    if (base.numeric > 1) {
      warnings.push(
        `resistStatus: "${base.statusId}" block ${base.numeric} capped to 1.0 (100%) - the engine blocks at most 100%, values above 1.0 have no extra effect.`,
      );
    }
    result.push({ statusId: base.statusId, fraction, sourceAbility: base.sourceAbility });
  }
  return result;
}
...
function normalizeCreature(raw: unknown, warnings: string[]): { creature: CreatureRuntime | null; error?: string } {
  ...
  // A custom breath profile overrides the name lookup, so a
  ...
  // Carry the user-authored breath profile. Like userAbilityIds
...
/** Coerce a raw custom-breath payload into a valid
...
export function normalizeCustomCreaturePayload(input: {
  ...
  // Canonical breath: a breath ability defines the creature's breath. If the
  // breath stat wasn't filled (e.g. the ability was added via a path where the
  // editor's auto-fill didn't fire), derive it from the breath ability here - at
  // the one save boundary both editors go through - so the breath model actually
  // runs. A custom breath profile, when present, stays authoritative.
  if (!creature.customBreathProfile && (!creature.stats.breath || creature.stats.breath === "N/A")) {
    const breathAbility = (creature.breathAbilities ?? [])[0];
    if (breathAbility?.name) {
      creature.stats.breath = breathAbility.name;
      warnings.push(`Breath set to "${breathAbility.name}" from the breath ability so it applies.`);
    }
  }
  ...
  // Accept only an image data URL within the size ceiling; anything else (a
  // stray http URL, an oversized blob) is dropped with a warning rather than
  // poisoning the store.
  let iconDataUrl: string | null = null;
  if (isCustomIconDataUrl(input.iconDataUrl)) {
    if (dataUrlByteLength(input.iconDataUrl) <= CUSTOM_ICON_RESULT_MAX_BYTES) {
      iconDataUrl = input.iconDataUrl;
    } else {
      warnings.push("Custom icon was too large and was dropped.");
    }
  }
  return {
    ok: true,
    warnings,
    payload: { creature, effects, appetite, iconName, iconDataUrl },
  };
}
