...
// Wiki-scraped plushie data can carry the same modifier row twice (e.g. Cow's two weightPct:+10 entries).
// Collapse exact duplicates by stat::op::value so a single plushie applies each modifier once, not twice.
// Everything downstream (plushieByName, applyPlushies) reads the deduped result - not the raw json.
