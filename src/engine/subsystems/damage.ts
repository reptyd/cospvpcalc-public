...
export function computeBreathDamage(
  ...
  const weightRatioCap = rules.damage.breath.weightRatioCap;
  ...
  const weightRatio = Math.min(attackerWeight / defenderWeight, weightRatioCap);
  const finalDamage = (defenderMaxHp * (weightRatio + 1)) / 2;
