...
/** Per-side breath firing discipline. Mirrors the Rust contract enum
 * `SimpleBreathPolicyMode` (see `wasm-engine/src/composable/config.rs`).
 *
 * - `onAvailability` - fire whenever the fuel meter is positive. For a chain
 *                      breath every isolated tap resets the chain, so the ramp
 *                      never builds.
 * - `onFullBar`      - hold fire while charging and only burst once the meter
 *                      is full, so the chain ramps until the meter drains.
 * - `ideal`          - engine-replay search for the burst timing that maximises
 *                      the actor's own end HP (`breath_ideal_bridge` in Rust).
 *
 * `ideal` is offered in Compare and Sandbox, whose single fights can afford its
 * per-burst replay; Best Builds filters it out (see `BREATH_POLICY_OPTIONS_BB`
 * in `engine/breathPolicy.ts`) since the search would be ruinous across its
 * ~95 k matchups, mirroring how posture policy is left off there. */
export type BreathPolicyMode = "onAvailability" | "onFullBar" | "ideal";

/** What the UI stores per side. `auto` is not an engine mode - it means "use
 * the creature's own default", resolved against the breath spec at
 * config-construction time (chain breaths burst on a full bar, everything else
 * fires on availability). The engine cannot do that resolution itself because
 * serde can't tell an unset field from an explicit `onAvailability`. */
export type BreathPolicySetting = "auto" | BreathPolicyMode;
...
export type FinalStats = CreatureStats & {
  ...
  /** Percent of Muddy's own regen bonus the equipped Oceanwing plushies add,
   * summed over copies. Consumed by `toRustStatusMeleeStats`. */
...
export type SimulationOptions = {
  ...
  // Fixed Bite Cadence. Best Builds sends the same pair through
  // `bestBuildsBattleSettingsBridge`, so this is not a compare-only field.
  compareAirRuleEnabled?: boolean;
  compareAirRuleCooldownSec?: number;
  // No Move Facetank. When enabled, persistent stand-and-fight statuses decay naturally.
  // Best Builds sends it as `block_persistent_decay = !compareNoMoveFacetank`.
  compareNoMoveFacetank?: boolean;
  // First Tick Rule. Best Builds sends the same per-side pair, so this is not compare-only either.
...
export type SimulationSummary = {
  ...
  ehpA: number;
  ehpB: number;
  /** Damage-reduction amplification (Hunker/Guilt) per side; the Compare view
   *  multiplies the windowed (max HP + healing-to-cutoff) pool by this for a
   *  mode-aware Effective HP. Optional/defaults to 1 for legacy summaries. */
...
export type SimulationDebug = {
  ...
  biteCount?: number;
  /** Clock time of every breath tick the side fired, in order. */
