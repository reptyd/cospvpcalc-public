...
function createTrackedOnlyStatusEntry(
  ...
  return {
    ...
    notes: [
      ...(polarity === "negative" ? ["Polarity is negative - Fortify cleanses it."] : []),
...
export const KNOWN_APPROXIMATION_REFERENCE_DRAFTS: ApproximationReferenceEntry[] = [
  ...
  },
  {
    ...
    name: "Fixed Bite Cadence",
    summary: "A fight decided partly by movement is modeled as one number: a fixed bite cooldown in place of the creature's own.",
    gameTruth: [
      "In game a creature that keeps its distance - in the air, or running in and out on the ground - bites at a rate set by how it moves.",
    ],
    currentApproximation: [
      "While a side is set to a fixed cadence, its bite cooldown is that number of seconds.",
      "The two sides use the same value unless they are given different ones.",
      "A fixed cadence is not changed by any status or ability that would otherwise move the bite cooldown.",
      "Distance, altitude and pursuit are not modeled; the bite cooldown is the only channel the setting changes.",
    ],
    whyApproximated: "The stand-and-fight model has no positions to move between, so the rate of biting is the only part of a moving fight it can carry.",
    notes: [],
  },
  {
...
export const BATTLE_SETTING_REFERENCE_DRAFTS: AbilityReferenceEntry[] = [
  {
    ...
    summary: "Each incoming bite and breath tick has a set chance to land on a side; it dodges the rest.",
    mechanics: [
      "Each side is given the percentage of incoming attacks that land on it, 25% by default.",
      "The chance applies whether or not the creature flies.",
      "A creature counts as flying when it has a fly speed or a take-off stat.",
      "An aquatic creature that has either stat counts as flying.",
      "Shredded Wings stops a flying creature from dodging, so every attack on it lands.",
      "Shredded Wings does not change the dodge of a creature that does not fly.",
  ...
  },
  {
    ...
    name: "Fixed Bite Cadence",
    status: "Battle setting",
    summary: "Replaces a side's bite cooldown with a fixed number of seconds.",
    mechanics: [
      "A side set to a fixed cadence bites on that interval for the whole fight.",
      "Both sides take one value by default, and each side can be given its own.",
      "A side can instead keep its own bite cooldown while the other side is on a fixed cadence.",
      "A fixed cadence is not changed by any status or ability that would otherwise move the bite cooldown.",
    ],
    policyDifferences: [],
    notes: [
...
export const STATUS_REFERENCE_DRAFTS: StatusReferenceEntry[] = [
  ...
  createTrackedOnlyStatusEntry("Shock", "negative"),
  {
    ...
    name: "Shredded Wings",
    status: "Modeled",
    summary: "Stops a flying creature from dodging under the Dodge Chance rule.",
    mechanics: [
      "Wing Shredder, Power Charge, Crystal Breath, Rock Breath, Yolk Bomb and a Bad Omen follow-up all apply Shredded Wings.",
      "While a flying creature has Shredded Wings, every incoming bite and breath lands on it.",
      "It does not change the dodge of a creature that does not fly.",
      "With the Dodge Chance rule off, the status has no separate combat effect in the stand-and-fight model.",
    ],
    notes: [],
  },
  {
...
export const MOVEMENT_SPEED_REFERENCE_DRAFTS: StatusReferenceEntry[] = [
  {
    ...
    mechanics: [
      ...
      "The ambush multiplier is never below 1.",
    ],
    notes: [
  ...
  },
  {
    ...
    status: "Speed-Builds-only",
    summary: "Only plushies that can move a movement channel are varied.",
    mechanics: [
