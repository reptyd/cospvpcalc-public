...
import { DEFAULT_COMPARE_AIR_RULE_COOLDOWN_SEC, isCompareAirRuleEligible } from "../engine/compareAirRule";
import {
...
export default function ComparePage({
  ...
  // Round 42 / A11: per-fight level picks for user abilities with
  ...
  const airRuleEligible = isCompareAirRuleEligible(creatureA) || isCompareAirRuleEligible(creatureB);
  ...
  useEffect(() => {
    if (!airRuleEligible) setCompareAirRuleEnabled(false);
  }, [airRuleEligible]);
  useEffect(() => {
  ...
  const { finalA, finalB, summary, needsCalc, calculate } = useCompareSimulation({
    creatureA,
    creatureB,
    buildA,
    buildB,
      ...
      <div className="layout-grid">
        <div className="panel-grid">
          ...
          <BattleSettingsPanel
            ...
            compareDpsSettings={compareDpsSettings}
            airRuleEligible={airRuleEligible}
