...
import { readBetaResultsCache, writeBetaResultsCache } from "./betaResultsCache";
import { getBbCustomPool, setBbCustomPool } from "../shared/bbCustomPool";
...
// The slice of post-run state worth surviving a tab switch (see
// betaResultsCache). The results plus the run-summary metadata the page header
// + dev diagnostic read; transient run state (progress/isRunning/errors) is
// deliberately excluded - a run can't outlive the unmount.
type BestBuildsResultsCacheBundle = {
  ...
  topResultDiagnostic: BestBuildsTopResultDiagnostic | null;
};
...
export function useBestBuildsPageController({
  ...
  /** Gates the top-1 diagnostic. It re-fights the winner against the whole pool
   * on the main thread after the workers are done, and only the developer panel
   * renders it - so for everyone else it is pure added wall clock (19 s on
   * Korathos / meta80). */
  ...
  // Seed from the persisted Search->BB pool: if the user queued opponents on the
  // beta Search page, Best Builds opens straight into Custom mode with them
  // pre-filled; otherwise the usual meta default.
  const initialSharedPool = getBbCustomPool();
  const [poolMode, setPoolMode] = useState<BestBuildPoolMode>(
    initialSharedPool.length > 0 ? "custom" : "meta80",
  );
  const [poolScope, setPoolScope] = useState<DefaultPoolScope>("withinOneTier");
  const [customPoolText, setCustomPoolText] = useState(() => encodeCreaturePoolCode(initialSharedPool));
  const [customPickerQuery, setCustomPickerQuery] = useState("");
  const [selectedPoolTiers, setSelectedPoolTiers] = useState<number[]>([]);
  // Restore the last run for THIS source creature on mount (lazy initializers);
  // a different nameA misses the signature and starts fresh.
  const cachedBundle = readBetaResultsCache<BestBuildsResultsCacheBundle>("bestBuilds", nameA);
  const [results, setResults] = useState<BestBuildAggregateResult[]>(() => cachedBundle?.results ?? []);
  const [progress, setProgress] = useState(0);
  const [isRunning, setIsRunning] = useState(false);
  const [lastRunMs, setLastRunMs] = useState<number | null>(() => cachedBundle?.lastRunMs ?? null);
  const [lastRunTimings, setLastRunTimings] = useState<(BestBuildsStageTimings & { candidatePrepMs: number }) | null>(() => cachedBundle?.lastRunTimings ?? null);
  const [lastRunRuntimePathTelemetry, setLastRunRuntimePathTelemetry] = useState<BestBuildsRuntimePathTelemetry | null>(() => cachedBundle?.lastRunRuntimePathTelemetry ?? null);
  const [runtimeRequirementError, setRuntimeRequirementError] = useState<string | null>(null);
  const [topResultDiagnostic, setTopResultDiagnostic] = useState<BestBuildsTopResultDiagnostic | null>(() => cachedBundle?.topResultDiagnostic ?? null);
  ...
  // Mirror the custom pool out to the shared store so picks made on the beta
  // Search page and edits made in the pool builder stay consistent. The store
  // no-ops when contents are unchanged, so this can't loop with the seed above.
  useEffect(() => {
    setBbCustomPool(customPool);
  }, [customPool]);
  ...
  // Persist the post-run bundle to the cross-tab cache so a tab switch doesn't
  // discard the search. Keyed by nameA (the source creature identity).
  useEffect(() => {
    writeBetaResultsCache<BestBuildsResultsCacheBundle>("bestBuilds", nameA, {
      results,
      lastRunMs,
      lastRunTimings,
  ...
  }, [nameA, results, lastRunMs, lastRunTimings, lastRunRuntimePathTelemetry, topResultDiagnostic]);
  ...
  const runBestBuilds = async () => {
    ...
    if (!cancelRef.current) {
      ...
      setProgress(1);
      if (developerMode && flowResult.results.length > 0) {
