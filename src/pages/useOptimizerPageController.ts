...
// The Optimizer fights a single fixed opponent, so it can afford Compare's full
// 900s horizon (vs Best Builds' cheaper 480s pool cap). Resolving the fight
// instead of capping at 480s stops a slow win from being truncated into a
// time-cap draw, which the win-rate counts as a win - the Golgaroth-vs-
// Caldonterrus "100% win rate on a Draw" report. Matches COMPARE_MAX_TIME_SEC
// so the Optimizer's verdict agrees with Compare.
const OPTIMIZER_MATCHUP_MAX_TIME_SEC = 900;
...
export function useOptimizerPageController({
  ...
  // Results survive a tab switch (the page unmounts on navigation) via a
  // module-level cache keyed by the matchup creature identities - see
  // betaResultsCache. The lazy initializer restores the last run for THIS
  // matchup; a name change elsewhere yields a signature miss -> fresh empty.
  const resultsCacheKey = `${nameA}::${nameB}`;
  const [results, setResults] = useState<BestBuildAggregateResult[]>(
    () => readBetaResultsCache<BestBuildAggregateResult[]>("optimizer", `${nameA}::${nameB}`) ?? [],
  );
  ...
  // Persist results to the cross-tab cache whenever they change, under the
  // current matchup signature, so they're restored on remount.
  useEffect(() => {
    writeBetaResultsCache("optimizer", resultsCacheKey, results);
  }, [resultsCacheKey, results]);
  ...
  const runOptimizer = async () => {
    ...
    try {
      const flowResult = await runBestBuildsFlow({
        ...
        combatEventOrder,
        battleSettings,
        // Counter-build mode: the fixed target is built with the user's
        // specified Creature A build (the on-page Veneration / Traits /
        // Ascension / Plushies / Elder editor), so B is optimized against the
        // actual opponent build the user set - not a bare creature. An explicit
        // Battle Settings opponent-baseline override still takes precedence.
        defaultOpponentBaselineBuild: fixedBuildA,
        maxTimeSec: OPTIMIZER_MATCHUP_MAX_TIME_SEC,
      });
