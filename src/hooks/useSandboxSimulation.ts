...
export function useSandboxSimulation(input: UseSandboxSimulationInput): UseSandboxSimulationResult {
  ...
  const payload = useMemo(() => {
    ...
    if (battleSettings) {
      ...
      // Per-side combatant-stat rules (Special Air PvP Rule cooldown + Aerial
      // Dodge hit chance). Without this, Aerial Dodge's active flag would land
  ...
  const provisionSandbox = useCallback(async () => {
    ...
    try {
      ...
      const createPayload = Array.isArray(payload.config.badOmenOutcomes)
        ? { ...payload, config: { ...payload.config, badOmenOutcomes: rollBadOmenBatch() } }
        : payload;
      const { id, view: initialView } = await createSandbox(createPayload);
