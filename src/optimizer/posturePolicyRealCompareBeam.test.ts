...
//   so there are no two code paths for the same decision.
//
...
describe("posture policy vs math ideal at real Compare conditions", () => {
  ...
  // natural, x4 in settled Lay = 0.75 s per stack). The "lay to clear
  ...
  //
  // Does this beat OFF, and how does it compare to the registered
  ...
  it("Opra vs Gimon - natural lay-to-clear-Cause-Fear strategy", async () => {
    ...
    // Several lay-window variants. Fear has 10 stacks, x4 decay in Lay
    ...
    // into x4 decay at T+2. Effective lay-decay window:
    // [T+2, standUp_time].
  ...
  // per-bite trade cleanly. Cost: 2 x N_bites simulations per matchup
