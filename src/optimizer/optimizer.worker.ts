...
workerScope.onmessage = async (event: MessageEvent<IncomingMessage>) => {
  try {
    ...
    if ((payload as BestBuildsPhase2Job).kind === "bestBuildsPhase2") {
      ...
      const { bestBuildsResults, pathCounts, workUnits } = await runBestBuildsWorkerJob(phaseJob);
      postOptimizerWorkerResponse({ id: phaseJob.id, bestBuildsResults, bestBuildsPathCounts: pathCounts, bestBuildsWorkUnits: workUnits } satisfies OptimizerWorkerResponse);
      return;
    }
