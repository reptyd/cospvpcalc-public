...
// These cases cover the legacy staged path (stage-1 screen -> shortlist ->
// stage-2 sweep), which the funnel now replaces by default but which still
// serves as the fallback. Pinning the gate off here keeps that path tested.
vi.mock("./bbFunnel/bbFunnelFlag", () => ({
  BB_FUNNEL_ENABLED: false,
}));
