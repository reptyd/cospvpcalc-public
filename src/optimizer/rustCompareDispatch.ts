...
// ---------------------------------------------------------------------------
// Compare -> Rust dispatch (plan A wiring).
//
...
export type TrySimulateRustCompareInput = {
  ...
  /** Per-fight user-ability level picks (Compare-time). */
  ...
  /** Compare-page day/night + moon UI knobs. Forwarded
   ...
   *  moistureTime pools ride the shared stat producer; "off" / undefined is
   *  inert. */
  ...
  /** Nearby-radiated rule: extra radiated creatures sharing the area. Scales
   *  each fighter's Radiation DOT. Matchup-level (linked across both sides). */
...
function mapRustDebug(rust: RustSimulationDebug): SimulationDebug {
  return {
    ...
    breathFireTimes: rust.breathFireTimes,
...
function adaptRustToSimulationSummary(rust: RustMatchupSummary): SimulationSummary {
  ...
  const summary: SimulationSummary = {
    ...
    ehpA: rust.ehpA,
    ehpB: rust.ehpB,
    ehpMitigationMultA: rust.ehpMitigationMultA ?? 1,
    ehpMitigationMultB: rust.ehpMitigationMultB ?? 1,
    winner: rust.winner,
...
export async function trySimulateRustCompareMatchup(
  ...
  const args = toRustComposableArgsFromCompare({
    ...
    breathPolicyA: input.breathPolicyA,
    breathPolicyB: input.breathPolicyB,
    ...
    compareAirRuleCooldownSecB: input.compareAirRuleCooldownSecB,
    compareAirRuleCooldownLinked: input.compareAirRuleCooldownLinked,
    compareAerialDodgeEnabled: input.compareAerialDodgeEnabled,
    compareAerialDodgeHitChancePctA: input.compareAerialDodgeHitChancePctA,
    compareAerialDodgeHitChancePctB: input.compareAerialDodgeHitChancePctB,
    compareAerialDodgeRollStyle: input.compareAerialDodgeRollStyle,
    compareAerialDodgeSeed: input.compareAerialDodgeSeed,
  ...
  try {
    ...
    // Distinguish a genuine engine throw (panic / serde mismatch) from a
    // legitimately ineligible matchup, which also returns null but does not
    // throw. Reporting here surfaces the failure in the /diagnose overlay and
    // any wired sink instead of swallowing it as an empty result.
    reportError("rustCompareDispatch.simulate", err, {
      attackerKeys: Object.keys(args.attacker ?? {}),
      attackerBreath:
        typeof args.attackerBreath === "object" && args.attackerBreath ? "obj" : String(args.attackerBreath),
      defenderBreath:
        typeof args.defenderBreath === "object" && args.defenderBreath ? "obj" : String(args.defenderBreath),
      policy: args.abilityPolicy,
    });
    return null;
  }
}
