...
export async function runBestBuildsPhase2WorkerExecution({
  ...
  timeoutMs?: number;
  /** Terminate each worker as it runs dry. Callers that keep a pool across
   * several executions pass `false` and dispose it themselves - a terminated
   * worker answers nothing, so the next execution would time out every chunk
   * and fall back to the main thread. */
  ownsWorkers?: boolean;
}): Promise<{ results: BestBuildsPhase2Result[]; pathCounts: BestBuildsPathCounts; workUnits: number }> {
  ...
  await new Promise<void>((resolve) => {
    ...
    // Chunks a timed-out worker abandoned. They go back to the front of the
    // queue so a live worker retries them; only a run with no workers left
    // falls through to the main thread.
    const requeued: number[] = [];

    const dispatchNext = (worker: Worker): number | null => {
      if (cancelRef.current) return null;
      const idx = requeued.shift() ?? chunkIndex;
      if (idx >= chunks.length) return null;
      if (idx === chunkIndex) chunkIndex += 1;
    ...
    const finishWorker = (worker: Worker) => {
      if (ownsWorkers) worker.terminate();
      else {
      ...
      active -= 1;
      if (active <= 0) {
        for (const idx of requeued.splice(0)) runChunkFallback(idx);
        resolve();
      }
    };
    ...
    workers.forEach((worker, workerIdx) => {
      ...
      const armTimeout = () => {
        timeoutId = timerApi.setTimeout(() => {
          ...
          // The worker is still computing the chunk we just gave up on. Keeping
          // it would burn a core and, worse, let its late answer land against a
          // chunk id that has already moved on - so it is terminated outright,
          // regardless of who owns the pool.
          worker.terminate();
          if (currentChunkId != null && !cancelRef.current) {
            if (aliveWorkers > 1) requeued.push(currentChunkId);
            else runChunkFallback(currentChunkId);
          }
          aliveWorkers -= 1;
          active -= 1;
          if (active <= 0) {
            // Nothing left to hand the requeued chunks to.
            for (const idx of requeued.splice(0)) runChunkFallback(idx);
            resolve();
          }
        }, timeoutMs);
      ...
      worker.onmessage = (event: MessageEvent<OptimizerWorkerResponse>) => {
        ...
        if (payload.error) {
          ...
          workUnits += payload.bestBuildsWorkUnits ?? 0;
  ...
  return cancelRef.current ? { results: [], pathCounts: {}, workUnits: 0 } : { results, pathCounts, workUnits };
}
