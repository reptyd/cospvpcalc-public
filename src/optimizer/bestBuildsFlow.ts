...
// Order skeletons by the objective's combat metric, breaking ties by how well
// the build's composition fits the objective. The tie-break is what stops the
// pure-regen Health trait from winning DPS/TTK ties (where the 0-ascension
// second trait barely moves the metric) via `compareAggregate`'s survival
// fallback - and equally lets regen win a survival tie. Applied at every
// selection point so an off-objective build can't ride ties into the shortlist.
function compareSkeletonByObjective(
  a: RankedSkeletonAggregate,
  b: RankedSkeletonAggregate,
  objective: BestBuildAggregateObjective,
): number {
  const metric = compareAggregate(a.aggregate, b.aggregate, objective);
  if (metric !== 0) return metric;
  const toBuild = (s: BestBuildFlowSkeleton): BuildOptions => ({
    venerationStage: s.venerationStage,
    traits: s.traits,
    ascensionAssignments: s.ascensionAssignments ?? ["", "", "", "", ""],
    plushies: s.plushies,
    elder: s.elder,
  });
  return objectiveBuildFit(toBuild(b.skeleton), objective) - objectiveBuildFit(toBuild(a.skeleton), objective);
}
...
function collectRankedShortlist(
  ...
  const primaryRanked = [...rankedEntries].sort((a, b) => compareSkeletonByObjective(a, b, primaryObjective));
...
export function buildStageShortlists({
  ...
  const quickRanked = collectRankedShortlist(
    ...
    stage1TopK,
  ).sort((a, b) => compareSkeletonByObjective(a, b, stage1PrimaryObjective));
  ...
  const stage2Skeletons = collectRankedShortlist(
    ...
    .sort((a, b) => compareSkeletonByObjective(a, b, stage2PrimaryObjective))
    .map((x) => x.skeleton);
