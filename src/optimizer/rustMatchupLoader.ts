...
/**
 * TEST-ONLY: install a pre-built bridge so `getLoadedRustMatchupBridge` (and
 * every BB dispatcher that reads it) resolves against a WASM instance the test
 * already owns. The Node real-WASM tests instantiate the engine once via
 * `__wbg_set_wasm` (a second instance corrupts the shared memory view); this
 * setter lets that single instance back the production bridge accessor without
 * routing through `loadRustMatchupBridge`, whose wasm-pack `.js` init path is
 * browser-oriented. Never called in the shipped bundle.
 */
export function __setLoadedRustMatchupBridgeForTests(bridge: LoadedRustMatchupBridge | null): void {
  loadedBridge = bridge;
}
...
export type GeneratedRustModule = {
  ...
  // Best Builds pin-replay funnel (Phase 2a). Capture both sides' defensive
  // schedule from a full `ideal` run, then replay it pinned under `GateOnly`.
  // Ship in the same rebuild as `simulate_composable_matchup_js`, so required.
  capture_defensive_pin_schedule_js: (
    attacker: RustSimpleCombatantStats,
    defender: RustSimpleCombatantStats,
    attackerBreath: RustSimpleBreathProfile | null | undefined,
    defenderBreath: RustSimpleBreathProfile | null | undefined,
  ...
  simulate_composable_matchup_pinned_js: (
    attacker: RustSimpleCombatantStats,
    defender: RustSimpleCombatantStats,
    attackerBreath: RustSimpleBreathProfile | null | undefined,
    defenderBreath: RustSimpleBreathProfile | null | undefined,
  ...
  // Batched fight rectangle - one crossing for many (build x opponent) pairs.
  // Optional so the loader keeps working against a rust-pkg built before it.
  simulate_composable_matchup_batch_js?: (batch: RustComposableMatchupBatch) => unknown;
...
/** Raised when the loaded binary reports a wire contract other than the one
 *  this bundle was generated against - almost always a `.wasm` committed
 *  before a `contracts.rs` change, i.e. a skipped `npm run rust:build`. In
 *  production it degrades like any other load failure; in development it is
 *  rethrown, because a silent banner is how the stale binary went unnoticed
 *  in the first place. */
class ContractSchemaMismatchError extends Error {
  constructor(loaded: string) {
    super(
      `WASM wire contract ${loaded} does not match the ${CONTRACT_SCHEMA_HASH} this bundle expects. ` +
        "The committed binary predates a contracts.rs change - run `npm run rust:build`.",
    );
    this.name = "ContractSchemaMismatchError";
  }
}
...
export async function loadRustMatchupBridge(): Promise<RustMatchupBridge | null> {
  ...
  bridgePromise = loadGeneratedRustModule()
    .then((mod) => {
      const loadedContract = mod.rust_matchup_contract_version();
      if (loadedContract !== CONTRACT_SCHEMA_HASH) {
        throw new ContractSchemaMismatchError(loadedContract);
      }
      ...
      loadedBridge = {
        ...
        captureDefensivePinSchedule: (
          ...
          markCall("rust:capture_defensive_pin_schedule", () =>
            mod.capture_defensive_pin_schedule_js(
              stripNullsForWasm(attacker),
              stripNullsForWasm(defender),
              stripNullsForWasm(attackerBreath ?? undefined),
              stripNullsForWasm(defenderBreath ?? undefined),
              stripNullsForWasm(abilityConfig),
              maxTimeSec,
            ) as RustPinnedDefensiveSchedule,
          ),
        simulateComposableMatchupPinned: (
          attacker: RustSimpleCombatantStats,
          defender: RustSimpleCombatantStats,
          attackerBreath: RustSimpleBreathProfile | null,
          defenderBreath: RustSimpleBreathProfile | null,
          ...
          markCall("rust:simulate_composable_matchup_pinned", () =>
            mod.simulate_composable_matchup_pinned_js(
              stripNullsForWasm(attacker),
              stripNullsForWasm(defender),
              stripNullsForWasm(attackerBreath ?? undefined),
              stripNullsForWasm(defenderBreath ?? undefined),
              stripNullsForWasm(abilityConfig),
              stripNullsForWasm(pinnedSchedule),
              maxTimeSec,
              recordTrace,
            ) as RustMatchupSummary,
          ),
        ...(mod.simulate_composable_matchup_batch_js
          ? {
              // One strip pass over the deduplicated pools instead of one per
              // fight - that walk is a real slice of a cheap fight's cost.
              simulateComposableMatchupBatch: (batch: RustComposableMatchupBatch) =>
                markCall("rust:simulate_composable_matchup_batch", () =>
                  mod.simulate_composable_matchup_batch_js!(
                    stripNullsForWasm(batch),
                  ) as RustMatchupSummary[],
                ),
            }
          : null),
    ...
    .catch((error) => {
      ...
      // A stale binary is a build mistake, not a runtime condition, so it is
      // not something to degrade past while developing.
      if (import.meta.env?.DEV && error instanceof ContractSchemaMismatchError) throw error;
      return null;
    });
