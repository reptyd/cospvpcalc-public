...
export function selectCandidateTraitIds(): string[] {
  // Always offer all four stat traits, including Health. The legacy
  // regen-relevance heuristic + solo/dummy strip stay disabled - candidates are
  // fully simulated and ranked by the real objective, with an objective-fit
  // tie-break at every selection point, so a build that doesn't help the
  // objective simply loses its ties and won't rank. (We deliberately do NOT
  // blanket-exclude Health for damage objectives: that would mask the real
  // reason it surfaced - ties broken by survival - rather than fix it.)
  return traits
