...
describe("rustBestBuildsRuntime composable eligibility", () => {
  ...
  it("builds Best Builds default timing overrides for Warden's Rage only", () => {
    // After the policy rework the Hunker (DEFAULT_ABILITY_TIMING_OVERRIDES) and
  ...
  it("falls back to effects catalog values when the creature's stored value is empty", () => {
    // Shadow Barrage is present but value-less on the creature, so resolution
    // must fall back to the effects catalog. Both the empty creature value AND
    // the catalog value (7) are owned by this test - a wiki sync can't change
    // what it asserts, yet it still fails loudly if the fallback wiring breaks.
    ...
    const config = withRegisteredFixture(
      sourceCreature,
    ...
    expect(config.attackerShadowBarrageValue).toBe(7);
  });
...
describe("Stubborn Stacker Rust marshalling", () => {
  ...
  it("threads the Gentle elder's +10% all-ailment block into the Rust stats as an umbrella scalar", () => {
    // The elder ailment block (Gentle = +10% to ALL ailments) is the game's
    // umbrella BlockAilment. It rides its own `elderBlockFraction` scalar,
    // applied to every incoming ailment, and is kept out of the per-ailment
    // `plushieStatusBlockFractions` map so Radiation cannot weaken it.
    ...
    expect(noElderRust.elderBlockFraction ?? 0).toBe(0);
    expect(noElderRust.plushieStatusBlockFractions?.["Bleed_Status"] ?? 0).toBe(0);
    // Gentle's +10% rides the umbrella scalar, not the per-ailment map.
    expect(gentleRust.elderBlockFraction).toBeCloseTo(0.1, 6);
    expect(gentleRust.plushieStatusBlockFractions?.["Bleed_Status"] ?? 0).toBe(0);
  });

  it("threads the Astral Quetzal plushie breath-resistance boost into the Rust stats", () => {
    // Regression: toRustStatusMeleeStats fed only the Breath Resistance ability
    // value and dropped finalStats.breathResistance (the plushie contribution),
    // so Astral Quetzal's +50% never reached Compare / Best Builds. Synthetic
    // creature so the ability value is test-owned, not a live game number.
    const c = makeSyntheticCreature({
      name: "SynthBreathResister",
      passiveAbilities: [makeAbility("Breath Resistance", 0.3)],
    });
    const build = { venerationStage: 0, traits: [], ascensionAssignments: ["", "", "", "", ""] };
    const noPlushie = applyRulesAndBuild(c, { ...build, plushies: [] });
    const withQuetzal = applyRulesAndBuild(c, { ...build, plushies: ["Astral Quetzal"] });

    // No base breathResistance stat, so finalStats carries only the plushie.
    expect(noPlushie.breathResistance ?? 0).toBeCloseTo(0, 6);
    expect(withQuetzal.breathResistance ?? 0).toBeCloseTo(0.5, 6);

    // Ability value alone without the plushie...
    expect(__test_toRustStatusMeleeStats(c, noPlushie).breathResistance).toBeCloseTo(0.3, 6);
    // ...ability value + the +50% plushie, clamped to [0,1].
    expect(__test_toRustStatusMeleeStats(c, withQuetzal).breathResistance).toBeCloseTo(0.8, 6);
  });
...
describe("creature-identity marshalling", () => {
...
describe("custom breath profile", () => {
  it("returns the authored profile (bypassing name lookup) with build buffs on top", () => {
    ...
    expect(buffed?.dpsPct).toBeCloseTo(6, 6); // 4 x 1.5
