...
describe("best builds common wins ranking", () => {
  ...
  it("ranks survival by absolute average lifespan, not wins — a tanky loser beats a fast winner", () => {
    const ranked = rerankBestBuildsByCommonWinsData([
      {
        // Wins almost everything, but dies fast when it does fight on.
        result: result("fast-killer", 0.9, 8, 900, 9000, 90),
        rows: [
          { opponentName: "A", winner: "A", ttk: 8, dps: 900, effective: 9000, survival: 90 },
          { opponentName: "B", winner: "A", ttk: 8, dps: 900, effective: 9000, survival: 90 },
        ],
      },
      {
        // Loses most matchups, but outlives everyone (highest avgSurvival).
        result: result("tanky-loser", 0.2, 180, 30, 1000, 170),
        rows: [
          { opponentName: "A", winner: "B", ttk: 180, dps: 30, effective: 1000, survival: 170 },
          { opponentName: "B", winner: "B", ttk: 180, dps: 30, effective: 1000, survival: 170 },
        ],
      },
    ], "survival");

    // Pure survivability: longest absolute lifespan wins even though it loses
    // and has the lower win rate.
    expect(ranked[0].build.traits[0]).toBe("tanky-loser");
    expect(ranked[1].build.traits[0]).toBe("fast-killer");
    expect(ranked[0].aggregate.avgSurvival).toBe(170);
  });
  ...
  it("breaks a DPS tie toward the offence build, not the higher-survival regen build", () => {
    // Bite5 + {Weight0 | Health0}: the 0-ascension second trait barely moves
    // avgDps, so these TIE on the DPS metric. The Health (regen) build has the
    // higher survival, which under the old `compareAggregate` survival fallback
    // made it win every such tie - flooding DPS tops with the Health trait.
    // The objective-fit tie-break must now hand the win to the offence build.
    const mk = (second: "Weight" | "Health"): { result: BestBuildAggregateResult; rows: CommonWinsOutcomeRow[] } => ({
      result: {
        build: {
          venerationStage: 5,
          traits: ["Bite", second],
        ...
        aggregate: {
          winRate: 0.8,
          drawRate: 0,
          // Regen build outlives - the old tiebreak's survival key favoured it.
          avgSurvival: second === "Health" ? 200 : 150,
          avgDps: 1000,
          avgTtkWin: 10,
          avgImmortalDamage: 5000,
        },
      ...
      rows: [
        { opponentName: "A", winner: "A", ttk: 10, dps: 1000, effective: 5000, survival: 180 },
        { opponentName: "B", winner: "A", ttk: 10, dps: 1000, effective: 5000, survival: 180 },
      ],
    });
    // Health listed first to prove ordering isn't just input order.
    const ranked = rerankBestBuildsByCommonWinsData([mk("Health"), mk("Weight")], "avgDps");
    expect(ranked[0].build.traits).toContain("Weight");
    expect(ranked[0].build.traits).not.toContain("Health");
  });

  it("breaks a winRate tie toward the offence build, not the higher-survival regen build", () => {
    // Same as above but for the winRate objective - the one whose tiebreak was
    // forgotten. Identical win rate + identical common-win combat, but the
    // Health (regen) build has the higher survival. The win-rate ranking used to
    // fall through to survival (common-win, then aggregate) and hand it the tie,
    // flooding win-rate tops with the un-leveled Health trait. The offence
    // build-fit tiebreak must now win.
    const mk = (second: "Weight" | "Health"): { result: BestBuildAggregateResult; rows: CommonWinsOutcomeRow[] } => ({
      result: {
        build: {
          venerationStage: 5,
    ...
    const ranked = rerankBestBuildsByCommonWinsData([mk("Health"), mk("Weight")], "winRate");
    expect(ranked[0].build.traits).toContain("Weight");
    expect(ranked[0].build.traits).not.toContain("Health");
  });
