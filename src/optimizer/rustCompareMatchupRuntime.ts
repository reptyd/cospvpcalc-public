...
// ---------------------------------------------------------------------------
// Compare → Rust composable bridge (Phase 3 step 5 harness).
//
...
export type CompareToRustInput = {
  ...
  /** Round 42 / A11: per-fight active-level override map for user
   ...
   * `env.*` expression namespace for user abilities (round 32 / A5).
...
export function toRustAbilityPolicyOverrides(
  ...
  // Round 42 / A11: per-fight level picks. Engine clamps stale or
...
export function toRustComposableArgsFromCompare(input: CompareToRustInput): CompareToRustOutput {
  ...
  // P3 moved variant selection into the Rust engine (see
  // `wasm-engine/src/policy/decisions/bite_variant.rs`). The bridge now
  // forwards `damage` and `damage2` both unconditionally (Step 2a) and
  ...
  // same observable behavior the pre-P3 substitution did (damage2 +
  ...
  // Round 32 / A5: forward day/night + moon strings verbatim so the
  ...
  // P3: forward the per-side bite-variant mode picked from the
