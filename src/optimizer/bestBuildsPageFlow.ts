...
import { candidateGoalForObjective } from "./candidateGenerationScoring";
...
export async function runBestBuildsFlow({
  ...
  /** Opponent build used when battleSettings carries no opponent-baseline
   * override. Best Builds leaves this unset (falls back to the strong
   * BEST_BUILDS_OPPONENT_BUILD); the Optimizer passes the user's fixed
   * Creature A build so the counter is judged against the actual opponent. */
  defaultOpponentBaselineBuild?: BuildOptions;
  /** Per-matchup fight ceiling forwarded to every scoring stage. The
   * single-opponent Optimizer passes Compare's 900s so a fight that resolves
   * past 480s isn't truncated into a time-cap draw (which the win-rate then
   * counts as a win); Best Builds leaves it unset and keeps its pool caps. */
  maxTimeSec?: number;
  ...
  // The funnel keeps the breath `ideal` policy off its screening stages and
  // runs it only on the band it completes, so the setting is affordable here
  // and no longer dropped on the way in.
  const extraAbilityConfig = buildBestBuildsExtraAbilityConfig(battleSettings, true);
  ...
  const opponentBaselineBuild = buildBestBuildsOpponentBaselineBuild(battleSettings) ?? defaultOpponentBaselineBuild;
  ...
  const candidates = await createOptimizerCandidates({
    ...
    searchToggles: false,
    // Candidate generation goal - shapes the pre-score heuristic AND the trait
    // pool per objective (see candidateGoalForObjective). e.g. avgDps/avgTtk ->
    // "dps" (offence pre-score; the pure-regen Health trait is dropped, since it
    // can't help damage rate / kill speed); survival -> "survival" (bulk/regen
    // weighted). Previously hard-coded to "lexicographic" for everything.
    goal: candidateGoalForObjective(objective),
...
export async function loadPerOpponentRowsFlow({
  ...
  return loadPerOpponentRows({
    ...
    item,
    combatEventOrder,
    extraAbilityConfig: buildBestBuildsExtraAbilityConfig(battleSettings, true),
    ...
    opponentBaselineBuild: buildBestBuildsOpponentBaselineBuild(battleSettings) ?? defaultOpponentBaselineBuild,
  });
}
