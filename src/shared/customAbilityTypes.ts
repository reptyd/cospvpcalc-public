...
export type EffectKind =
  ...
  /** Round 35 / A3: apply N statuses in one effect through the
  ...
  | {
       ...
       * adapter (Sprint 5) reads modifiers and applies them when
  ...
  | {
       ...
       * adapter (Sprint 5) resolves the id via the user-ability
  ...
  | {
      /**
       * v2 step 7: glass-cannon ↔ tank form swap. Sugar over `modify_stat`:
  ...
  /** Round 35 / A3: remove N statuses in one effect. Each id processed
  ...
  // Round 46 / B3: numbered-key extras arrays.
  ...
  /** Round 33 / A4: set when the named status next fires its DOT tick.
  ...
  /** Round 34 / A2: weighted one-of-N exclusive picker. Evaluates
  ...
  /** Round 38 / A12: find the first queued entry with the matching
...
export type EffectBatch = {
  name: string;
  effects: EffectKind[];
  /** Round 45 / B6: optional batch-level gate. When present and the
   ...
   * / undefined evaluates as "no gate" (pre-B6 behavior). */
  when?: Expr;
};
...
export type UserAbilitySpec = {
  ...
  /**
   * Round 40 / A11: total number of levels this ability has. `1`
   ...
   * clamped to `1..=levels`). Compare UI will override this per
   * matchup in a later round; for now this is the effective level at
   * dispatch time.
   */
...
export type TriggerHooks = {
  ...
  /** Round 37 / A7: fired when actor received healing this iteration.
  ...
  on_heal?: EffectBatch;
  /** Round 37 / A7: fired when one or more user `active_until` windows
  ...
  /** Round 43 / A13: fires on the victim immediately before HP
   ...
   * are pass-through this round). Handler can write
  ...
  /** Round 43 / A13: symmetric — fires on the dealer just before its
...
// ── User-defined statuses (Phase 6 / G6) ─────────────────────────────────
...
/**
 * User-defined status spec (Custom Abilities v2 Phase 6 / G6). Mirrors the
...
export type UserStatusSpec = {
  ...
  // ── Phase 9 (programmable statuses): optional ability-like extensions.
...
/**
 * Constructor-coverage lock for user statuses (Phase 8 / G6).
 *
