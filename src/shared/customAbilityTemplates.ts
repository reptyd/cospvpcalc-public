...
export const ABILITY_TEMPLATES: AbilityTemplate[] = [
  ...
  },
  {
    ...
    description:
      "Active on cooldown: swap into a 12s glass-cannon form (×2 damage, ×0.5 weight), preserving HP fraction, then auto-revert. Showcases form_swap (Phase 2 / G10).",
    build: ({ id, display_name }) => ({
      id,
      display_name,
      utility: { kind: "var", path: "self.bite_dps" },
  ...
  },
  {
    ...
    description:
      "Active vs Fliers only: 400 bonus damage when the opponent's creature type is Flier, on an 8s cooldown. Showcases the is_type creature-attribute read (Phase 5 / G8).",
    build: ({ id, display_name }) => ({
      id,
      display_name,
      utility: {
  ...
  },
  {
    ...
    description:
      "Passive: apply 3 stacks of your own user.MyDoT custom status to the opponent on every bite dealt. Define it first under Custom > Statuses (id user.MyDoT), then edit the id here. Showcases user-defined statuses (Phase 6 / G6).",
    build: ({ id, display_name }) => ({
      id,
      display_name,
      utility: { kind: "const", value: 0 },
