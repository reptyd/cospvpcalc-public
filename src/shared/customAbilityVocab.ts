...
export const EXPR_GLOBAL_PATHS: ReadonlyArray<string> = [
  ...
  // Cumulative per-fight combat counters (perspective
  // side = whoever owns the firing spec). Bite-only for now;
  // breath / DOT / trap sources don't accumulate yet.
  "combat.bites_dealt",
  "combat.bites_taken",
  "combat.damage_dealt_total",
  "combat.damage_taken_total",
  // Compare-page environment flags. Seeded at simulation
  ...
  //   env.air_rule_active   - Compare "Fixed Bite Cadence" enabled
  "env.is_day",
  "env.is_night",
  "env.is_blue_moon",
  "env.is_blood_moon",
  "env.air_rule_active",
  // Per-ability scaling table. `<key>` is one of the
...
export const EXPR_SIDE_FIELDS: ReadonlyArray<string> = [
  ...
  // Numeric creature tier (ordinal - `opp.tier >= 4`) and
 ...
 *
 * Includes `is_posture` (Standing / Sitting / Laying) plus
 * `is_type` / `is_diet` / `is_elder`. The numeric
...
export const EXPR_EVENT_FIELDS: ReadonlyArray<string> = [
  ...
  "first_strike_active",
  // Damage-kind flags. 0 or 1. Set inside
  ...
  // currently leaves all three at 0 - these flags cover the common
  // anti-bite / anti-breath / anti-DOT reaction surface; more
  // flags may be added later.
  "is_bite",
  "is_breath",
  "is_dot",
  // Heal amount inside on_heal trigger.
  "heal_amount",
  // Count of user active windows that expired this
  // iteration inside on_active_end trigger. Per-id flags live under
  // event.ended.<ability_id> (see EXPR_EVENT_ACTIVE_END_FAMILY).
  "ended_count",
  // Pre-mitigation damage totals. Available inside
  ...
  // Only bite damage instruments these for now; breath / DOT / traps
...
/**
 * Inside an `on_active_end` trigger, the engine writes
 ...
 * used for applied/expired statuses. Reads of unknown ids
...
/**
 * Inside an `on_status_apply` / `on_status_expire` trigger,
...
/**
 * Compose every var-path the engine resolver currently
...
export function buildVarSuggestions(): string[] {
  ...
  // Identity / posture families - boolean builtins.
