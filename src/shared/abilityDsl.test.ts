...
describe("printAbility round-trip", () => {
  ...
  on_fire:
    ...
    repeat 3:
      deal 50 to opp`,
    // Round 41 / A11: levels + scaling round-trip.
...
describe("levels + scaling parse — Round 41 / A11", () => {
...
`);
  ...
  it("omits levels block when single-level (pre-A11 default)", () => {
    const spec = parse(`
...
describe("batch-level `when` gate (B6) — DSL round-trip", () => {
...
describe("Round 35 / A3 — apply_statuses_to_target + clear_statuses (array DSL forms)", () => {
...
describe("Round 33-34 / A4 + Round 38 / A12 — status-timer + named-schedule DSL forms", () => {
...
describe("Round 34 / A2 — choose: block DSL form", () => {
