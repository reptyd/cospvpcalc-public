...
function validateExpr(expr: Expr, path: string, errors: string[]): void {
  switch (expr.kind) {
    ...
    case "clamp":
      ...
      return;
    case "rand":
      // Round 34 / A1: no-operand. Nothing to validate.
      return;
    default: {
...
function validateEffect(effect: EffectKind, path: string, errors: string[]): void {
  switch (effect.kind) {
    ...
    case "apply_statuses_to_target":
      // Round 35 / A3: array variant. Empty array allowed (no-op);
    ...
    case "chance":
      ...
      return;
    case "choose":
      // Round 34 / A2: one-of-N weighted picker. Need at least one
...
function validateBatch(batch: EffectBatch, path: string, errors: string[]): void {
  ...
  // Round 45 / B6: validate the optional gate expr.
  if (batch.when) {
...
function countSpecComplexity(spec: UserAbilitySpec): number {
  ...
  if (spec.triggers) {
    ...
    // 2026-05-12: triggers added across R37 + R43 that were missing
    // from the complexity tally. on_heal / on_active_end (R37);
    // on_before_take_damage / on_before_deal_damage (R43). Pre-fix
...
export function validateUserAbility(spec: UserAbilitySpec): ValidationResult {
  ...
  // Round 40 / A11: validate level fields. The Rust path normalizes
