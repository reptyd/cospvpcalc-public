...
export function printAbility(spec: UserAbilitySpec): string {
  ...
  // Round 41 / A11: levels block. Emit only when the spec has actual
  // level shape — single-level abilities (the pre-A11 default) print
  // exactly as before.
...
// Phase 9: the status DSL composes the SAME effect grammar for its hooks
...
class Parser {
   ...
   * gate (B6). `parseEffectBlock` doesn't handle `when` directly
  ...
  parseBlockEffect(line: TokenLine): EffectKind | null {
    ...
    // The `as <name>` suffix is the Round 38 / A12 named-schedule form
    // that pairs with `cancel_schedule` / `reschedule`. Omitting it
    // keeps the pre-A12 fire-and-forget semantics.
    ...
    //   weight <expr>:
    //     <effects>
    // Round 34 / A2: weighted one-of-N picker. Every child of `choose:`
...
function parseEffectStatement(text: string, lineNo: number): EffectKind {
  ...
  // Round 35 / A3 array form. Must precede the single-status pattern
  ...
  // Round 33 / A4: re-arm the named status's next DOT-style tick. Engine
  ...
  // Round 38 / A12: remove all queued scheduled entries on the caster's
  ...
  // Round 38 / A12: find the first queued entry with the matching `name`
